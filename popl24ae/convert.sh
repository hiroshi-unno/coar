...
cat results_spacer.csv | while IFS=, read file answer time _; do
...
cat results_pcsat.csv | while IFS=, read file answer time _; do
    file_name=$(basename $file)
    if [[ "$answer" == "timeout" ]]; then
        res="-"
        tim="timeout"
...
cat results_cps.csv | while true; do
