...
module Make (Config : Config.ConfigType) = struct
  ...
  let is_pure = function Sort.Pure -> true | _ -> false
  let is_eff = function Sort.Eff _ -> true | _ -> false
  let extract_ev = function Sort.EVar ev -> Some ev | _ -> None
  ...
  module Constr = struct
    type t = {
      ...
      sub_ev_eff :
        (evar * (Sort.o * Sort.t * Sort.e * Sort.o * Sort.t * Sort.e)) list;
      sub_evs_ev : (evar list * evar) list;
      ecomps :
        (Sort.e list * (Sort.o * Sort.t * Sort.e * Sort.o * Sort.t * Sort.e))
        list;
    }
  ...
  let rec simplify_esubs (state : SimplState.t) =
    ...
    | (e1, e2) :: esubs -> (
        ...
        | e, Sort.Pure ->
            let state =
              SimplState.map_res
                (SimplResult.add_eeqls [ (e, Sort.Pure) ])
                state
            in
            ...
            let state =
              ...
              |> SimplState.map_res
                   (SimplResult.add_eeqls
                      [ (e, Sort.Eff (ov1, sv1, ev1, ov2, sv2, ev2)) ])
              |> SimplState.add_osubs [ (o1, ov1); (ov2, o2) ]
              |> SimplState.add_ssubs [ (sv1, t1); (t2, sv2) ]
              |> SimplState.add_esubs [ (ev1, e1); (e2, ev2) ]
            in
            simplify_esubs state
        | Sort.EVar ev1, Sort.EVar ev2 ->
            let state =
              SimplState.map_constr (Constr.add_sub_ev_ev (ev1, ev2)) state
            in
            simplify_esubs state
        | Sort.EVar ev, Sort.Eff (o1, t1, e1, o2, t2, e2) ->
            let state =
              SimplState.map_constr
                (Constr.add_sub_ev_eff (ev, (o1, t1, e1, o2, t2, e2)))
                state
            in
            simplify_esubs state
        | Sort.Pure, Sort.EVar ev ->
            let state =
              SimplState.map_constr (Constr.add_sub_pure_ev ev) state
            in
            simplify_esubs state
    ...
    | (t1, t2) :: ssubs -> (
            ...
            let state =
              ...
              |> SimplState.map_res
                   (SimplResult.add_seqls
                      [ (t, Sort.SArrow (sv1, (ov, sv2, ev))) ])
              |> SimplState.add_esubs [ (ev, e) ]
              |> SimplState.add_osubs [ (o, ov) ]
              |> SimplState.add_ssubs [ (t1, sv1); (sv2, t2) ]
            in
        ...
        | t, T_array.SArray (t1, t2) ->
            ...
            let state =
              state
              |> SimplState.map_res
                   (SimplResult.add_seqls [ (t, T_array.SArray (sv1, sv2)) ])
        ...
        | t, T_dt.SDT dt when Fn.non List.is_empty (Datatype.params_of dt) ->
            ...
            let state =
              ...
              |> SimplState.map_res
                   (SimplResult.add_seqls
                      [ (t, T_dt.SDT (Datatype.update_params dt svs)) ])
              |> SimplState.add_ssubs
                   (List.map2_exn svs args ~f:(fun sv s -> (sv, s)))
            in
            simplify_ssubs state
        | t, T_tuple.STuple (_ :: _ as ss) ->
            ...
            let state =
              state
              |> SimplState.map_res
                   (SimplResult.add_seqls [ (t, T_tuple.STuple svs) ])
              |> SimplState.add_ssubs
                   (List.map2_exn svs ss ~f:(fun sv s -> (sv, s)))
            in
            simplify_ssubs state
            ...
            let state =
              ...
              |> SimplState.map_res
                   (SimplResult.add_seqls
                      [ (t, T_dt.SDT (Datatype.update_params dt svs)) ])
              |> SimplState.add_ssubs
                   (List.map2_exn args svs ~f:(fun s sv -> (s, sv)))
            in
            simplify_ssubs state
        | T_tuple.STuple (_ :: _ as ss), t ->
            ...
            let state =
              ...
              |> SimplState.map_res
                   (SimplResult.add_seqls [ (t, T_tuple.STuple svs) ])
              |> SimplState.add_ssubs
                   (List.map2_exn ss svs ~f:(fun s sv -> (s, sv)))
            in
            simplify_ssubs state
        | Sort.SVar sv1, Sort.SVar sv2 ->
            let state =
              SimplState.map_constr (Constr.add_sub_sv_sv (sv1, sv2)) state
            in
            simplify_ssubs state
        | _, T_dt.SDT _
        | _, T_tuple.STuple []
        ...
        | T_regex.SRegEx, _ ->
            let state =
              SimplState.map_res (SimplResult.add_seqls [ (t1, t2) ]) state
            in
            simplify_ssubs state
  ...
  let rec simplify_osubs (state : SimplState.t) =
    ...
    | (o1, o2) :: osubs -> (
        ...
        | Sort.OpSig (opmap1, Some rv1), Sort.OpSig (opmap2, Some rv2) ->
            let lefts, boths, rights =
              Common.Util.ALMap.split_lbr opmap1 opmap2
            in
            let rv = mk_fresh_rvar () in
            let opsigr =
              Sort.OpSig (Common.Util.ALMap.of_alist_exn rights, Some rv)
            in
            let opsigl =
              Sort.OpSig (Common.Util.ALMap.of_alist_exn lefts, Some rv)
            in
            let state =
              state
              |> SimplState.map_res
                   (SimplResult.add_oeqls
                      [ (Sort.mk_empty_open_opsig_from_rvar rv1, opsigr) ])
              |> SimplState.map_res
                   (SimplResult.add_oeqls
                      [ (Sort.mk_empty_open_opsig_from_rvar rv2, opsigl) ])
        ...
        | Sort.OpSig (opmap1, None), Sort.OpSig (opmap2, Some rv2) ->
            ...
            in
            let opsigr =
              Sort.OpSig (Common.Util.ALMap.of_alist_exn rights, None)
            in
            let opsigl =
              Sort.OpSig (Common.Util.ALMap.of_alist_exn lefts, None)
            in
            let state =
              state
              |> SimplState.map_res
                   (SimplResult.add_oeqls [ (Sort.empty_closed_opsig, opsigr) ])
              |> SimplState.map_res
                   (SimplResult.add_oeqls
                      [ (Sort.mk_empty_open_opsig_from_rvar rv2, opsigl) ])
              |> SimplState.add_ssubs (List.map boths ~f:snd)
    ...
    let rec go (res : SimplResult.t) = function
      ...
      | (effs, eff) :: ecompsubs ->
          if is_pure eff then
            let res =
              SimplResult.add_eeqls
                (List.map effs ~f:(fun e -> (e, Sort.Pure)))
                res
            in
            go res ecompsubs
          else if List.exists effs ~f:is_pure then
            let ecompsubs =
              (List.filter effs ~f:(not <<< is_pure), eff) :: ecompsubs
            in
            go res ecompsubs
            ...
            let res =
              res
              |> SimplResult.add_eeqls
                   [ (eff, Sort.Eff (ovn, svn, evn, ov0, sv0, ev0)) ]
              |> SimplResult.map_constr
                   (Constr.add_ecomp (effs, (ovn, svn, evn, ov0, sv0, ev0)))
            in
            go res ecompsubs
          else
            let unwrap_ev e =
              Option.value_exn ~message:"unknown effect" (extract_ev e)
            in
            let evs = List.map effs ~f:unwrap_ev in
            let ev = unwrap_ev eff in
            let res =
              SimplResult.map_constr (Constr.add_sub_evs_ev (evs, ev)) res
            in
            go res ecompsubs
    in
    go res
  ...
  let rec unify_eeqls (state : UniState.t) =
    ...
    | (e1, e2) :: eeqls -> (
        ...
        if Stdlib.(e1 = e2) then unify_eeqls state
          ...
          match (e1, e2) with
          | ( Sort.Eff (o11, t11, e11, o12, t12, e12),
              Sort.Eff (o21, t21, e21, o22, t22, e22) ) ->
              let state =
                state
                |> UniState.add_oeqls [ (o11, o21); (o12, o22) ]
                |> UniState.add_seqls [ (t11, t21); (t12, t22) ]
                |> UniState.add_eeqls [ (e11, e21); (e12, e22) ]
              in
              unify_eeqls state
        ...
        if Stdlib.(s1 = s2) then unify_seqls state
          ...
          | T_dt.SDT dt1, T_dt.SDT dt2 ->
              ...
              if List.length args1 = List.length args2 then
                let state =
                  UniState.add_seqls (List.zip_exn args1 args2) state
                in
                unify_seqls state
              else fail ()
  ...
  module PreprocState = struct
    ...
    let from_subst_constr (subst : Subst.t) (constr : Constr.t) : t =
      ...
      let esubs_from_ev_eff, sub_ev_eff =
        List.partition_map constr.sub_ev_eff
          ~f:(fun (ev, (o1, s1, e1, o2, s2, e2)) ->
            ...
            | None -> Second (ev, (o1, s1, e1, o2, s2, e2)))
      in
      let esubs =
        List.concat [ esubs_from_ev_ev; esubs_from_pure_ev; esubs_from_ev_eff ]
      in
      let ecompsubs, sub_evs_ev =
        List.partition_map constr.sub_evs_ev ~f:(fun (evs, ev) ->
            ...
            if List.exists es ~f:Either.is_first || Either.is_first e then
              let f =
                Either.value_map ~first:Fn.id ~second:(fun ev -> Sort.EVar ev)
              in
      ...
      let ecomps =
        List.map constr.ecomps ~f:(fun (es, (o1, s1, e1, o2, s2, e2)) ->
            ...
            let s2 = subst_to_sort s2 in
            let e2 = subst_to_cont e2 in
            (List.map es ~f:subst_to_cont, (o1, s1, e1, o2, s2, e2)))
      in
      {
        res =
          {
  ...
  let rec preprocess (state : PreprocState.t) =
    ...
    then state.res
      ...
      let simpl_res =
        simplify_ecompsubs
          { constr = state.res.constr; eeqls = []; seqls = []; oeqls = [] }
          state.ecompsubs
      in
      let simpl_res =
        fully_simplify_subs
      ...
      in
      let state =
        PreprocState.from_subst_constr uni_res.subst simpl_res.constr
      in
      preprocess state
  ...
  let rec search_subs ~embed (state : SearchState.t) =
    ...
    | (ev, (o1, s1, e1, o2, s2, e2)) :: sub_ev_eff -> (
        ...
        let assume_eff () =
          ...
          let ev2 = Sort.mk_fresh_evar () in
          let preproc_res =
            SearchState.add_subst_cont
              (ev, Sort.Eff (ov1, sv1, ev1, ov2, sv2, ev2))
              state
          ...
          let state : SearchState.t =
            { constr = preproc_res.constr; subst = preproc_res.subst; embedded }
          in
          search_subs ~embed state
        in
        let assume_pure () =
          let preproc_res =
            preprocess (SearchState.add_subst_cont (ev, Sort.Pure) state)
          in
          let state =
            {
              state with
        ...
        in
        let assume1, assume2 =
          if embed then (assume_eff, assume_pure) else (assume_pure, assume_eff)
        in
  ...
  let rec search_comps ~embed (state : SearchState.t) =
    ...
    | (es, (o_first, s_first, e_first, o_last, s_last, e_last)) :: ecomps -> (
        ...
        | [] ->
            ...
            let preproc_res =
              preprocess
                (PreprocState.from_subst_constr uni_res.subst state.constr)
            in
            let state =
              {
                state with
        ...
        | Sort.Pure :: es ->
            let ecomps =
              (es, (o_first, s_first, e_first, o_last, s_last, e_last))
              :: ecomps
            in
            let state = { state with constr = { state.constr with ecomps } } in
            search_comps ~embed state
        | Sort.Eff (o1, s1, e1, o2, s2, e2) :: es ->
            ...
            in
            let ecomps =
              (es, (o_first, s_first, e_first, o1, s1, e1)) :: ecomps
            in
            let state = { state with constr = { state.constr with ecomps } } in
            let preproc_res =
              preprocess
        ...
        | Sort.EVar ev :: es -> (
            let assume_eff () =
              ...
              let ecomps =
                (es, (o_first, s_first, e_first, ov_new, sv_new, ev_new))
                :: ecomps
              in
              let state =
                { state with constr = { state.constr with ecomps } }
              in
              let preproc_res =
                SearchState.add_subst_cont
                  (ev, Sort.Eff (ov_new, sv_new, ev_new, o_last, s_last, e_last))
                  state
                |> preprocess
              in
              let embedded = Set.add state.embedded ev in
            ...
            let assume_pure () =
              ...
              in
              let preproc_res =
                SearchState.add_subst_cont (ev, Sort.Pure) state |> preprocess
              in
              let state =
                {
                  state with
        ...
        let effs =
          List.filter_map evs ~f:(fun ev ->
              Option.filter (Map.Poly.find state.subst.etheta ev) ~f:(fun e ->
                  is_eff e && not (Set.mem state.embedded ev)))
        in
        Option.value_map (List.hd effs) ~default:[] ~f:(fun eff ->
            List.map evs ~f:(fun ev ->
                let e =
                  Option.value
                    (Map.Poly.find state.subst.etheta ev)
                    ~default:(Sort.EVar ev)
                in
                (e, eff)))
        @ eeqls)
  ...
  let rec fully_search ~embed ~ev_equiv (state : SearchState.t) =
    if
      List.is_empty state.constr.sub_ev_eff && List.is_empty state.constr.ecomps
    then state
    else
      ...
      let preproc_res =
        ...
        in
        preprocess (PreprocState.from_subst_constr uni_res.subst state.constr)
      in
      let state =
  ...
  let resolve_svs ~init_svs (constr : Constr.t) =
    ...
    let rec go subst = function
      ...
      | (sv1, sv2) :: sub_sv_sv ->
          let sv, sv' =
            if Set.mem init_svs sv1 then (sv1, sv2) else (sv2, sv1)
          in
          go
  ...
  let discriminate_evs_cont ecompsub =
    List.fold ecompsub ~init:(Map.Poly.empty, [])
      ~f:(fun (ev_map, ecompsub) (effs, eff) ->
        let evs =
          Set.Poly.union_list (List.map (eff :: effs) ~f:Term.evs_of_cont)
        in
        let ev_map, etheta =
          Set.fold evs ~init:(ev_map, Map.Poly.empty)
            ~f:(fun (ev_map, etheta) ev ->
              let new_ev = ev (*mk_fresh_evar ()*) in
              let ev_map =
                Map.Poly.update ev_map ev ~f:(fun new_evs ->
                    new_ev :: Option.value new_evs ~default:[])
              in
              let etheta =
                Map.Poly.add_exn etheta ~key:ev ~data:(Sort.EVar new_ev)
              in
              (ev_map, etheta))
        in
  ...
  let discriminate_evs_opsig osub =
    List.fold osub ~init:(Map.Poly.empty, []) ~f:(fun (ev_map, osub) (o1, o2) ->
        let evs =
          Set.Poly.union_list (List.map [ o1; o2 ] ~f:Term.evs_of_opsig)
        in
        let ev_map, etheta =
          Set.fold evs ~init:(ev_map, Map.Poly.empty)
            ~f:(fun (ev_map, etheta) ev ->
  ...
  let solve eff_constrs opsig_constrs : solution =
    ...
    let ev_equiv =
      Map.Poly.merge ev_equiv ev_equiv' ~f:(fun ~key:_ data ->
          match data with
    ...
    let init_evs =
      ...
      @@ Set.Poly.of_list (List.concat_no_order (Map.Poly.data ev_equiv))
         :: List.map osubs ~f:(fun (o1, o2) ->
                Set.Poly.union_list
                  [ Term.evs_of_opsig o1; Term.evs_of_opsig o2 ])
    in
    let init_svs =
      Set.Poly.union_list
      @@ List.map (Set.to_list eff_constrs) ~f:(fun (es, e) ->
             Set.Poly.union_list
               (Term.svs_of_cont e :: List.map es ~f:Term.svs_of_cont))
    ...
    let init_rvs =
      ...
      @@ List.map (Set.to_list eff_constrs) ~f:(fun (es, e) ->
             Set.Poly.union_list
               (Term.rvs_of_cont e :: List.map es ~f:Term.rvs_of_cont))
      @ List.map (Set.to_list opsig_constrs) ~f:(fun (o1, o2) ->
    ...
    let subst_purest =
      postprocess ~init_evs ~init_svs ~init_rvs search_res_purest
    in
