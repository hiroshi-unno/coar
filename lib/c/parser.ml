...
module ElimFuncall : sig
  ...
  *)
  let rec mk_inline_funcall nxt_label_id fundecls params args labelname return_replaced_funbody =
    ...
    let pa_int = List.filter ~f:(fun ((_, sort), _) -> Term.is_int_sort sort) zipped in
    let pa_unref = List.filter ~f:(fun ((_, sort), _) -> Term.is_int_ref_sort sort) zipped in
    ...
    in
    Statement.mk_compound
      ((List.map ~f:(fun ((varname, _), _) -> Statement.mk_vardecl varname T_int.SInt) pa_int)
       @ (List.map ~f:(fun ((varname, _), arg) -> Statement.mk_assign varname arg) pa_int)
    ...
    if Statement.is_call_assign stmt then
      ...
      if FunDecl.is_fun_nondet fundecl then
        ...
        if FunDecl.is_non_recursive fundecls fundecl then
          ReturnReplacer.replace_return ~f:(fun stmt ->
              if Statement.is_return_int stmt then
                let term = Statement.let_return_int stmt in
                Statement.mk_compound
      ...
      if Statement.is_nop body then
        ...
        let labelname, nxt_label_id = mk_labelname funname nxt_label_id in
        if FunDecl.is_non_recursive fundecls fundecl then
          ReturnReplacer.replace_return ~f:(fun _ -> Statement.mk_goto labelname) body
          |> mk_inline_funcall nxt_label_id fundecls params args labelname
        else assert false
    else if Statement.is_if stmt then
      let cond_fml, t_stmt, f_stmt = Statement.let_if stmt in
...
module ElimVardecl : sig
  ...
  let rec elim_vardecl_rep renamed_vars stmt =
    if Statement.is_call_assign stmt then
      let varname, funname, args = Statement.let_call_assign stmt in
      Statement.mk_call_assign (rename renamed_vars varname) funname
        (List.map args ~f:(rename_term renamed_vars)),
      renamed_vars
    else if Statement.is_call_voidfun stmt then
      let funname, args = Statement.let_call_voidfun stmt in
      Statement.mk_call_voidfun funname (List.map args ~f:(rename_term renamed_vars)),
      renamed_vars
    else if Statement.is_if stmt then
      let cond_fml, t_stmt, f_stmt = Statement.let_if stmt in
      ...
      let varname, term = Statement.let_assign stmt in
      Statement.mk_assign (rename renamed_vars varname) (rename_term renamed_vars term),
      renamed_vars
    else if Statement.is_nondet_assign stmt then
      let varname = Statement.let_nondet_assign stmt in
      Statement.mk_nondet_assign (rename renamed_vars varname),
      renamed_vars
    else if Statement.is_assume stmt then
      let fml = Statement.let_assume stmt in
      Statement.mk_assume (rename_formula renamed_vars fml),
      renamed_vars
...
let read_file file =
  ...
  let res = ref "" in
  try
    while true; do res := !res ^ In_channel.input_line_exn ic ^ "\n" done;
    assert false
  with End_of_file ->
    In_channel.close ic;
    !res
...
module Preprocess : sig
  ...
  let type_of_preprocess line =
    assert (Str.string_match reg_type_of line 0); Str.matched_group 1 line
  ...
  let rec string_repeat str times = if times = 0 then "" else str ^ string_repeat str (times - 1)
  ...
  let preprocess program =
    ...
    let program, _ =
      (List.fold_left ~f:(fun (program, line_number) line ->
           let line_number = line_number + 1 in
           if Fn.non is_preprocess line then
             program, line_number
             ...
             if is_define_macro line then
               ...
               let params =
                 ...
                 |> (fun params -> if String.equal params "" then [] else String.split_on_chars ~on:[','] params)
               in
