...
module rec BuchiAutomaton : sig
  ...
  val mk_ba: states:int -> initial_state:int -> final_states:int Set.Poly.t -> transition:(int * 'a) list array -> 'a t
  val init_ba: states:int -> initial_state:int -> final_states: int list -> transition: (int * int * 'a) list -> 'a t
  ...
  let mk_ba ~states ~initial_state ~final_states ~transition = BA (states, initial_state, final_states, transition)
  let init_ba ~states ~initial_state ~final_states ~transition =
    ...
    List.iter transition ~f:(fun (from_state_id, to_state_id, label_or_cond) ->
        graph.(from_state_id) <- (to_state_id, label_or_cond) :: graph.(from_state_id));
    mk_ba ~states ~initial_state ~final_states ~transition:graph

  let let_ba = function BA (states, initial_state, final_states, transition) -> states, initial_state, final_states, transition

  let get_state_count = function BA (states, _, _, _) -> states
  let get_initial_state_id = function BA (_, initial_state, _, _) -> initial_state
...
module rec HMES : sig
  ...
  let get_ftv = function Hmes (preds, _) ->
    List.fold_left ~init:Variables.empty preds
      ~f:(fun vars (_, _, fml) -> Variables.union vars (HMESFormula.get_ftv fml))
end
and HMESFormula : sig
  type t =
      And of t * t
  ...
  let rec of_formula fml =
    if Formula.is_and fml then
      ...
      failwith "HMESFormula.of_formula: not implemented or invalid formula"
...
and HMESAtom : sig
  ...
  let let_predapp = function PredApp (sym, args) -> sym, args | _ -> assert false
    ...
    if Atom.is_true atom then
      ...
      if Predicate.is_var pred then
        ...
        if List.is_empty args then
          ...
          failwith "HMESAtom.of_atom: pvar must have the type: () -> bool"
      else
        failwith "HMESAtom.of_atom: not implemented"
    else
      failwith "HMESAtom.of_atom: not implemented or invalid atom"
  ...
  let atom_of = function
    ...
    | PredApp (sym, args) -> Atom.mk_app (Predicate.mk_psym sym) args ~info:Dummy
...
module rec Statement : sig
  ...
  let let_unref_assign = function UNREF_ASSIGN (varname, term) -> varname, term | _ -> assert false
  ...
  let let_vardecl = function VARDECL (varname, sort) -> varname, sort | _ -> assert false
  ...
  let varname_of_assign = function ASSIGN(varname, _) | NONDET_ASSIGN varname -> varname | _ -> assert false
  ...
  let rec subst sub stmt =
    ...
    | RETURN_INT term ->
      mk_return_int (Term.subst sub term)
    | LABEL _
    | GOTO _
    | RETURN_VOID
    | RETURN_NONDET
  ...
  let rec get_label_references_rep ?(nxt_stmt=Statement.mk_nop ()) ?(res=Hashtbl.Poly.create ~size:10 ()) = function
    | IF (_, stmt1, stmt2)
    | NONDET (stmt1, stmt2) ->
    ...
    | COMPOUND (stmt1, stmt2) ->
      ...
      res
    | LOOP stmt ->
      get_label_references_rep ~nxt_stmt:stmt ~res stmt
    | LABEL label_name ->
  ...
  let get_all_labels stmt =
    get_all_labels_rep Variables.empty stmt
    |> Variables.to_list

  let rec get_read_vars_rep label_refs used_labels = function
    | IF (cond_fml, t_stmt, f_stmt) ->
      let vars1, used_labels = get_read_vars_rep label_refs used_labels t_stmt in
      let vars2, used_labels = get_read_vars_rep label_refs used_labels f_stmt in
      Formula.tvs_of cond_fml
      |> Variables.of_tvarset
      |> Variables.union vars1
      |> Variables.union vars2,
      used_labels
    | LOOP stmt' ->
      get_read_vars_rep label_refs used_labels stmt'
    | ASSIGN (_, term) ->
      Term.tvs_of term
      |> Variables.of_tvarset,
      used_labels
    ...
    | NONDET (stmt1, stmt2)
    | COMPOUND (stmt1, stmt2) ->
      let vars1, used_labels = get_read_vars_rep label_refs used_labels stmt1 in
      let vars2, used_labels = get_read_vars_rep label_refs used_labels stmt2 in
      Variables.union vars1 vars2,
      used_labels
    | ASSUME fml ->
      Formula.tvs_of fml
      |> Variables.of_tvarset,
...
module rec Init : sig
  ...
  let update_formula_E fml = function
    ...
    | NONDET_ASSIGN _ -> fml
    | ASSUME fml' ->
      Formula.mk_and fml' fml ~info:Dummy
  ...
  let subst sub = function
    ...
    | ASSUME fml ->
      mk_assume (Formula.subst sub fml)
    | NONDET_ASSIGN varname ->
      mk_nondet_assign varname

  let is_already_assigned inits varname =
    List.exists inits ~f:(function
          ASSIGN (varname', _) | NONDET_ASSIGN varname' -> String.equal varname' varname
        | ASSUME _ -> false)
  ...
  let rec of_stmt_rep stmt inits_rev =
    if Statement.is_compound stmt then
      ...
      match nxt_stmt with
        None ->
        of_stmt_rep stmt2 inits_rev
      | Some nxt_stmt ->
        inits_rev, Some (Statement.mk_compound nxt_stmt stmt2)
    else if Statement.is_assign stmt then
      let varname, term = Statement.let_assign stmt in
      if is_already_assigned inits_rev varname then
        inits_rev,
        Some stmt
      else
        ASSIGN (varname, term) :: inits_rev,
        None
...
and State : sig
  ...
  let of_variables variables =
    let state =
      ...
      |> List.map ~f:(fun varname -> varname, ref (Term.mk_var (Ident.Tvar varname) T_int.SInt))
    in
    STATE state
  ...
  let get varname = function STATE state ->
    !(List.Assoc.find_exn ~equal:String.equal state varname)

  let mem varname = function STATE state ->
    List.exists ~f:(fun (varname', _) -> String.equal varname' varname) state
...
module FunDecl : sig
  ...
  let let_fun_int = function FUN_INT (funname, params, body) -> funname, params, body | _ -> assert false
  let let_fun_void = function FUN_VOID (funname, params, body) -> funname, params, body | _ -> assert false
  let let_fun = function FUN_INT (funname, params, body) | FUN_VOID (funname, params, body) -> funname, params, body | _ -> assert false
  ...
  let get_next_funnames fundecl =
    ...
    get_next_funnames_rep body Variables.empty
    |> Variables.to_list
  ...
  let rec is_non_recursive_rep fundecls fundecl ancestors used =
    ...
    |> List.fold_left
      ~f:(fun (res, used) funname' ->
          ...
          if not res then
            false, used
          else if Variables.is_mem ancestors funname' then
            true, used
          else if Variables.is_mem used funname' then
            true, used
          else
