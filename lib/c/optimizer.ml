...
module Util : sig
  ...
  val remove_nobranch: query:LinkedStatement.t -> LinkedStatement.t -> LinkedStatement.t list -> LinkedStatement.t
...
module ReadGraph = struct
  ...
  let rgenv_mem tvar = function RGENV rgenv -> List.Assoc.mem ~equal:Stdlib.(=) rgenv tvar
  ...
  let string_of_rgenv query_stmt = function RGENV rgenv ->
    string_of ~info:(fun stmt ->
  ...
  let rec rc_of_term_rep varname term res =
    if Term.is_var term then
      ...
      if String.equal (Ident.name_of_tvar tvar) varname then
        res + 1
      else
        res
      ...
      failwith "rc_of_term_rep: not implemented"
  ...
  let of_term varname stmt term =
    of_rc stmt @@ rc_of_term_rep varname term 0

  let rec get_rg_dfs varname prev_env stmt res: rg_t =
    let rs =
      List.fold ~init:empty (get_next_statements stmt)
    ...
    if is_assign stmt then
      ...
      if String.equal varname' varname then res
      else update res rs
...
module RemoveAssign : sig
  ...
  let remove_stmt stmt_key stmt_keys (query_stmt_key, new_stmts) =
    ...
    | [next_stmt_key] ->
      if not @@ phys_equal stmt_key next_stmt_key then
        let query_stmt_key = Util.remove_nobranch ~query:query_stmt_key stmt_key stmt_keys in
  ...
  let sub_assign_one init_state rgenv stmt_key stmt_keys (query_stmt_key, new_stmts) =
    if StatementEnv.exists stmt_key new_stmts then
      let stmt = StatementEnv.get stmt_key new_stmts in
      get_read_vars stmt
      |> Variables.to_list
      |> List.fold ~init:(query_stmt_key, new_stmts)
        ~f:(fun (query_stmt_key, new_stmts) varname ->
            ...
            if ReadGraph.rgenv_mem tvar rgenv then
              ...
              in
              let value =
                List.fold ~init:init_value stmt_keys
  ...
  let remove_unused_assign_one rgenv stmt_key stmt_keys (query_stmt_key, new_stmts) =
    if StatementEnv.exists stmt_key new_stmts then
      let stmt = StatementEnv.get stmt_key new_stmts in
      if is_assign stmt || is_nondet_assign stmt then
        ...
        if ReadGraph.rgenv_mem tvar rgenv &&
           (*ToDo*)Stdlib.(ReadGraph.rgenv_get tvar rgenv |> ReadGraph.rg_get !nxt_stmt_key = Never) then
          remove_stmt stmt_key stmt_keys (query_stmt_key, new_stmts)
        else
          (* case: <tvar> \in <phi> *)
          query_stmt_key, new_stmts
  ...
  let remove_unused_assign stmts spec_fv query_stmt =
    ...
    let query_stmt, _ =
      List.fold ~init:(query_stmt, new_stmts) stmts
        ~f:(fun (query_stmt, new_stmts) stmt -> remove_unused_assign_one rgenv stmt stmts (query_stmt, new_stmts))
    in
    Util.replace_stmt ~query:query_stmt new_stmts
...
module RemoveNop : sig
  ...
  let optimize_stmt query_stmt =
    ...
    List.fold ~init:query_stmt stmts ~f:(fun query_stmt stmt ->
        if is_nop stmt then
          Util.remove_nobranch ~query:query_stmt stmt stmts
        else query_stmt)
  ...
  let optimize_stmt spec_fv query_stmt =
    ...
    let noreplace_stmts =
      List.fold ~init:(StatementEnv.mk_empty ()) stmts ~f:(fun used stmt ->
          if is_assume stmt
          || Fn.non Variables.is_empty (Variables.inter (get_written_vars stmt) spec_fv) then
