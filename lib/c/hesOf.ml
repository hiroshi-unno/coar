...
let fresh_pvar env phi =
  ...
  in
  let id = List.fold_left ~init:1 env
      ~f:(fun id (_, _, pvar) ->
          if String.is_prefix (Ident.name_of_pvar pvar) ~prefix:pvarname then id + 1 else id)
  in
  ...
  let state =
    Variables.union
      ...
      (ReadGraph.rgenv_entries rgenv
       |> List.filter ~f:(fun (_, rg) -> ReadGraph.rg_get stmt rg |> ReadGraph.length > 0)
       |> List.map ~f:(fst >> Ident.name_of_tvar)
       |> Variables.of_list)
    |> State.of_variables
  in
  let pvar_opt =
    List.find ~f:(fun (phi', stmt', _) -> phys_equal phi phi' && phys_equal stmt stmt') env
  in
  let env, pvar =
    match pvar_opt with
    | None -> let pvar = fresh_pvar env phi in (phi, stmt, pvar) :: env, pvar
...
let next_formula_of_phi prev_env env phi = formula_of_phi prev_env env (Ctl.let_unop phi)
...
let hesfunc_of_c_one prev_env env phi stmt =
  ...
  let env, body =
    if LinkedStatement.is_assign stmt then
      ...
      let binop = if Ctl.is_a phi then Formula.Imply else Formula.And
      in
      env,
...
let rec hes_of_c_rep_stmt prev_env (env, preds_rev) phi stmt =
  ...
  let exists =
    List.exists preds_rev
      ~f:(fun func -> let _, pvar', _, _ = MuCLP.Pred.let_ func in Stdlib.(pvar = pvar'))
  in
  if exists then
    env, preds_rev
  else
    ...
    |> List.fold_left ~init:(env, preds_rev) ~f:(fun (env, preds_rev) nxt_stmt ->
...
let rec hes_of_c_rep_phi prev_env (env, preds_rev) phi stmt =
  let dispose_next_phis phi (env, preds_rev) =
    List.fold_left (get_next_phis phi) ~init:(env, preds_rev)
      ~f:(fun (env, preds_rev) phi' -> hes_of_c_rep_phi prev_env (env, preds_rev) phi' stmt)
  in
...
let query_of_c prev_env env (phi, _, inits, stmt) =
  ...
  let bounds = Formula.tvs_of fml |> Set.to_list |> List.map ~f:(fun tvar -> tvar, T_int.SInt)
  in
...
let hes_of_cctl ~print (phi, decls, inits, stmt) =
  ...
  print @@ lazy "";
  let _, decls, inits, stmt = Optimizer.optimize ~print (Ctl.get_fv phi) decls inits stmt in
...
let hes_of_chmes ~print (hmes, decls, inits, query_stmt) =
  ...
  let _, _, inits, query_stmt = Optimizer.optimize ~print hmes_fv decls inits query_stmt in
  print @@ lazy "vvvvvvvvvvvvv Optimized Linked C vvvvvvvvvvvvv";
  print @@ lazy (LinkedStatement.string_of query_stmt);
  print @@ lazy "";
  ...
  List.iteri ~f:(fun idx stmt -> LinkedStatementHashtbl.add stmt_to_id stmt idx) stmts;
  let id_of_stmt stmt = LinkedStatementHashtbl.find stmt_to_id stmt in
  let pvar_of stmt hmes_pvar =
    Ident.Pvar (sprintf "L%d%s" (id_of_stmt stmt + 1) (Ident.name_of_pvar hmes_pvar))
  in
  let state_of stmt =
  ...
  let formula_of_dia stmt hmes_pvar =
    if LinkedStatement.is_assign stmt then
      ...
      Formula.mk_and
        ...
        (Formula.mk_imply (Formula.mk_neg cond_fml ~info:Dummy) f_fml ~info:Dummy)
        ~info:Dummy
    else if LinkedStatement.is_nondet stmt then
  ...
  let atom_of hmes_atom =
    if HMESAtom.(is_true hmes_atom || is_false hmes_atom || is_predapp hmes_atom) then
  ...
  let rec formula_of stmt hmes_pvar hmes_fml =
    if HMESFormula.is_and hmes_fml then
      ...
      Formula.mk_and (formula_of stmt hmes_pvar hmes_fml1) (formula_of stmt hmes_pvar hmes_fml2) ~info:Dummy
    else if HMESFormula.is_or hmes_fml then
      let hmes_fml1, hmes_fml2 = HMESFormula.let_or hmes_fml in
      Formula.mk_or (formula_of stmt hmes_pvar hmes_fml1) (formula_of stmt hmes_pvar hmes_fml2) ~info:Dummy
  ...
  in
  let hes_preds =
    List.fold_left ~init:[] hmes_preds ~f:(fun hes_preds (fix, pvar, hmes_fml) ->
        List.fold_left ~init:hes_preds stmts ~f:(fun hes_preds stmt ->
            let pvar' = pvar_of stmt pvar in
            let state = state_of stmt in
            let bounds = State.bounds_of state in
            (MuCLP.Pred.make fix pvar' bounds @@ formula_of stmt pvar hmes_fml) :: hes_preds))
    |> List.rev
  in
  let query =
