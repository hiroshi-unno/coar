  ...
  let state =
    Variables.union
      ...
      (
        ...
        |> List.filter
          ~f:(fun (_, rg) -> ReadGraph.rg_get stmt rg |> ReadGraph.length > 0)
        |> List.map
          ~f:(fun (tvar, _) -> Ident.name_of_tvar tvar)
        |> Variables.of_list
      )
    |> State.of_variables
  in
...
let hes_of_chmes (hmes, decls, inits, query_stmt) =
  let query_stmt = LinkedStatement.of_statement query_stmt in
  Debug.print @@ lazy "vvvvvvvvvvvvv Linked C vvvvvvvvvvvvv";
  Debug.print @@ lazy (LinkedStatement.string_of query_stmt);
  Debug.print @@ lazy "";
  let hmes_fv = HMES.get_ftv hmes in
  let _, _, inits, query_stmt = Optimizer.optimize hmes_fv decls inits query_stmt in
  Debug.print @@ lazy "vvvvvvvvvvvvv Optimized Linked C vvvvvvvvvvvvv";
  Debug.print @@ lazy (LinkedStatement.string_of query_stmt);
  Debug.print @@ lazy "";
  ...
  let hes_preds =
    List.fold_left ~init:[] hmes_preds ~f:(fun hes_preds (fix, pvar, hmes_fml) ->
        List.fold_left ~init:hes_preds stmts ~f:(fun hes_preds stmt ->
            ...
            MuCLP.Pred.make fix pvar' bounds fml
            :: hes_preds))
    |> List.rev
  in
  let query =
  ...
  Debug.print @@ lazy "vvvvvvvvvvvvv Converted Hes (neg) vvvvvvvvvvvvv";
  Debug.print @@ lazy (MuCLP.Problem.str_of hes_neg);
  Debug.print @@ lazy "";
