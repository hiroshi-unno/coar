...
let rec sort_of_sexp ~print dtenv = function
  ...
  | Sexp.List [Sexp.Atom "Array"; s1; s2] ->
    T_array.mk_array_sort (sort_of_sexp ~print dtenv s1) (sort_of_sexp ~print dtenv s2)
  | Sexp.List [Sexp.Atom "RegEx"; Sexp.Atom "String"] ->
    T_regex.SRegEx
  | Sexp.List (Sexp.Atom name :: args) as sexp ->
    (match DTEnv.look_up_dt dtenv name with
     | Some dt ->
       T_dt.SDT (Datatype.update_params (Datatype.fresh_of dt) @@
                 List.map args ~f:(sort_of_sexp ~print dtenv))
     | _ -> failwith @@ sprintf "[sort_of_sexp] unknown sort: %s" (Sexp.to_string sexp))
  | Sexp.Atom name ->
    print @@ lazy (sprintf "[sort_of_sexp] %s" name);
    (match DTEnv.look_up_dt dtenv name with
     | Some dt -> Datatype.sort_of @@ Datatype.fresh_of dt
     | _ -> Sort.SVar (Ident.Svar name))
  | sexp -> failwith @@ sprintf "[sort_of_sexp] unknown sort: %s" (Sexp.to_string sexp)

let bind_of_sexp ~print dtenv = function
  | Sexp.List [Sexp.Atom var; sort] -> Ident.Tvar var, sort_of_sexp ~print dtenv sort
  | sexp -> failwith @@ sprintf "[bind_of_sexp] unknown bind: %s" (Sexp.to_string sexp)

let of_params ~print uni_senv sexps dtenv =
  List.rev sexps
  |> List.fold ~init:(uni_senv, Map.Poly.empty) ~f:(fun (uni_senv, acc) -> function
      | Sexp.List [Sexp.Atom name; sort] ->
        Map.Poly.add_exn uni_senv ~key:(Ident.Tvar name) ~data:(sort_of_sexp ~print dtenv sort),
        Map.Poly.add_exn acc ~key:(Ident.Tvar name) ~data:(sort_of_sexp ~print dtenv sort)
...
let is_fun_sym1 = function
  | "-"  | "to_real" | "to_int" | "str.to.re" | "re.comp" | "re.*" | "re.+" | "re.opt" -> true
  | _ -> false
...
let is_fun_sym2 = function
  | "+"  | "-" | "*" | "/" | "div" | "mod" | "rem" | "^" | "re.++" | "re.union" | "re.inter" -> true
  | _ -> false
...
let rec of_formula ~print ~inline (envs:Problem.envs) phi = let open Formula in
  (fun ret ->
     ...
     | None ->
       (match Map.Poly.find envs.exi_senv (Ident.Tvar name) with
        ...
        | None ->
          ...
          | Some ([], T_bool.SBool, Term.FunApp (T_bool.Formula phi, _, _), _, _) ->
            if inline then phi
            else Formula.of_bool_term @@ Term.mk_fvar_app (Ident.Tvar name) [T_bool.SBool] []
  ...
  | Sexp.List [Sexp.Atom "forall"; Sexp.List params; phi] ->
    ...
    of_formula ~print ~inline { envs with uni_senv = uni_senv } phi
  ...
  | Sexp.List [Sexp.Atom "random"; Sexp.List params; phi] ->
    ...
    mk_randoms params @@ of_formula ~print ~inline { envs with uni_senv = uni_senv } phi
  (* let *)
  | Sexp.List [Sexp.Atom "let"; Sexp.List bounds; phi] ->
    ...
    List.rev bounds
    |> List.fold ~init:(of_formula ~print ~inline envs phi) ~f:(fun body (var, sort, def) ->
        Formula.mk_let_formula var sort def body)
  (* ite *)
  | Sexp.List [Sexp.Atom "ite"; cond; then_; else_] ->
    let t =
      T_bool.mk_if_then_else (of_term ~print ~inline envs cond) (of_term ~print ~inline envs then_) (of_term ~print ~inline envs else_)
    in
    if T_bool.is_sbool t then Formula.eq t @@ T_bool.mk_true () else assert false
  ...
  | Sexp.List [Sexp.Atom op; t1; t2] when is_pred_sym2 op ->
    mk_atom @@ of_pred_sym2 op (of_term ~print ~inline envs t1) (of_term ~print ~inline envs t2)
  (* predicate symbol application *)
  | Sexp.List (Sexp.Atom "distinct" :: ts) ->
    and_of @@ Set.to_list @@ Set.fold_distinct_pairs ~init:Set.Poly.empty ~f:(fun acc x y -> Set.add acc @@ Formula.neq x y) @@
    Set.Poly.of_list @@ List.map ts ~f:(of_term ~print ~inline envs)
  (* datatype predicate application *)
  | Sexp.List [Sexp.List [Sexp.Atom "_"; Sexp.Atom "is";
                          (Sexp.List [Sexp.Atom name; _; _] | Sexp.Atom name)]; t] ->
    (match DTEnv.look_up_func envs.dtenv name with
     | Some (dt, Datatype.FCons cons) ->
       mk_atom @@ T_dt.mk_is_cons dt (Datatype.name_of_cons cons) @@
  ...
  | Sexp.List [Sexp.Atom "!"; t; Sexp.Atom ":named"; Sexp.Atom _] ->
    ...
    if Term.is_bool_sort sort then of_formula ~print ~inline envs t else failwith "must be bool fun"
    ...
    (match Map.Poly.find envs.exi_senv (Ident.Tvar name) with
     ...
     | None ->
       ...
       | None ->
         ...
         | Some (fargs, T_bool.SBool, Term.FunApp (T_bool.Formula phi, [], _), false, _) ->
           if inline then begin
             assert (List.length args = List.length fargs);
             let sub = Map.Poly.of_alist_exn @@ List.zip_exn (List.map ~f:fst fargs) args in
             Formula.subst sub phi
           end else
             Formula.of_bool_term @@
             Term.mk_fvar_app (Ident.Tvar name) (List.map ~f:snd fargs @ [T_bool.SBool]) args
         | Some (fargs, T_bool.SBool, _, true, _) ->
           assert (List.length args = List.length fargs);
           Formula.of_bool_term @@
           Term.mk_fvar_app (Tvar name) (List.map ~f:snd fargs @ [T_bool.SBool]) args
         | Some _ -> failwith ""
...
and of_term ~print ~inline envs = let open Term in function
    (* ite *)
    | Sexp.List [Sexp.Atom "ite"; cond; then_; else_] ->
      T_bool.mk_if_then_else (of_term ~print ~inline envs cond) (of_term ~print ~inline envs then_) (of_term ~print ~inline envs else_)
    (* let *)
    | Sexp.List [Sexp.Atom "let"; Sexp.List def; body] ->
      let aux def body =
        ...
        in
        let envs = { envs with uni_senv = uni_senv } in
        List.fold def ~init:(of_term ~print ~inline envs body) ~f:(fun acc (tvar, t) ->
            Term.mk_let_term tvar (Term.sort_of t) t acc)
      in
      (try
    ...
    (* constant variable *)
    | Sexp.Atom name ->
      if String.is_prefix name ~prefix:"\"" && String.is_suffix name ~suffix:"\"" then
        T_string.make @@ String.sub name ~pos:1 ~len:(String.length name - 2)
      else
            ...
            | None ->
              ...
              | None ->
                ...
                | Some ([], ret_sort, t, false, _) ->
                  if inline then t else Term.mk_fvar_app (Ident.Tvar name) [ret_sort] []
    ...
    | Sexp.List [Sexp.Atom op; t1; t2] when is_fun_sym2 op ->
      of_fun_sym2 op (of_term ~print ~inline envs t1) (of_term ~print ~inline envs t2)
    (* function symbol application *)
    | Sexp.List (Sexp.Atom "+" :: arg :: args) ->
      List.fold args ~init:(of_term ~print ~inline envs arg)
        ~f:(fun acc -> of_term ~print ~inline envs >> T_num.mk_nadd acc)
    | Sexp.List (Sexp.Atom "*" :: arg :: args) ->
    ...
    | Sexp.List (Sexp.Atom "re.++" :: arg :: args) ->
      List.fold args ~init:(of_term ~print ~inline envs arg)
        ~f:(fun acc -> of_term ~print ~inline envs >> T_regex.mk_concat acc)
    ...
    | Sexp.List (Sexp.Atom name :: args) as t
      ...
      (match DTEnv.look_up_func envs.dtenv name, List.map args ~f:(of_term ~print ~inline envs) with
    ...
    | Sexp.List (Sexp.Atom name :: args) as t ->
      (match Map.Poly.find envs.fenv (Tvar name) with
       | Some (fargs, ret_sort, term, false, _) ->
         if inline then
           let sub =
             Map.Poly.of_alist_exn @@ List.zip_exn (List.map ~f:fst fargs) @@
             List.map args ~f:(of_term ~print ~inline envs)
           in
           Term.subst sub term
         else
           Term.mk_fvar_app (Ident.Tvar name) (List.map ~f:snd fargs @ [ret_sort]) @@
           List.map args ~f:(of_term ~print ~inline envs)
...
and dist_of_sexp ~print ~inline envs = function
  | Sexp.List [Sexp.Atom "Uniform"; t1; t2] ->
    Rand.mk_uniform (of_term ~print ~inline envs t1) (of_term ~print ~inline envs t2)
  | Sexp.List [Sexp.Atom "Gauss"; t1; t2] ->
    Rand.mk_gauss (of_term ~print ~inline envs t1) (of_term ~print ~inline envs t2)
  | Sexp.List [Sexp.Atom "IntUniform"; t1; t2] ->
    Rand.mk_int_uniform (of_term ~print ~inline envs t1) (of_term ~print ~inline envs t2)
  | _ -> assert false
and of_random_params ~print ~inline envs sexp =
  List.fold (List.rev sexp) ~init:(envs.uni_senv, []) ~f:(fun (uni_senv, params) -> function
      | Sexp.List [Sexp.Atom name; dist] ->
        let rand = dist_of_sexp ~print ~inline envs dist in
        Map.Poly.add_exn uni_senv ~key:(Ident.Tvar name) ~data:(Rand.sort_of rand),
...
let is_available str =
  let logiclist = ["HORN";"SYGUS";"QF_LIA";"QF_NRA";"QF_LIA";"QF_LRA";"QF_LIRA";"QF_NIA";"QF_NRA";"QF_NIRA";"LIA";"LRA";"NIA";"NRA";"SAT";"QF_DT";"QF_UF";"QF_ALIA";"ALL_SUPPORTED";"AUFLIA"] in
...
let mk_dt_sel ~print dtenv dt dts = function
  ...
  | Sexp.List [Sexp.Atom name; Sexp.List (Sexp.Atom ret_name :: args) as ret] ->
    (match List.find dts ~f:(Datatype.name_of_dt >> String.(=) ret_name) with
     | Some _ ->
       Datatype.mk_insel name ret_name @@
       if String.(Datatype.name_of_dt dt = ret_name)
       then Datatype.params_of_dt dt (*ToDo: args?*)
...
let mk_dt_cons ~print dtenv dt dts = function
  ...
  | Sexp.List (Sexp.Atom name :: sels) ->
    Datatype.mk_cons name ~sels:(List.map sels ~f:(mk_dt_sel ~print dtenv dt dts))
  | sexp -> failwith @@ Sexp.to_string sexp
        ...
        | Sexp.List [Sexp.Atom name; Sexp.Atom _] ->
          (match func with
           | Sexp.List (Sexp.List [Sexp.Atom "par"; Sexp.List args] :: _) ->
             Datatype.mk_dt name @@ List.map args ~f:(function
                 | Sexp.Atom name -> Sort.SVar (Ident.Svar name) | _ -> assert false)
           | _ -> assert false)
        | _ -> assert false)
  in
  let datatypes =
    List.map2_exn datatypes funcs ~f:(fun dt -> function
        ...
        | _ -> assert false)
  in
  List.map datatypes ~f:(fun dt -> Datatype.make (Datatype.name_of_dt dt) datatypes flag)
...
let rec toplevel ~print ~inline acc (envs:Problem.envs) = function
  | [] ->
    let acc' =
      List.map acc ~f:(Normalizer.normalize_let ~rename:true
                       >> Formula.elim_let_with_unknowns (Map.key_set envs.exi_senv )
                       >> Normalizer.normalize)
    in
    (*ToDo*)LogicOld.set_fenv @@
    Map.force_merge (Map.Poly.filter envs.fenv ~f:(fun (_, _, _, is_rec, _) -> is_rec)) @@
    LogicOld.get_fenv ();
    acc', envs
  ...
  | Sexp.List (Sexp.Atom "set-info" :: Sexp.Atom name :: t :: _) :: es ->
    (match Map.Poly.find envs.fenv (Tvar name) with
     | Some (fargs, ret_sort, body, is_rec, _(* ToDo *)) ->
       let fenv' = (* ToDo *)
         let phi = Typeinf.typeinf_formula ~print ~to_sus:true @@ of_formula ~print ~inline envs t in
         Map.Poly.set envs.fenv ~key:(Tvar name) ~data:(fargs, ret_sort, body, is_rec, phi)
       in
  ...
  | Sexp.List [Sexp.Atom "declare-sort"; Sexp.Atom name; Sexp.Atom numeral] :: es ->
    let dtenv' =
      DTEnv.update_dt envs.dtenv @@
      Datatype.mk_uninterpreted_datatype ~numeral:(Int.of_string numeral) name
    in
  ...
  | Sexp.List [Sexp.Atom "declare-codatatypes"; Sexp.List dts; Sexp.List funcs] :: es ->
    ...
    print @@ lazy (DTEnv.str_of dtenv');
    toplevel ~print ~inline acc { envs with dtenv = dtenv' } es
  | Sexp.List [Sexp.Atom "declare-fnp"; Sexp.Atom name; Sexp.List args; Sexp.Atom "Bool"] :: es ->
    (*print @@ lazy ("adding " ^ name);*)
    let exi_senv' =
      Map.Poly.add_exn envs.exi_senv ~key:(Ident.Tvar name)
        ~data:(Sort.mk_fun @@ List.map args ~f:(sort_of_sexp ~print envs.dtenv) @ [T_bool.SBool])
    in
    let kind_map' = Map.Poly.add_exn envs.kind_map ~key:(Ident.Tvar name) ~data:Kind.FN in
    toplevel ~print ~inline acc { envs with exi_senv = exi_senv'; kind_map = kind_map' } es
  | Sexp.List [Sexp.Atom "declare-wfp"; Sexp.Atom name; Sexp.List args; Sexp.Atom "Bool"] :: es ->
    (*print @@ lazy ("adding " ^ name);*)
    let exi_senv' =
      Map.Poly.add_exn envs.exi_senv ~key:(Ident.Tvar name)
        ~data:(Sort.mk_fun @@ List.map args ~f:(sort_of_sexp ~print envs.dtenv) @ [T_bool.SBool])
    in
    let kind_map' = Map.Poly.add_exn envs.kind_map ~key:(Ident.Tvar name) ~data:Kind.WF in
    toplevel ~print ~inline acc { envs with exi_senv = exi_senv'; kind_map = kind_map' } es
  | Sexp.List [Sexp.Atom "declare-fun"; Sexp.Atom name; Sexp.List args; Sexp.Atom "Bool"] :: es ->
    (*print @@ lazy ("adding " ^ name);*)
    let exi_senv' =
      Map.Poly.add_exn envs.exi_senv ~key:(Ident.Tvar name)
        ~data:(Sort.mk_fun @@ List.map args ~f:(sort_of_sexp ~print envs.dtenv) @ [T_bool.SBool])
    in
    let kind_map' = Map.Poly.add_exn envs.kind_map ~key:(Ident.Tvar name) ~data:Kind.Ord in
    toplevel ~print ~inline acc { envs with exi_senv = exi_senv'; kind_map = kind_map' } es
  | Sexp.List [Sexp.Atom "declare-fun"; Sexp.Atom name; Sexp.List args; ret] :: es ->
    (*print @@ lazy ("adding " ^ name);*)
    ...
    let exi_senv' = Map.Poly.add_exn envs.exi_senv ~key:(Ident.Tvar name) ~data:fun_sort in
    let kind_map' =
      let kind =
        if Term.is_int_sort ret_sort then Kind.IntFun
        else if Term.is_regex_sort ret_sort && List.is_empty args_sort then Kind.RegEx
        else failwith ("synthesis of a function of the sort " ^
  ...
  | Sexp.List [Sexp.Atom "declare-const"; Sexp.Atom name; ty] :: es ->
    let exi_senv' = Map.Poly.add_exn envs.exi_senv ~key:(Ident.Tvar name) ~data:(sort_of_sexp ~print envs.dtenv ty) in
    let kind_map' = envs.kind_map(*ToDo*) in
    toplevel ~print ~inline acc { envs with exi_senv = exi_senv'; kind_map = kind_map' } es
  | Sexp.List [Sexp.Atom "define-fun"; Sexp.Atom name; Sexp.List fargs; ty; body] :: es ->
    let fenv' =
      ...
      let body =
        ...
        let uni_senv' = Map.force_merge envs.uni_senv (Map.Poly.of_alist_exn fargs') in
        let envs = { envs with uni_senv = uni_senv' } in
        Typeinf.typeinf_term ~print ~to_sus:true @@
        if Term.is_bool_sort sort
        then T_bool.of_formula @@ of_formula ~print ~inline envs body
    ...
    in
    toplevel ~print ~inline acc { envs with fenv = fenv' } es
  | Sexp.List [Sexp.Atom "define-fun-rec"; Sexp.Atom name; Sexp.List fargs; ty; body] :: es ->
    let fenv' =
      let fargs' = List.map fargs ~f:(bind_of_sexp ~print envs.dtenv) in
      let sort = sort_of_sexp ~print envs.dtenv ty in
      let body =
  ...
  | Sexp.List [Sexp.Atom "assert"; phi] :: es ->
    toplevel ~print ~inline (of_formula ~print ~inline envs phi :: acc) envs es
  (** extensions for CHCs *)
  | Sexp.List [Sexp.Atom "declare-var"; Sexp.Atom name; sort] :: es ->
    let uni_senv' =
      Map.Poly.add_exn envs.uni_senv ~key:(Ident.Tvar name) ~data:(sort_of_sexp ~print envs.dtenv sort)
    in
  ...
  | Sexp.List [Sexp.Atom "rule"; phi] :: es ->
    ...
    let bounds =
      List.filter_map (Set.to_list @@ Formula.fvs_of phi)
        ~f:(fun v -> match Map.Poly.find envs.uni_senv v with
            | None -> None | Some s -> Some (v, s))
    in
    toplevel ~print ~inline (Formula.mk_forall bounds phi :: acc) envs es
  | Sexp.List [Sexp.Atom "query"; phi] :: es ->
    let phi = of_formula ~print ~inline envs phi in
    assert (Set.is_empty @@ Formula.fvs_of phi) (*ToDo: check that phi is a predicate variable without arguments*);
    toplevel ~print ~inline (Formula.mk_neg phi :: acc) envs es
  | sexps -> failwith @@ "parse error : " ^ Sexp.to_string_hum @@ Sexp.List sexps

let from_smt2_file ~print ~inline
    ?(uni_senv=Map.Poly.empty) ?(exi_senv=Map.Poly.empty) ?(kind_map=Map.Poly.empty)
    ?(fenv=Map.Poly.empty) ?(dtenv=Map.Poly.empty) filename =
  let phis, _envs =
    filename |> In_channel.create |> Lexing.from_channel |> Parser.program Lexer.token
    |> toplevel ~print ~inline [] { uni_senv = uni_senv; exi_senv = exi_senv;
                                    kind_map = kind_map; fenv = fenv; dtenv = dtenv }
  in
