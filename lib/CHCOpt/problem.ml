...
let str_of_dir_map dir_map =
  ...
  |> sprintf "{%s}"
let str_of_priority priority = String.concat_map_list ~sep:" ⊏ " priority ~f:(name_of_tvar)
      ...
      | (p, _) :: _ ->
        ...
        let indegrees =
          List.fold priority_pairs ~init:indegrees ~f:(fun acc (a, b) ->
              ...
              if Stdlib.(a = p) then Map.Poly.set acc ~key:b ~data:(Map.Poly.find_exn acc b - 1)
              else acc)
        in
        inner indegrees @@ p :: ret
  in
  let indegrees =
    Map.Poly.of_alist_exn @@ List.map ps ~f:(fun p ->
        (* L.debug ~pre:"indegree 0" @@ name_of_tvar p;  *)
        (p, 0))
  in
  let indegrees =
    List.fold priority_pairs ~init:indegrees ~f:(fun acc (_, b) -> (* a --> b*)
        (* L.debug ~pre:"indegree++" @@ name_of_tvar b; *)
...
(* For tvar symbol *)
let direction_tvar d = function
  | Tvar ident -> Tvar (str_of_direction d ^ ident)
...
let direction_of_tvar_exn = function
  | Some d -> d
  | None -> assert false
...
let equal_sol sol1 sol2 =
  ...
  | Sat sol1, Sat sol2 | OptSat sol1, OptSat sol2 | OptSatInSpace sol1, OptSatInSpace sol2 ->
