...
module LinkedStatement : sig
  ...
  let rec get_all_statements_rep stmt res =
    if List.exists (fun stmt' -> stmt' == stmt) res then
      res
    else
      let res = stmt :: res in
      get_next_statements stmt
      |> List.fold_left
        (fun res nxt_stmt -> get_all_statements_rep nxt_stmt res)
        res
  ...
  let rec of_statement_rep nxt_stmt break_nxt_stmt stmt =
    if Statement.is_assign stmt then
      ...
      if break_nxt_stmt == dummy_stmt then
      ...
      of_statement_rep
        (ref (of_statement_rep nxt_stmt break_nxt_stmt stmt2))
        break_nxt_stmt
        stmt1
      ...
      let t_stmt = of_statement_rep nxt_stmt break_nxt_stmt t_stmt in
      let f_stmt = of_statement_rep nxt_stmt break_nxt_stmt f_stmt in
      mk_if cond_fml (ref t_stmt) (ref f_stmt)
    else if Statement.is_loop stmt then
      let first_stmt = ref (mk_exit ()) in
      let body = of_statement_rep first_stmt nxt_stmt (Statement.let_loop stmt) in
      let body =
        if body == !first_stmt then
      ...
      let stmt1 = of_statement_rep nxt_stmt break_nxt_stmt stmt1 in
      let stmt2 = of_statement_rep nxt_stmt break_nxt_stmt stmt2 in
  ...
  let of_statement stmt =
    ...
    of_statement_rep exit_stmt exit_stmt stmt

  let rec get_vars_rep get_vars_one stmt (used_stmts, vars) =
    if List.exists (fun used_stmt -> used_stmt == stmt) used_stmts then
      used_stmts, vars
    else
  ...
  let sub tvar term stmt =
    let subst = [tvar, term] |> Core.Map.Poly.of_alist_exn in
    match stmt with
      IF (cond_fml, t_stmt, f_stmt) ->
