...
type mode =
  | Auto | Prove | Disprove | Parallel | ParallelExc (** with exchange of learned clauses *)
[@@deriving yojson]
...
let instantiate_ext_files cfg =
  Or_error.
    (PCSPSolver.Config.load_ext_file cfg.pcsp_solver_primal >>= fun pcsp_solver_primal ->
     PCSPSolver.Config.load_ext_file cfg.pcsp_solver_dual >>= fun pcsp_solver_dual ->
     ...
     Ok { cfg with pcsp_solver_primal = pcsp_solver_primal;
                   pcsp_solver_dual = pcsp_solver_dual;
                   optimizer = optimizer; qelim = qelim })
...
let load_ext_file = function
  ...
  | Filename filename ->
    ...
    | Error msg ->
      error_string @@ sprintf "Invalid MuVal Configuration (%s): %s" filename msg
