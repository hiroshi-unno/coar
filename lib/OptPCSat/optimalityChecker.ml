...
module type OptimalityCheckerType = sig
  val check : ?only_simple:bool -> Messenger.t option * int option ->
    Ident.tvar -> PCSP.Problem.t -> sort_env_map -> term_subst_map -> CHCOpt.Problem.solution
end
...
let set_id new_id = Atomic.set id new_id; Debug.set_id new_id
...
let mk_tseitin_pred_clause ?(enable=true) i (args, sargs) exi_senv phiss =
  ...
  if size <= 1 then exi_senv, List.concat @@ phiss
    ...
    let phis = List.mapi phiss ~f:(fun i phis->
        ExtTerm.imply_of (ExtTerm.mk_var_app pvars.(i) args) @@ ExtTerm.and_of phis)
    in
    let exi_senv = Array.fold pvars ~init:exi_senv ~f:(fun acc p ->
        Map.Poly.add_exn ~key:p ~data:(Sort.mk_fun @@ sargs @ [ExtTerm.SBool]) acc)
    in
    let tseitin_phi = ExtTerm.or_of @@ List.init size ~f:(fun i ->
        ExtTerm.mk_var_app pvars.(i) (args))
    in
...
let simple_check ~is_max p exi_senv theta =
  ...
  let phi = ExtTerm.beta_reduction sol args in
  let phi = ExtTerm.to_old_formula exi_senv (Map.Poly.of_alist_exn param_senv) phi [] in
  let phi = if is_max then phi else LogicOld.Formula.mk_neg phi in
  try Z3Smt.Z3interface.is_valid ~id:(Atomic.get id) Map.Poly.empty phi with _ -> false
...
let merge_get_leq pure_phi = (** assume [pure_phi] is a conjunction *) let open LogicOld in
...
let apply_pred theta p args =
  ExtTerm.beta_reduction (Map.Poly.find_exn theta p) args

let mk_fvs_fns fvs uni_senv pf params params_sorts =
  List.unzip @@ List.mapi fvs ~f:(fun i v ->
...
let apply_qelim bound exi_senv uni_senv pred_phi pure_phi = let open LogicOld in
  let pred_phi = ExtTerm.to_old_formula exi_senv uni_senv pred_phi [] in
  let pure_phi = ExtTerm.to_old_formula exi_senv uni_senv pure_phi [] in
  dnf_of_pure_formula (Map.Poly.map exi_senv ~f:ExtTerm.to_old_sort) pure_phi
  ...
  |> Set.Poly.map ~f:(fun phi ->
      ...
      in
      Debug.print_log ~tag:"after-qelim:" @@ lazy
        (ExtTerm.str_of_formula exi_senv uni_senv term);
      ExtTerm.neg_of term)
...
let new_senv_of = Map.Poly.map ~f:ExtTerm.of_old_sort
let inters sets =
      ...
      begin match Map.Poly.find models v with
        | Some (Some ret) ->
          if false(*ToDo*) && Stdlib.(`NeedCheck = check_flag) then
            if is_elimable_pair fnsols_senv phi0 (v, ret) then Set.Poly.singleton (v, ret)
            else (Debug.print_log @@ lazy ("eq_pair cannot be used..."); Set.Poly.empty)
          else begin
            Debug.print_log ~tag:("simple geted a eq pair") @@ lazy (sprintf "%s -> %s" (Ident.name_of_tvar v) (Term.str_of ret));
            Set.Poly.singleton (v, ret)
          end
        | Some None ->
          Set.Poly.singleton (v, Term.mk_dummy @@
                              List.Assoc.find_exn ~equal:Stdlib.(=) fnsols_senv v)
        | None -> Set.Poly.empty
      end
    | _ -> Set.Poly.empty

let complex_get_eq_pair v args atms exi_senv fnsols_senv check_flag phi0 = let open LogicOld in
  match check_flag with
  | `Unuseable -> Set.Poly.empty
  | _ ->
    let pure =
        ...
        if not continue then false, []
          ...
          Set.Poly.filter_map ps_with_v ~f:(fun atm ->
              ...
              | Atom.App (Predicate.Psym T_bool.Eq, [t1; t2], _)
                ...
                if is v t1 && useable t2 && is_elimable_pair fnsols_senv check_is_elimable_phi (v, t2) then Some (v, t2)
                else if is v t2 && useable t1 && is_elimable_pair fnsols_senv check_is_elimable_phi (v, t2) then Some (v, t1)
                else None
              | Atom.App (Predicate.Psym T_bool.Eq, [t1; _], _)
                ...
                begin match extract_term_from_affine (is v) t1 with
                  | Some t1' when is_elimable_pair fnsols_senv check_is_elimable_phi (v, t1') ->
                    Some (v, t1')
                  | _ -> None
                end
...
let qe_subst_of v args exi_senv uni_senv fnsols_senv pure_phi phi0 = let open LogicOld in
  let to_old phi = ExtTerm.to_old_formula exi_senv uni_senv phi [] in
  Debug.print_log ~tag:"phi0:" @@ lazy (Logic.ExtTerm.str_of_formula exi_senv uni_senv phi0);
  let phi0', check_flag =
    match check exi_senv uni_senv pure_phi phi0 with
    | Some true -> ExtTerm.mk_bool true, `Useable
    | _ -> phi0, `NeedCheck
  in
  let old_pure_phi = to_old pure_phi in
...
(**
   ...
   change to x1 = x5
*)
let merge_eqs_with v args exi_senv uni_senv pure_phi phi_map = let open LogicOld in
  let to_old phi =
    ExtTerm.to_old_formula exi_senv uni_senv phi []
    |> Evaluator.simplify |> Normalizer.normalize
  in
  let old_phi_map = Map.Poly.map phi_map ~f:(fun (pure, psi) -> to_old pure, psi) in
  let old_pure = to_old pure_phi in
  let dnfs = dnf_of_pure_formula (old_senv_of exi_senv) old_pure in
  let mk_term v = Term.mk_var v (Map.Poly.find_exn uni_senv v |> ExtTerm.to_old_sort) in
  ...
  |> (fun atom_pairs ->
      let atom_map =
        Set.Poly.fold atom_pairs ~init:Map.Poly.empty ~f:(fun acc (atm1, atm2) ->
            ...
            | Some phi ->
              Map.Poly.set acc ~key:atm1 ~data:(Formula.and_of [Formula.mk_atom atm2; phi]))
      in
      let map_atms phi =
        Formula.map_atom phi ~f:(fun atm ->
            match Map.Poly.find atom_map atm with
            | Some phi ->
              Debug.print_log ~tag:"merge eq" @@ lazy
                (sprintf "%s -> %s" (Atom.str_of atm) (Formula.str_of phi));
              phi
            | None -> Formula.mk_atom atm)
      in
      ExtTerm.of_old_formula @@ map_atms old_pure,
      Map.Poly.map old_phi_map ~f:(fun (pure, psi) -> ExtTerm.of_old_formula pure, psi))

let filter_pfapps exi_senv uni_senv pfapps pure_formula (phi_map: (int, term * term) Map.Poly.t) =
  let sol_args =
    let sols =
      List.map pfapps ~f:(fun (_, (_, (_, fnret))) -> fnret |> ExtTerm.let_var |> fst)
    in
    fun p -> List.filter sols ~f:(Stdlib.(<>) p)
  in
  let pfapps =
    List.map pfapps ~f:(fun (((fnpred, ext), ((phi_pf, pfapp), (fnargs, fnret)))) ->
        let pfapp' =
          ExtTerm.of_old_formula @@
          Evaluator.simplify @@
          ExtTerm.to_old_formula exi_senv uni_senv pfapp []
        in
  ...
  let rec inner pfapps pure_formula (phi_map: (int, term * term) Map.Poly.t) subst_map =
    List.fold pfapps ~init:(pure_formula, phi_map, subst_map, [])
      ~f:(fun (pure_phi, phi_map, subst_map, acc)
           (((fnpred, ext), (((phi_pf, pfapp), (fnargs, fnret))))) ->
           Debug.print_log ~tag:"*** try to elim fn:" @@ lazy
             (ExtTerm.str_of_formula exi_senv uni_senv pfapp);
           if ExtTerm.is_false pfapp then begin
             Debug.print @@ lazy "";
             (pure_phi, phi_map, subst_map, ((fnpred, ext), ((phi_pf, pfapp), (fnargs, fnret)))::acc)
           end else
             let v = fst @@ ExtTerm.let_var fnret in
             let args = List.map fnargs ~f:(fun term -> ExtTerm.let_var term |> fst) in
             let args = args @ (if ExtTerm.is_pvar_app exi_senv uni_senv pfapp then sol_args v else []) in
             let pure_phi, phi_map = merge_eqs_with v args exi_senv uni_senv pure_phi phi_map in
             let phi_map =
             ...
             if Map.Poly.is_empty subst then begin
               ...
               let phi_map' = Map.Poly.map phi_map ~f:(fun (phi, psi) -> ExtTerm.subst subst phi, ExtTerm.subst subst psi) in
               let pure_phi' = ExtTerm.subst subst pure_phi in
               Debug.print_log ~tag:"fn elimed" @@ lazy
                 (ExtTerm.str_of_formula exi_senv uni_senv pfapp);
               let acc = ((fnpred, ext), ((phi_pf, pfapp), (fnargs, fnret))) :: acc in
               Debug.print @@ lazy "";
...
let bwd_theta_of ~init exi_senv clauses =
  ...
  |> List.fold ~init ~f:(fun acc (v, term, sort_env, args_y) ->
      ...
      | Some term1 ->
        let term = ExtTerm.beta_reduction term args_y in
        let term1 = ExtTerm.beta_reduction term1 args_y in
        Map.Poly.set acc ~key:v ~data:(ExtTerm.mk_lambda sort_env @@ ExtTerm.or_of [term; term1]))

let pysenv_of pvars =
  Map.Poly.of_alist_exn @@
  List.map pvars ~f:(fun (pv, s) -> pv, mk_fresh_sort_env_list @@ Sort.args_of s)
let idxs_of = List.mapi ~f:(fun i (p, sort) ->
