...
module type OptimizerType = sig
  val optimize : ?filename:(string option) ->
    Ident.tvar list -> PCSP.Problem.t -> CHCOpt.Problem.solution * int
end
module Make
  ...
  let equiv_old_sol_of (p:tvar) (sol:term) =
    ...
    | _ -> Hashtbl.Poly.set old_theta ~key:p ~data:sol; sol
  ...
  let _qelim ~id delta phi = let open LogicOld in
    let clauses =
  ...
  let subst_pcsp ?(bpvs=Set.Poly.empty) subst priority pcsp =
    ...
    (* |> PCSP.Problem.map_if_raw_old ~f:(Set.Poly.map ~f:(fun (env, phi) -> env, qelim ~id:(PCSP.Problem.id_of pcsp) (PCSP.Problem.senv_of pcsp) phi)) *)
  ...
  let check_optimality ?(only_simple=false) (messenger, id) pcsp delta theta priority pvar =
    Debug.print_log @@ lazy
      (sprintf "************************* Optimality Checking Start (%s) **************************\n" @@
       name_of_tvar pvar);
    ...
    in
    let res =
      (if NonOptChecker.is_dup pvar then MaximalityChecker.check else MinimalityChecker.check(*ToDo*))
        ~only_simple (messenger, id) pvar pcsp delta theta
    in
    (match res with
     | CHCOpt.Problem.OptSat _ ->
       Debug.print_log @@ lazy (sprintf "*** Optimality Checking Result: Optimal!\n");
       Debug.print_log @@ lazy
         (sprintf "************************* Optimality Checking End (%s) **************************\n" @@
          name_of_tvar pvar)
     | CHCOpt.Problem.Unknown -> ()(* optimality unknown *)
     | _ -> failwith "maximality and minimality checkers only return OptSat or Unknown");
    res

  let clean () =
    Debug.print_log @@ lazy "clean evaluator cache"; Evaluator.clean ();
    (* Debug.print_log @@ lazy "clean z3"; Z3Smt.Z3interface.clean (); *)
    (* Debug.print_log @@ lazy "gc.compact"; Gc.compact (); *)
    Debug.print_log @@ lazy "gc.full_major"; Gc.full_major ();
  ...
  let improve (messenger, id) improved full_priority priority pcsp delta theta =
    ...
    let c, nepvs = NonOptChecker.improve delta (if config.improve_current then full_priority else priority) theta in
    ...
    let pcsp' =
      extend_pcsp_with (messenger, id) pcsp delta
        (ExtTerm.of_old_formula @@ Evaluator.simplify phi) nepvs
    in
    let pcsp' =
      let maxcs = (*ToDo: necessary?*)
  ...
  let parallel_check_opt improved full_priority priority pcsp delta theta =
    match check_optimality ~only_simple:true (None, Some 0)
            pcsp delta theta priority (List.hd_exn priority) with
    | OptSat _ as sol -> sol
    | _ ->
      ...
      let optimality_check_task =
        Task.async task_pool (fun () ->
            match check_optimality (Some messenger, optimality_check_id)
                    pcsp delta theta priority (List.hd_exn priority) with
            | CHCOpt.Problem.Unknown -> CHCOpt.Problem.Skip, optimality_check_id
      ...
      let improve_task =
        Task.async task_pool (fun () ->
            match improve (Some messenger, improve_task_id)
                    improved full_priority priority pcsp delta theta with
      ...
      let wait_other_task id =
        match
          if Stdlib.(id = improve_task_id)
          then Task.await task_pool optimality_check_task
  ...
  let sequential_check_opt improved full_priority priority pcsp delta theta =
    match check_optimality ~only_simple:true (None, PCSP.Params.new_id ())
            pcsp delta theta priority (List.hd_exn priority) with
    | CHCOpt.Problem.OptSat sol -> CHCOpt.Problem.OptSat sol
    | _ ->
      if !num_improved < improve_until then
        improve (None, Some 0) improved full_priority priority pcsp delta theta
      else
        match check_optimality (None, PCSP.Params.new_id ())
                pcsp delta theta priority (List.hd_exn priority) with
        | OptSat sol -> OptSat sol
        | _ -> improve (None, Some 0) improved full_priority priority pcsp delta theta

  let check_opt = if enable_parallel then parallel_check_opt else sequential_check_opt
  ...
  let add_side_conditions priority pcsp =
    ...
    if not (Config.is_non_trival_mode config.mode || Config.is_non_vacuous_mode config.mode) then pcsp
      ...
      |> PCSP.Problem.of_clauses ~params |> PCSP.Problem.formulas_of
      |> PCSP.Problem.of_formulas ~params

  let optimize ?(filename=None) full_priority pcsp =
    Debug.print_log ~tag:"optimization priority" @@ lazy
      (String.concat_map_list ~sep:"," full_priority ~f:name_of_tvar);
    let src_pcsp_wo_side_conds =
      pcsp
      |> preprocessor full_priority
      |> PCSP.Problem.merge_clauses
      |> preprocessor full_priority
    ...
    let src_pcsp =
      PCSP.Problem.update_params src_pcsp { (PCSP.Problem.params_of src_pcsp) with id = Some 0 }
    in
    let init_sol =
      let sol_opt =
    ...
    | Ok (PCSP.Problem.Sat theta, _) ->
      let src_pcsp = if config.improve_current then src_pcsp_wo_side_conds else src_pcsp in
        ...
        let rec inner max_in_space iter improved priority pcsp theta =
          ...
          | p :: priority' ->
            ...
            | OptSat theta, iter | OptSatInSpace theta, iter ->
              Debug.print_log
                ~tag:(sprintf "Optimization for (%s) is over" @@ name_of_tvar p) @@ lazy
