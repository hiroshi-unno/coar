...
module Make (Config : Config.ConfigType) : OptimalityChecker.OptimalityCheckerType = struct
  ...
  let backward_of pysenv bwd_theta idx_map (nwf:PCSP.Kind.nwf) exi_senv i (uni_senv, ps, ns, phi) =
    ...
    let p_y_subst =
      ...
      List.map ~f:fst @@ Map.Poly.find_exn pysenv p_y
    in
    let p_xs, args_xs =
      Set.Poly.to_list ns
    ...
    let pfs_senv, pfapps =
      ...
      |> List.concat_mapi ~f:(fun j ((idx, p_x), arg_xs, sorts) ->
          List.mapi (List.zip_exn arg_xs sorts) ~f:(fun k (x, s) ->
              ...
              let fn_ret = x in
              let pfapp = None, ExtTerm.mk_var_app pfi @@ fn_args @ [fn_ret] in
              (pfi, pfi_sort), ((pfi, (idx, j)), (pfapp, (fn_args, fn_ret)))))
      |> List.unzip
    in
    let wfs_senv, wfapps =
      List.map (List.zip_exn p_xs args_xs) ~f:(fun (p_x, args_x) ->
          let wf, wfsort, appterm =
            PCSP.Kind.app_nwf_predicate nwf [] (p_y, args_y) (p_x, args_x)
          in
          (wf, wfsort), appterm) |> List.unzip
    in
    let exi_senv =
    ...
    in
    let fvs, pure_phi = OptimalityChecker.apply_qelim_on_pure_formula bound exi_senv uni_senv @@ ExtTerm.neg_of phi in
    Debug.print_log ~tag:"pure_formula" @@ lazy
      (ExtTerm.str_of_formula exi_senv uni_senv pure_phi);
    assert (List.is_empty fvs);
    let sol_apply_map =
      Map.of_list_exn @@ List.mapi (List.zip_exn p_xs args_xs) ~f:(fun j (_, args) ->
          let psi =
            ExtTerm.and_of [
    ...
    let body =
      ...
      ExtTerm.and_of [
        ExtTerm.mk_var_app pdom_0 @@ args_y;
    ...
    in
    let constrs = List.map bodys ~f:(fun body -> ExtTerm.rename p_y_subst @@ ExtTerm.imply_of body head') in
    Debug.print_log ~tag:"backward_constrs:\n" @@ lazy
      (String.concat_map_list ~sep:"\n" constrs ~f:(ExtTerm.str_of_formula exi_senv uni_senv));
    let kind_map =
  ...
  let init_bwd_theta_of exi_senv theta =
    Map.Poly.filter_mapi exi_senv ~f:(fun ~key:p_y ~data:sort ->
        if not @@ Map.Poly.mem theta p_y then None
          ...
          let param_senv =
            List.map sargs ~f:(fun s -> fst @@ ExtTerm.let_var @@ ExtTerm.mk_var (mk_fresh_tvar ()), s)
          in
          let pure_phi = ExtTerm.mk_bool false in
          Option.some @@ ExtTerm.mk_lambda param_senv pure_phi)

  let backward_clauses_of_bwd_theta pvars idx_map exi_senv uni_senv bwd_theta trans_cls =
  ...
  let backward_clauses_of pvars pysenv idx_map nwf theta exi_senv uni_senv defs =
    ...
    in
    let bwd_theta = OptimalityChecker.bwd_theta_of ~init:(init_bwd_theta_of exi_senv theta) exi_senv start_cls in
    let init_clauses =
      backward_clauses_of_bwd_theta pvars idx_map exi_senv uni_senv bwd_theta trans_cls
    in
    let exi_senv, kind_map, uni_senv, phis =
      if List.is_empty defs && List.is_empty init_clauses then
        exi_senv, Map.Poly.empty, uni_senv, []
      else
        List.mapi trans_cls ~f:(backward_of pysenv bwd_theta idx_map nwf exi_senv)
    ...
    let exi_senv, phis =
      ...
      |> List.mapi ~f:(fun i phis ->
          ...
          OptimalityChecker.mk_tseitin_pred_clause (i + 1) (args, sargs) exi_senv phis)
      |> List.unzip
  ...
  let check_min (messenger, id) p pcsp senv theta =
    ...
    Debug.print_log ~tag:"check_min" @@ lazy
    ...
    let nwf = PCSP.Kind.create_nwf (Tvar (sprintf "WF%d" pi)) [] in
    ...
    let goal =
      ...
      ExtTerm.imply_of (ExtTerm.beta_reduction p_sol w) (ExtTerm.mk_var_app dom_p w)
      |> (fun constr ->
          Debug.print_log ~tag:"goal" @@ lazy
            (ExtTerm.str_of_formula exi_senv uni_senv constr);
          constr)
      |> ExtTerm.nnf_of |> ExtTerm.cnf_of exi_senv uni_senv
    ...
    let clauses =
      Set.Poly.union trans_clauses goal
    ...
    in
    let bpvs = Set.Poly.filter ~f:OptimalityChecker.is_ptseitin @@ Map.key_set exi_senv in
    (* Solver.reset (); *)
  ...
  let check ?(only_simple=false) (messenger, id) p pcsp senv theta =
    ...
    if OptimalityChecker.simple_check ~is_max:false p senv theta then CHCOpt.Problem.OptSat theta
