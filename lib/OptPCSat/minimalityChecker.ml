...
module Make (Config : Config.ConfigType) :
  ...
  let init_bwd_theta_of exi_senv theta =
    Map.Poly.filter_mapi exi_senv ~f:(fun ~key:p_y ~data:sort ->
        if not @@ Map.Poly.mem theta p_y then None
          ...
          let param_senv =
            List.map (Sort.args_of sort) ~f:(fun s ->
        ...
        if has_trans_clause p_y || (not @@ Map.Poly.mem bwd_theta p_y) then None
          ...
          let constr =
            let body =
              ...
              ExtTerm.and_of [ ExtTerm.mk_var_app pdom_0 @@ args_y ]
            in
      ...
      let exi_senvs, phiss =
        ...
        @@ List.mapi ~f:(fun i phis ->
            let args, sargs =
              ...
              List.unzip @@ Map.Poly.find_exn pysenv p
            in
  ...
  let check_min (messenger, id) p pcsp senv theta =
    ...
    let pcsp =
      ...
      @@ PCSP.Problem.clauses_of @@ PCSP.Problem.to_cnf
      @@ PCSP.Problem.to_nnf pcsp
    in
    ...
    let goal =
      ...
      @@ ExtTerm.nnf_of constr
    in
    let exi_senv1, kind_map1, trans_clauses =
      backward_clauses_of pvars pysenv idx_map nwf theta exi_senv uni_senv
