...
module Make (Config : Config.ConfigType) :
  ...
  let backward_of p p_has_trans_clause pysenv bwd_theta idx_map (nwf : Kind.nwf)
    ...
    let p_y, args_y =
      let p_y, args_y = Term.let_apps @@ List.hd_exn @@ Set.to_list ps in
      (fst @@ Term.let_var p_y, args_y)
    in
    let p_y_subst =
      Map.Poly.of_alist_exn
      @@ List.zip_exn
           (List.map ~f:(Term.let_var >> fst) args_y)
           (List.map ~f:fst @@ Map.Poly.find_exn pysenv p_y)
    in
    let p_xs, args_xs =
      List.unzip
      @@ List.map ~f:(fun (p, args) -> (fst @@ Term.let_var p, args))
      @@ List.map ~f:Term.let_apps @@ Set.to_list ns
    in
    ...
    let pure_phi, sol_apply_map, pfapps, eqsubst =
      let sol_apply_map =
        Map.Poly.of_alist_exn @@ Set.to_list @@ Set.Poly.of_list
        @@ List.mapi (List.zip_exn p_xs args_xs) ~f:(fun j (p_x, args) ->
            ...
            let phi =
              if Stdlib.(p_x = p) then
                ExtTerm.or_of
                  OptimalityChecker.
                    [ apply_pred theta p_x args; eqs_of w_sorts w args ]
      ...
      let bodys =
        let special_pfapps, pfapps =
          ...
          List.fold pfapps ~init:(Map.Poly.empty, [])
            ~f:(fun
                (acc, pfapps) ((_, (idx, j)), ((phi_pf, pfapp), (_, sol))) ->
        ...
        let special_pfappss =
          ...
          @@ List.map ~f:(fun special_pfapps ->
              ...
              [
                ExtTerm.or_of (List.dedup_and_sort ~compare:Stdlib.compare phis);
    ...
    let kind_map =
      ...
      @@ fst @@ List.unzip pfs_senv
    in
    let uni_senv =
      let tvs = ExtTerm.fvs_of (ExtTerm.and_of constrs) in
      Map.Poly.filter_keys uni_senv ~f:(Set.mem tvs)
    in
        ...
        if has_trans_clause p_y || (not @@ Map.Poly.mem bwd_theta p_y) then None
          ...
          let constr =
            let body =
              ...
              let args_y = List.map param_senv ~f:(fst >> ExtTerm.mk_var) in
              ExtTerm.and_of
                [
                  ExtTerm.neg_of @@ OptimalityChecker.apply_pred theta p w;
                  ExtTerm.mk_var_app pdom_0 @@ w @ args_y;
            ...
            in
            ExtTerm.imply_of body pure_phi
          in
          let uni_senv =
  ...
  let backward_clauses_of p pvars pysenv
    ...
    let exi_senv, kind_map, uni_senv, phis =
      ...
      in
      let bwd_theta =
        let init = init_bwd_theta_of w p exi_senv theta in
        OptimalityChecker.bwd_theta_of ~init exi_senv start_cls
      in
      let init_clauses =
      ...
      if List.is_empty defs && List.is_empty init_clauses then
        ...
        let exi_senvs, kind_maps, phis =
          let p_has_trans_clause =
            List.exists defs ~f:(OptimalityChecker.head_is p)
          in
          List.unzip3 @@ init_clauses
          @ List.mapi trans_cls
              ~f:
                (backward_of p p_has_trans_clause pysenv bwd_theta idx_map nwf
                   theta w winfo exi_senv)
        in
    ...
    let exi_senv, phis =
      let exi_senvs, phiss =
        ...
        @@ List.mapi ~f:(fun i phis ->
            let args, sargs =
              List.unzip @@ Map.Poly.find_exn pysenv (fst @@ List.hd_exn phis)
            in
            OptimalityChecker.mk_tseitin_pred_clause (i + 1)
              (w @ List.map args ~f:ExtTerm.mk_var, wsorts @ sargs)
              exi_senv (List.map phis ~f:snd))
  ...
  let check ?(only_simple = false) (messenger, id) p pcsp senv theta =
    ...
    if OptimalityChecker.simple_check ~is_max:true p senv theta then
      ...
      let nwf : Kind.nwf =
        let (pi, _), sorts_shared, _ = pinfo in
        {
          name = Tvar (sprintf "WF%d" pi);
          sorts_shared;
          sorts_map = Hashtbl.Poly.create ();
          max_pri = (* dummy *) 0;
          sigma = (* dummy *) Map.Poly.empty;
          trunc = (* dummy *) Fn.id;
          acc_set = (* dummy *) Set.Poly.empty;
        }
      in
      List.iter idxs ~f:(fun (key, (_, data, _)) ->
          Hashtbl.Poly.set nwf.sorts_map ~key ~data);
      let kind_map = Kind.kind_map_of_nwf nwf in
      let wsenv = mk_fresh_sort_env_list nwf.sorts_shared in
      let w = List.map wsenv ~f:(fst >> ExtTerm.mk_var) in
      let uni_senv =
        Map.Poly.of_alist_exn
      ...
      let pdom_senv =
        let pdoms =
          ...
          @@ List.map idxs ~f:(fun (p, (_, sorts, pdom)) ->
              ( p,
                ( pdom,
                  Sort.mk_fun @@ nwf.sorts_shared @ sorts @ [ ExtTerm.SBool ] )
              ))
        in
        Map.Poly.of_alist_exn
        @@ List.dedup_and_sort ~compare:Stdlib.compare
        @@ snd @@ List.unzip @@ Map.Poly.to_alist pdoms
      in
      ...
      let exi_senv1, kind_map1, bwd_clauses =
        backward_clauses_of p pvars pysenv idx_map nwf theta w pinfo exi_senv
          uni_senv
        @@ OptimalityChecker.defs_of pcsp
      in
      let exi_senv2, kind_map2, fwd_clauses =
        forward_clauses_of p idx_map theta w pinfo exi_senv uni_senv
        @@ OptimalityChecker.goals_of pcsp
      in
      ...
      let pcsp' =
        ...
        @@ PCSP.Problem.of_clauses ~params clauses
      in
      let bpvs =
        if true then Set.Poly.empty
        else Set.filter ~f:OptimalityChecker.is_ptseitin @@ Map.key_set exi_senv
      in
      (* Solver.reset (); *)
