...
module Make (Config : Config.ConfigType) : OptimalityChecker.OptimalityCheckerType = struct
  ...
  let pcsp_solver_of =
    ...
    fun p -> Hashtbl.Poly.find_or_add solvers p ~default:(fun _ ->
  ...
  let init_bwd_theta_of w p exi_senv theta =
    Map.Poly.filter_mapi exi_senv ~f:(fun ~key:p_y ~data:sort ->
        if not @@ Map.Poly.mem theta p_y then None
          ...
          let args = List.map sargs ~f:(fun _ -> ExtTerm.mk_var (mk_fresh_tvar ())) in
          let param_senv =
            List.map2_exn args sargs ~f:(fun v s -> fst @@ ExtTerm.let_var v, s)
          in
          let pure_phi =
  ...
  let backward_of p p_has_trans_clause pysenv bwd_theta idx_map
    ...
    let idxs, sortss, pdoms = List.unzip3 @@ List.map p_xs ~f:(Map.Poly.find_exn idx_map) in
    ...
    let pfs_senv, pfapps =
      ...
      |> List.concat_mapi ~f:(fun j ((idx, p_x), arg_xs, sorts) ->
          List.mapi (List.zip_exn arg_xs sorts) ~f:(fun k (x, s) ->
              let pfi = OptimalityChecker.pnc (OptimalityChecker.pnt (OptimalityChecker.pn pf j) p_x) k in
              let pfi_sort = Sort.mk_fun @@ w_sorts @ sorts_0 @ [s; ExtTerm.SBool] in
              let fn_args = w @ args_y in
              let fn_ret = x in
              let pfapp =  ExtTerm.mk_var_app pfi @@ fn_args @ [fn_ret] in
              let pfapp =
                if idx = idw && not p_has_trans_clause then
                  let theta_p = OptimalityChecker.apply_pred theta p arg_xs in
                  Some (OptimalityChecker.eqs_of w_sorts w arg_xs), ExtTerm.and_of [theta_p; pfapp]
                else None, pfapp
              in
    ...
    let exi_senv =
      Map.force_merge_list
        ...
        [exi_senv;
         Map.Poly.of_alist_exn @@ List.dedup_and_sort ~compare:Stdlib.compare pfs_senv;
         Map.Poly.of_alist_exn @@ List.dedup_and_sort ~compare:Stdlib.compare wfs_senv]
    in
    let bound =
      Map.Poly.of_alist_exn @@ List.map (args_y @ List.concat args_xs)
        ~f:(fun t -> let v, _ = ExtTerm.let_var t in v, Map.Poly.find_exn uni_senv v)
    in
    let fvs, pure_phi =
      OptimalityChecker.apply_qelim_on_pure_formula bound exi_senv uni_senv @@
      ExtTerm.neg_of phi
    in
    Debug.print_log ~tag:"pure_formula" @@ lazy
      ...
      List.mapi (List.zip_exn p_xs args_xs) ~f:(fun j (p_x, args) ->
          ...
          let phi =
            if Stdlib.(p_x = p) then
              ExtTerm.or_of [OptimalityChecker.apply_pred theta p_x args;
    ...
    let pure_phi, sol_apply_map, pfapps, eqsubst =
      OptimalityChecker.filter_pfapps exi_senv uni_senv pfapps pure_phi sol_apply_map
    in
    let psubst =
      if p_has_trans_clause then Map.Poly.empty
      else Map.Poly.singleton dom_w
          (ExtTerm.mk_lambda (mk_fresh_sort_env_list (w_sorts @ w_sorts)) @@
           ExtTerm.mk_bool true)
    in
    let head =
      ExtTerm.subst psubst @@
      ...
      List.mapi (List.zip_exn p_xs args_xs) ~f:(fun j (_, _) ->
          ...
          match OptimalityChecker.check exi_senv uni_senv pure_phi app_phi with
          | Some true -> ExtTerm.mk_bool true
          | Some false -> dom_phi
          | None -> ExtTerm.or_of [app_phi; dom_phi])
    in
    Debug.print_log ~tag:"head" @@ lazy (ExtTerm.str_of_formula exi_senv uni_senv head);
    ...
    let special_pfappss =
      ...
      |> List.map ~f:(fun special_pfapps ->
          ...
          [ExtTerm.or_of @@ phis; ExtTerm.subst eqsubst @@ ExtTerm.and_of pfapps])
    ...
    let bodys =
      ...
      else List.map special_pfappss ~f:(fun phis -> ExtTerm.and_of @@ body::phis)
    in
    let head =
      ExtTerm.rename p_y_subst @@
      if Map.Poly.mem bwd_theta p_y then
        ExtTerm.or_of [OptimalityChecker.apply_pred bwd_theta p_y args_y; head]
      else head
    in
    let constrs =
    ...
    in
    Debug.print_log ~tag:"backward_constrs:" @@ lazy
  ...
  let forward_of p (idx_map:(tvar, (int * tvar) * Sort.t list * tvar) Map.Poly.t)
    ...
    let pfs_senv, pfapps =
      ...
      |> List.concat_mapi ~f:(fun j ((_, p_x), arg_xs, sorts) ->
          List.mapi (List.zip_exn arg_xs sorts) ~f:(fun k (x, s) ->
              ...
              (pfi, pfi_sort), ((pfi, ()), ((None, ExtTerm.mk_var_app pfi @@ fnargs @ [fnret]), (fnargs, fnret)))))
      |> List.unzip
    in
    let exi_senv =
      Map.force_merge_list
        [exi_senv; Map.Poly.of_alist_exn @@ List.dedup_and_sort ~compare:Stdlib.compare pfs_senv]
    in
    let bound =
      List.map (List.concat arg_xs) ~f:(fun t ->
          let v, _ = ExtTerm.let_var t in v, Map.Poly.find_exn uni_senv v)
      |> Map.Poly.of_alist_exn
    in
    let fvs, pure_phi =
      OptimalityChecker.apply_qelim_on_pure_formula bound exi_senv uni_senv @@
    ...
    assert (List.is_empty fvs);
    let sol_apply_map =
      List.mapi (List.zip_exn p_xs arg_xs)
        ~f:(fun j (p_x, args) ->
            let phi =
              if Stdlib.(p_x = p) then
                ExtTerm.or_of [OptimalityChecker.apply_pred theta p_x args;
  ...
  let backward_clauses_of_bwd_theta
      p pvars idx_map w exi_senv uni_senv bwd_theta theta trans_cls =
    let has_trans_clause p = List.exists trans_cls ~f:(OptimalityChecker.head_is p) in
    List.filter_map pvars ~f:(fun (p_y, _) ->
        if has_trans_clause p_y || not @@ Map.Poly.mem bwd_theta p_y then None
          ...
          let args_y = List.map param_senv ~f:(fun (v, _) -> ExtTerm.mk_var v) in
          let body =
            ExtTerm.and_of [
              ExtTerm.neg_of @@ OptimalityChecker.apply_pred theta p w;
              ExtTerm.mk_var_app pdom_0 @@ w @ args_y]
          ...
          Debug.print_log ~tag:"backward_clauses_of_bwd_theta" @@ lazy
            (ExtTerm.str_of_formula exi_senv uni_senv constr);
          Some (exi_senv, Map.Poly.empty, (uni_senv, (p_y, [constr]))))

  let backward_clauses_of p pvars pysenv
      (idx_map:(tvar, (int * tvar) * Sort.t list * tvar) Map.Poly.t)
      nwf theta w winfo exi_senv uni_senv defs =
    ...
    let init_clauses =
      backward_clauses_of_bwd_theta p pvars idx_map w exi_senv uni_senv bwd_theta theta trans_cls
    in
    let exi_senv, kind_map, uni_senv, phis =
      if List.is_empty defs && List.is_empty init_clauses then
        ...
        List.mapi trans_cls
          ~f:(backward_of p p_has_trans_clause pysenv bwd_theta idx_map nwf theta w winfo exi_senv)
        |> List.append init_clauses
        |> List.unzip3
        |> (fun (exi_senvs, kind_maps, phis) ->
            ...
            List.fold uni_senvs ~init:uni_senv ~f:(Map.force_merge ~catch_dup:(fun (key, data1, data2) ->
                print_endline @@ sprintf "%s : %s != %s" (name_of_tvar key) (ExtTerm.str_of_sort data1) (ExtTerm.str_of_sort data2))),
            phis)
    in
    let exi_senv, phis =
      ...
      |> List.mapi ~f:(fun i phis ->
          ...
          OptimalityChecker.mk_tseitin_pred_clause (i + 1) ((w @ args), (wsorts @ sargs)) exi_senv phis)
      |> List.unzip
      |> (fun (exi_senvs, phiss) -> Map.force_merge_list exi_senvs, List.concat phiss)
    in
    Debug.print_log ~tag:"backward constrs" @@ lazy
      (sprintf "\n*** %s\n" @@
       String.concat_map_list ~sep:"\n*** " phis ~f:(ExtTerm.str_of_formula exi_senv uni_senv));
    exi_senv, kind_map,
    OptimalityChecker.to_cnf_clauses exi_senv uni_senv @@ ExtTerm.and_of @@ phis


  let forward_clauses_of p
      (idx_map:(tvar, (int * tvar) * Sort.t list * tvar) Map.Poly.t)
      theta w winfo exi_senv uni_senv goals =
    let _, wsorts, _ = winfo in
    let exi_senv, kind_map, uni_senv, phis =
      if List.is_empty goals then
        exi_senv, Map.Poly.empty, uni_senv, [[OptimalityChecker.apply_pred theta p w]]
      else
    ...
    in
    let exi_senv, phis = OptimalityChecker.mk_tseitin_pred_clause 0 (w, wsorts) exi_senv @@ phis in
    Debug.print_log ~tag:"forward constrs" @@ lazy
      (sprintf "\n*** %s\n" @@
       String.concat_map_list ~sep:"\n*** " phis ~f:(ExtTerm.str_of_formula exi_senv uni_senv));
    let phi = ExtTerm.and_of phis in
    (* let body = ExtTerm.neg_of @@ apply_pred theta p w in
       let phi = ExtTerm.imply_of body head in *)
    Debug.print_log ~tag:"forward_formula:" @@ lazy
      (ExtTerm.str_of_formula exi_senv uni_senv phi);
  ...
  let check ?(only_simple=false) (messenger, id) p pcsp senv theta =
    ...
    OptimalityChecker.set_id id; Debug.set_id id;
    if OptimalityChecker.simple_check ~is_max:true p senv theta then CHCOpt.Problem.OptSat theta
    else if only_simple then CHCOpt.Problem.Unknown
    else
      let pvars = OptimalityChecker.pvars_of p senv in
      Debug.print_log ~tag:"pvar id" @@ lazy
        (sprintf "%s" @@
         String.concat_mapi_list ~sep:"," pvars ~f:(fun i (var, _) -> sprintf "%d:%s" i @@ name_of_tvar var));
      ...
      let uni_senv =
        Map.Poly.fold pysenv ~init:wsenv ~f:(fun ~key:_ ~data:pys acc -> pys @ acc)
        |> Map.Poly.of_alist_exn
      in
      let pdoms =
      ...
      let pcsp =
        PCSP.Problem.clauses_of @@ PCSP.Problem.merge_clauses @@ PCSP.Problem.to_cnf @@ PCSP.Problem.to_nnf pcsp
        |> Set.Poly.map ~f:(fun (params, ps, ns, phi) -> Map.force_merge uni_senv params, ps, ns, phi)
      ...
      let exi_senv1, kind_map1, bwd_clauses =
        backward_clauses_of p pvars pysenv idx_map nwf theta w pinfo exi_senv uni_senv defs
      in
...
let make config =
  (module Make (struct let config = config end) : OptimalityChecker.OptimalityCheckerType)
