...
let param_senv_of phi =
  LogicOld.Formula.term_sort_env_of phi |> Set.to_list
  |> List.map ~f:(fun (v, s) -> v, ExtTerm.of_old_sort s)
  |> Map.Poly.of_alist_exn

let rec replace_tuple_var_to_cons_term tuple_var_map = let open LogicOld in
        ...
        begin
          ...
          | _ ->
            let tuple =
              ...
              List.map sorts ~f:(Term.mk_fresh_var >> replace_tuple_var_to_cons_term tuple_var_map)
            in
      ...
      if Fn.non Atom.is_pvar_app atm then
        ...
        if Set.mem fnpvs (Ident.pvar_to_tvar pvar) then
          ...
          Formula.and_of @@ List.foldi tsols ~init:[] ~f:(fun i acc t ->
              ...
              Hashtbl.Poly.set exi_senv' ~key:pf ~data:(ExtTerm.of_old_sort @@ Sort.mk_fun sorts);
              (Formula.mk_atom @@ Atom.mk_pvar_app (tvar_to_pvar @@ pf) sorts (tparams @ [t])) :: acc)
        else
          let ts' =
            List.fold ts ~init:[] ~f:(fun acc t -> acc @ extract_tuple tuple_var_map t)
          in
