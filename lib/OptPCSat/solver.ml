...
module type SolverType = sig
  ...
  val solve : ?filename:(string option) -> ?print_sol:bool ->
    CHCOpt.Problem.t -> CHCOpt.Problem.solution * int
end
module Make (Config : Config.ConfigType) : SolverType = struct
  ...
  let solve ?(filename=None) ?(print_sol=false) problem =
    ...
    if print_sol then
      print_endline @@ CHCOpt.Problem.str_of_solution (PCSP.Problem.senv_of pcsp) iter sol;
    sol, iter
end
