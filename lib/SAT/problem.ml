...
let minimize_core_cnf cnf solution =
  ...
  let vars = List.map ~f:fst solution |> List.dedup_and_sort ~compare:Stdlib.compare in
  let pos_edge_of v =
    List.filter ~f:(fun (_, pos) -> List.mem pos v ~equal:Stdlib.(=)) clauses in
  let neg_edge_of v =
    List.filter ~f:(fun (neg, _) -> List.mem neg v ~equal:Stdlib.(=)) clauses in
  let edges = List.map ~f:(fun v -> (v, (neg_edge_of v, pos_edge_of v))) vars in
  let assignments =
    List.sort ~compare:(fun a b ->
        Stdlib.compare (assess_of edges b) (assess_of edges a)) solution in
