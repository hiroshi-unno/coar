...
let rec parse clauses in_channel =
  try
    ...
    | line -> (
        ...
        |> function
        | [] | "c" :: _ | "p" :: _ -> parse clauses in_channel
        | "%" :: _ ->
            let rec exhaust () =
              try
        ...
        | literals ->
            let literals =
              ...
              | "0" :: literals' -> literals'
              | _ -> failwith "clause must end with 0"
            in
            let clause =
              List.partition_tf literals ~f:(fun literal ->
...
let from_dimacs_file file =
  file |> In_channel.create ~binary:false |> parse [] |> Problem.of_list
