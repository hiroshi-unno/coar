...
let rec neg_exprs_of_strings ctx = function
  | [] -> []
  | s::tl -> (Z3.Boolean.mk_not ctx @@ Z3.Boolean.mk_const_s ctx s) :: neg_exprs_of_strings ctx tl
let of_clause ctx (negatives, positives) =
  Z3.Boolean.mk_or ctx @@
  (pos_exprs_of_strings ctx positives @ neg_exprs_of_strings ctx negatives)
...
let opt_solve (soft : SAT.Problem.soft) hard =
  ...
  List.iter soft ~f:(fun (expr, weight) ->
      ignore @@ Z3.Optimize.add_soft optimizer (Z3.Boolean.mk_and ctx @@ of_cnf ctx expr) (string_of_int weight) (Z3.Symbol.mk_string ctx "hoge"));
  match Z3.Optimize.check optimizer with
  | SATISFIABLE ->
    let open Option.Monad_infix in
    Z3.Optimize.get_model optimizer >>= fun model ->
    let score = List.fold soft ~init:0 ~f:(fun score (expr, weight) ->
        score + (match Z3.Model.eval model (Z3.Boolean.mk_and ctx @@ of_cnf ctx expr) true with
            | None -> 0 | Some e -> if Z3.Boolean.is_true e then weight else 0)) in
...
let check_sat phi =
  ...
  | Z3.Solver.SATISFIABLE ->
    ...
    Some (List.map decls ~f:(fun decl ->
        (PropLogic.Formula.mk_atom (Z3.Symbol.get_string @@ Z3.FuncDecl.get_name decl),
    ...
    | Some model ->
      ...
      let negation = PropLogic.Formula.or_of @@
        List.map solution ~f:(fun (variable, boolean) ->
            ...
            else PropLogic.Formula.mk_atom variable) in
