...
module Make (Verbose : Debug.Config.ConfigType) = struct
  ...
  type clause =
    Atom.t Set.Poly.t * Atom.t Set.Poly.t * Formula.t *
  ...
  let str_of_erasure (e: erasure) =
    String.concat ~sep:"\n" @@ List.filter_map (Map.Poly.to_alist e) ~f:(fun (pvar, is) ->
        ...
        else Option.return @@ sprintf "{%s}" @@ String.concat_map_set ~sep:", " is
            ~f:(uncurry @@ sprintf "(%s,%d(%d))" (Ident.name_of_pvar pvar)))
  let str_of_times times =
    sprintf "{%s}" @@ String.concat_map_list ~sep:";  " ~f:(fun (tvar, time) ->
        sprintf "%s: %d" (Ident.name_of_tvar tvar) time) @@ Hashtbl.Poly.to_alist times

  let is_empty = Map.for_all ~f:Set.is_empty
  let size_of = Map.Poly.fold ~init:0 ~f:(fun ~key:_ ~data -> (+) (Set.length data))
  ...
  let occur_times_in_atoms =
    ...
    fun (x :Ident.tvar) (atoms : Atom.t Set.Poly.t) ->
      let ret = Set.fold atoms ~init:0 ~f:(fun ret atom ->
  ...
  let src_id_of id (param_log: bool array) =
    ...
    let rec inner step i =
      ...
      assert (i < len);
      if Array.get param_log i
      then if step = id then i else inner (step + 1) (i + 1)
      else inner step (i + 1)
    in
    inner 0 0
  ...
  let find_unsafe_raf clauses (e: erasure) (pvar, (is: (int * int) Set.Poly.t)) =
    ...
    Set.find_map clauses ~f:(fun (ns, ps, _, times) ->
        Set.filter is ~f:(fst >> find_tks ns pvar >> Set.exists ~f:(check_tk ps times))
  ...
  let find_unsafe_far clauses (e: erasure) (pvar, (is: (int * int) Set.Poly.t)) =
    ...
    Set.find_map clauses ~f:(fun (ns, ps, phi, _times) ->
        Set.filter is ~f:(fst >> find_tks ps pvar >> Set.exists ~f:(check_tk ps ns phi e))
        |> (fun uis -> if Set.is_empty uis then None else Some uis))
  ...
  let full_erasure_of_atom param_logs direct bpvs fnpvs = function
    | Atom.App (Predicate.Var (pvar1, sorts), _, _) ->
      if is_special_pvar bpvs pvar1 || is_far direct && is_special_pvar fnpvs pvar1
      then None else
        let bounds =
          if is_special_pvar fnpvs pvar1 then List.length sorts - 1 else List.length sorts
        in
        ...
        Some (pvar1, List.foldi sorts ~init:Set.Poly.empty ~f:(fun i ret _ ->
            if i < bounds then Set.add ret (i, src_id_of i param_log) else ret))
    | _ -> None
  ...
  let raf param_logs bpvs fnpvs clauses : erasure =
    ...
    let rec inner (e: erasure) =
      ...
      match List.filter_map (Map.Poly.to_alist e) ~f:(find_unsafe_raf clauses e) with
      | [] -> e
  ...
  let far param_logs bpvs fnpvs clauses : erasure =
    ...
    let rec inner (e: erasure) =
      ...
      match List.filter_map (Map.Poly.to_alist e) ~f:(find_unsafe_far clauses e) with
      | [] -> e
      | a -> inner (List.fold_left a ~init:e ~f:(fun e (pvar, uis, is) ->
          Map.Poly.set e ~key:pvar ~data:(Set.diff is uis)))
    in
  ...
  let rec elim_args param_logs bpvs fnpvs senv (clauses : clause Set.Poly.t)  =
    ...
    Debug.print @@ lazy (sprintf "[elim arg] param_logs after far:\n%s" (str_of_param_logs param_logs senv));
  ...
  let elim_args param_logs bpvs fnpvs senv clauses =
    ...
    Set.Poly.map clauses ~f:(fun cl ->
        ...
        Debug.print @@ lazy ("[#occurrences of each unknown in the clause] " ^ str_of_times times);
        cl')
    |> elim_args param_logs bpvs fnpvs senv
    |> (fun (senv, cls) ->
        Debug.print @@ lazy "*** args eliminated clauses:";
        senv, Set.Poly.map cls ~f:(fun ((ns, ps, phi, _) as cl) ->
            Debug.print @@ lazy (str_of_clause cl);
            ps, ns, phi))
    |> (fun res ->
        Debug.print @@ lazy "**************** elim args end   ****************"; res)
  ...
  let elim_pcsp_args ?(bpvs=Set.Poly.empty) param_logs pcsp =
    if not enable then pcsp else
      ...
      let bpvs =
        ...
        |> Set.filter ~f:(fun t ->
            Set.mem bpvs t ||
            PCSP.Problem.is_wf_pred pcsp t || PCSP.Problem.is_nwf_pred pcsp t ||
      ...
      let pcsp =
        PCSP.Problem.map_if_raw_old pcsp ~f:(fun inp ->
            ...
            Set.concat_map phis
              ~f:(Formula.cnf_of(*ToDo*)
                    Logic.(to_old_sort_env_map ExtTerm.to_old_sort @@
                           PCSP.Problem.senv_of pcsp))
            |> elim_args param_logs bpvs fnpvs !penv
            |> (fun (penv', cls) -> penv := penv'; cls)
