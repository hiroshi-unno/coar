...
let term_map examples = (* size of examples is more than 0 *)
  let open Graph in
  let open Pack.Digraph in
  let cnt = ref 0 in
  let size =
      ...
      | (_, _), terms ->
        ...
        let set term map =
          Map.Poly.update map term ~f:(function
              | None -> incr cnt; (V.create (!cnt))
              | Some n -> n)
        in
        set t2 @@ set t1 map
    ) ~init:Map.Poly.empty examples in
  let rmap = Map.Poly.fold ~f:(fun ~key:k ~data:v acc -> Map.Poly.add_exn acc ~key:v ~data:k) ~init:Map.Poly.empty map in
  map, rmap, !cnt, size
        ...
        |> List.fold ~f:(fun acc cycle ->
            let rec get_papps acc = function
              | v1 :: v2 :: tl ->
                ...
                let papp = PCSatCommon.ExAtom.PApp ((pvar, sorts), t1@t2) in
                (* Debug.print @@ lazy (str_of_example papp ^ ","); *)
                let acc' = papp::acc in
                get_papps acc' (v2::tl)
              | _ -> acc
            in
