...
module type SolverType = sig
  val solve : ?bpvs:(Ast.Ident.tvar Set.Poly.t) -> ?filename:(string option) -> ?print_sol:bool ->
    PCSP.Problem.t -> State.output Or_error.t
  val preprocess : ?bpvs:Ast.Ident.tvar Set.Poly.t ->
    PCSP.Problem.t -> PCSP.Problem.t Or_error.t
  val reset : unit -> unit
  val push : unit -> unit
  val pop : unit -> unit
end
...
module Make (Config: Config.ConfigType): SolverType = struct
  ...
  let solve ?(bpvs=Set.Poly.empty) ?(filename=None) ?(print_sol=false) pcsp =
    ...
    pvar_eliminator >>= fun (module PvarEliminator : PvarEliminator.PvarEliminatorType) ->
    ...
    in
    List.map pcsps ~f:(PvarEliminator.solve ~bpvs ~oracle Coordinator.solve)
    |> Or_error.combine_errors >>= (fun res ->
  ...
  let preprocess ?(bpvs=Set.Poly.empty) pcsp = let open Or_error in
    pvar_eliminator >>= fun (module PvarEliminator : PvarEliminator.PvarEliminatorType) ->
    Or_error.return (snd @@ PvarEliminator.preprocess ~bpvs pcsp)
