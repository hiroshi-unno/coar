...
module Config = struct
  ...
  let rec load_ext_phase =
    ...
    | P (phase1, phase2) ->
      load_ext_phase phase1 >>= fun phase1 -> load_ext_phase phase2 >>= fun phase2 ->
      Ok (P (phase1, phase2))

  let instantiate_ext_files cfg =
    let open Or_error in
    List.map ~f:load_ext_phase cfg.phase_list |> Result.all >>= fun phase_list ->
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid Coordinator Configuration (%s): %s" filename msg
...
module Make(Cfg: Config.ConfigType) (MContext : Context.ContextType) : CoordinatorType = struct
  ...
  let build_phase pcsp rl_config phase =
    ...
    let open Or_error in
    match phase with
    | V cfg -> ExtFile.unwrap cfg >>= fun cfg -> Ok (SU (Validator.make rl_config cfg pcsp))
    | R cfg -> ExtFile.unwrap cfg >>= fun cfg -> Ok (UU (Resolver.make rl_config cfg pcsp))
    | S cfg -> ExtFile.unwrap cfg >>= fun cfg -> Ok (US (Synthesizer.make rl_config cfg pcsp))
    | L cfg -> ExtFile.unwrap cfg >>= fun cfg -> Ok (UU (Labeler.make rl_config cfg pcsp))
  ...
  let print_status iter n =
    let header =
      "************** current phase of " ^
      Ordinal.string_of (Ordinal.make iter) ^ " iteration **************"
    in
  ...
  let update_context_version () =
    ...
    | Some vs ->
      ...
      | None -> assert false
      | Some version -> Context.set_version MContext.context @@ Context.version_of version vs
  ...
  let solve ?(oracle=None) pcsp =
    if Map.Poly.is_empty @@ PCSP.Problem.senv_of pcsp then
      ...
      config.phase_list |> List.map ~f:(build_phase pcsp rl_config)
      |> Result.all >>= (fun phase_list ->
          if rl_config.enable then begin
            RLConfig.lock ();
            if rl_config.show_constraints then
              Debug.print_stdout @@ lazy
                (sprintf "constraints: %s" @@
                 Yojson.Safe.to_string @@ PCSP.Problem.to_yojson pcsp);
            if rl_config.show_num_constrs then
              Debug.print_stdout @@ lazy
                (sprintf "#constrs: %d" @@ PCSP.Problem.num_constrs pcsp);
            if rl_config.show_num_pvars then
              Debug.print_stdout @@ lazy
                (sprintf "#pvars: %d" @@ PCSP.Problem.num_pvars pcsp);
            if rl_config.show_dep_graph then
              Debug.print_stdout @@ lazy
                (sprintf "dependency graph:\n%s\n" @@
                 PCSP.Problem.str_of_graph @@ PCSP.Problem.dep_graph_of_chc pcsp);
            RLConfig.unlock ()
          end;
          main_loop messenger (fun iter e ->
