...
module Config = struct
  ...
  let instantiate_ext_files = function
    | DT cfg ->
      ...
      DT_Regressor.Config.instantiate_ext_files cfg >>= fun cfg ->
      Ok (DT cfg)
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid Regressor Configuration (%s): %s" filename msg
end
...
module Make (Cfg: Config.ConfigType) =
  (val (match Cfg.config with
       | DT cfg ->
         (module DT_Regressor.Make (struct let config = cfg end): RegressorType)))