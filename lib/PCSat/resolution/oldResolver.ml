...
sig
  ...
  let is_ground_atom exi_senv papp =
    Set.Poly.is_empty @@ Set.Poly.diff (Logic.Term.fvs_of papp) (Map.key_set exi_senv)
  ...
  let classify_clauses exi_senv =
    Set.Poly.fold ~init:(Set.Poly.empty, Set.Poly.empty, Set.Poly.empty)
  ...
  let gen_uclause_c1 exi_senv _undecided c1 ((pos: ExClauseSet.t), (neg: ExClauseSet.t))
    ...
    in
    Set.Poly.fold c1 ~init:(pos, neg) ~f:(fun (pos, neg) (_c_senv, c_pos, c_neg, c_pure) ->
        let with_tvar_neg = Set.Poly.filter ~f:(Fn.non @@ is_ground_atom exi_senv) c_neg in
        let with_tvar_pos = Set.Poly.filter ~f:(Fn.non @@ is_ground_atom exi_senv) c_pos in
  ...
  let gen_uclause_c0x1 exi_senv undecided c0x1 ((pos: ExClauseSet.t), (neg: ExClauseSet.t))
    ...
    let undecided_papps =
      Set.Poly.fold ~init:Set.Poly.empty undecided
    ...
    in
    Set.Poly.fold c0x1 ~init:(Some (pos, neg)) ~f:(fun acc (_c_senv, c_pos, c_neg, c_pure) ->
        let open Option in
        match Set.Poly.find ~f:(Fn.non @@ is_ground_atom exi_senv) c_pos,
              Set.Poly.find ~f:(Fn.non @@ is_ground_atom exi_senv) c_neg with
  ...
  let rec resolution_loop (dpos, dneg, undecided) exi_senv cs =
    ...
    if Set.Poly.is_empty cs || ClauseSet.has_only_pure cs then Some (dpos, dneg) else
      cs
  ...
  let resolve e = let open State.Monad_infix in
    Ok e >>=? (fun vs _a ->
        ...
        let d_diff = Set.Poly.diff dpos !old_dpos, Set.Poly.diff dneg !old_dneg in
        old_dpos := dpos;
        old_dneg := dneg;
        ...
        |> Option.value_map ~default:State.Unsat ~f:(fun (dpos', dneg') ->
            ...
            let new_examples =
              ...
              |> (fun exs -> Set.Poly.diff exs (VersionSpace.examples_of vs))
