  ...
  let load_ext_file = function
    | ExtFile.Filename filename ->
      begin
        ...
        | Error msg ->
          error_string @@ sprintf "Invalid DefaultResolver Configuration (%s): %s" filename msg
      end
...
sig
    ...
    Set.fold ~init:(Set.Poly.empty, Set.Poly.empty, Set.Poly.empty)
      ~f:(fun (c0, c1, c0x1) ((_, c_pos, c_neg, _) as c) ->
          let num_of_ground_atoms =
            ...
            + (Set.length @@ Set.filter ~f:(is_ground_atom exi_senv) c_neg) in
          let num_of_non_ground_atoms =
            (Set.length c_pos + Set.length c_neg) - num_of_ground_atoms in
          if num_of_ground_atoms >= 2 then
            (c0, c1, c0x1)
          else if num_of_ground_atoms = 1 then
            (c0, Set.add c1 c, c0x1)
          else if num_of_non_ground_atoms = 1 then
            (c0, c1, Set.add c0x1 c)
          else if num_of_non_ground_atoms = 0 then
            (c0, c1, c0x1)
          else
            (Set.add c0 c, c1, c0x1))

  let gen_uclause_c1 exi_senv _undecided c1 ((pos: ExClauseSet.t), (neg: ExClauseSet.t))
      ...
      let rec inner ex pure withvars =
        ...
        | e::ex, w::withvars ->
          ...
          | Some (_param_senv(*ToDo*), e) ->
            match unify (Set.Poly.empty (*ToDo: is it OK with function varibles?*)) (Logic.ExtTerm.to_old_atom exi_senv (*ToDo*)Map.Poly.empty w []) e with
            ...
            | None -> inner ex pure (w::withvars)
      in inner (Set.to_list ex) pure (Set.to_list withvars)
    in
    Set.fold c1 ~init:(pos, neg) ~f:(fun (pos, neg) (_c_senv, c_pos, c_neg, c_pure) ->
        let with_tvar_neg = Set.filter ~f:(Fn.non @@ is_ground_atom exi_senv) c_neg in
        let with_tvar_pos = Set.filter ~f:(Fn.non @@ is_ground_atom exi_senv) c_pos in
        ...
        | None -> (* c1 will be a negative unit clause *)
          ...
          then (pos, Set.add neg {positive=Set.Poly.empty;
                                  negative=Set.Poly.singleton @@ ExAtom.of_ground_atom (LogicOld.Formula.mk_true ()) exi_senv candidate})
          else (pos, neg)
        | Some candidate -> (* c1 will be a positive unit clause *)
          if is_eligible pos c_pure with_tvar_neg
          && is_eligible neg c_pure with_tvar_pos
          then (Set.add pos {positive=Set.Poly.singleton @@ ExAtom.of_ground_atom (LogicOld.Formula.mk_true ()) exi_senv candidate;
  ...
  let gen_uclause_c0x1 exi_senv undecided c0x1 ((pos: ExClauseSet.t), (neg: ExClauseSet.t))
    ...
    let undecided_papps =
      Set.fold ~init:Set.Poly.empty undecided
        ~f:(fun acc ex -> Set.Poly.union_list [acc; ex.ExClause.positive; ex.ExClause.negative])
    in
    let is_eligible papp pure undecided_app =
      ...
      | Some (_param_senv(*ToDo*), papp') ->
        match unify (Set.Poly.empty (*ToDo: is it OK with function varibles?*))
                ...
                (Logic.ExtTerm.to_old_atom exi_senv (*ToDo*)Map.Poly.empty papp' []) with
        | None -> false
        | Some theta ->
          let theta = Map.Poly.map theta ~f:Logic.ExtTerm.of_old_term in
          refuted @@ Logic.Term.subst theta pure
    in
    Set.fold c0x1 ~init:(Some (pos, neg)) ~f:(fun acc (_c_senv, c_pos, c_neg, c_pure) ->
        let open Option in
        match Set.find ~f:(Fn.non @@ is_ground_atom exi_senv) c_pos,
              Set.find ~f:(Fn.non @@ is_ground_atom exi_senv) c_neg with
        ...
        | Some papp , None -> (* papp will be positive unit clause *)
          Set.fold undecided_papps ~init:acc ~f:(fun acc undecided ->
              acc >>= fun (pos, neg) ->
              if Set.exists ~f:(is_eligible papp c_pure) @@ ExClauseSet.exatoms_of neg then None (* unsat *)
        ...
        | None, Some papp -> (* papp will be negative unit clause *)
          Set.fold undecided_papps ~init:acc ~f:(fun acc undecided ->
              acc >>= fun (pos, neg) ->
              if Set.exists ~f:(is_eligible papp c_pure) @@ ExClauseSet.exatoms_of pos then None
              else if is_eligible papp c_pure undecided then
  ...
  let rec resolution_loop (dpos, dneg, undecided) exi_senv cs =
    ...
    if Set.is_empty cs || ClauseSet.has_only_pure cs then Some (dpos, dneg) else
      cs
  ...
  let resolve e = let open State.Monad_infix in
    Ok e >>=? (fun vs _a ->
        ...
        |> Option.value_map ~default:State.Unsat ~f:(fun (dpos', dneg') ->
            ...
            let new_examples =
              ...
              |> Set.Poly.map ~f:(fun ex -> ex, [ClauseGraph.Dummy, false]) in (*TODO: add sources for new_undecided *)
