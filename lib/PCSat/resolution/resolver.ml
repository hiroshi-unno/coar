...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid Resolver Configuration (%s): %s" filename msg
...
module Make (RLCfg: RLConfig.ConfigType) (Config: Config.ConfigType) (APCSP: PCSP.Problem.ProblemType)
  : ResolverType = struct
  ...
  module Resolver =
    (val (match config with
         ...
         | Old cfg -> (module OldResolver.Make (struct let config = cfg end) (APCSP))
         | SMT cfg -> (module GraphResolver.Make (struct let config = cfg end) (APCSP)))
       : ResolverType)
