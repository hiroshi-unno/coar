...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid Validator Configuration (%s): %s" filename msg
end
...
module Make (RLCfg: RLConfig.ConfigType) (Cfg: Config.ConfigType)
  ...
  let check_clause_param_smt ?(timeout=None) fenv params_senv uni_senv phi =
    let qphi =
      LogicOld.Formula.forall
        (Map.Poly.to_alist @@ Logic.to_old_sort_env_map Logic.ExtTerm.to_old_sort uni_senv)
        phi
    in
  ...
  let check_clause_param_strat_synth ?(timeout=None) fenv params_senv uni_senv phi =
    ...
    let esenv =
      Map.Poly.to_alist @@ Logic.to_old_sort_env_map Logic.ExtTerm.to_old_sort params_senv
    in
    let usenv =
      Map.Poly.to_alist @@ Logic.to_old_sort_env_map Logic.ExtTerm.to_old_sort uni_senv
    in
    ...
    | Some f ->
      ...
      | UNSAT ->
        ...
        (*Debug.print @@ lazy "strategy template:";
          ...
          Debug.print @@ lazy "";*)
        let chc =
          (Map.Poly.empty, Logic.Term.mk_apps (Logic.BoolTerm.mk_imply ()) [fml; Logic.BoolTerm.mk_bool false]) :: chc
        in
        let problem =
          PCSP.Problem.of_formulas ~params:(PCSP.Params.make @@ Map.of_list_exn params) @@
  ...
  let find_all_counterexamples clauses fenv ((params_senv, cand) : CandSol.t) =
    ...
    let cls_nonparam, cls_param =
      ...
      Set.Poly.map clauses ~f:(fun cl ->
          ...
          let phi =
            Logic.ExtTerm.to_old_formula Map.Poly.empty (Map.force_merge params_senv uni_senv) phi []
          in
          ...
          let params_senv' = Map.Poly.filter_keys params_senv ~f:(Set.mem fvs) in
    ...
    |> Set.concat_map ~f:(fun ((cls, phi, res), src_cl) ->
        ...
        | `Sat model -> (* not all but some clause in cls has a countermodel *)
          ...
          in
          if Fn.non Set.is_empty res then Set.Poly.map res ~f:(fun ex -> ex, src_cl)
          else
        ...
        | _ ->
          ...
          Set.Poly.singleton @@ (ExClause.mk_unit_pos @@ ExAtom.mk_true (),
                                 [ClauseGraph.Dummy, true]) (*dummy*))

  (** ToDo: fix to use real MaxSMT *)
  let find_common_counterexamples_for_clause fenv num_ex (candidates : (CandSol.t * CandSol.tag) list) clause =
    ...
    let cand_phi_list =
      List.map candidates ~f:(fun (cand, cand_tag) ->
          ...
          cand,
          (cand_tag,
           LogicOld.Formula.mk_neg @@ Logic.ExtTerm.to_old_formula uni_senv Map.Poly.empty
             (Logic.Term.subst cand_map clause) []))
    in
    let spurious_cand_phi_list =
      List.filter cand_phi_list ~f:(snd >> snd >> Z3Smt.Z3interface.is_sat ~id fenv)
    in
  ...
  let receive_bounds vs =
    ...
    | Some messenger ->
      let add =
        VersionSpace.add_bound_as_learned_clause ~print:Debug.print id APCSP.problem vs
      in
      Common.Messenger.receive_all_infos_with messenger id
        ~filter:(function PCSP.Problem.(LowerBounds _ | UpperBounds _) -> true | _ -> false)
    ...
    | Some messenger ->
      let lbs_opt =
        if config.check_part_sol_with_lbs then
          Some (PCSP.Problem.senv_of APCSP.problem,
      ...
      let violated_non_query_clauses =
        Set.concat_map cexs ~f:(fun (_, sources) ->
            ...
            Set.Poly.of_list @@ List.filter_map sources ~f:(function
                | ClauseGraph.Clause c, _ when not @@ Set.mem stable_clauses c -> Some c
                | _ -> None))
      in
        ...
        | CandSol.Partial target_pvar ->
          let partial_sol_cand_of =
            let pvars =
              ...
              Map.Poly.find_exn (PCSP.Problem.partial_sol_targets_of APCSP.problem) target_pvar
            in
            fun sol -> Map.Poly.filter_keys sol ~f:(Set.mem pvars)
          in
          Debug.print @@ lazy
            (sprintf "send a candidate partial solution to the messenger for [%s]" @@
             Ident.name_of_tvar target_pvar);
          Common.Messenger.send_boardcast messenger id @@
          PCSP.Problem.CandidateSolution
            (partial_sol_cand_of sol, partial_sol_cand_of sol_before_red,
             violated_non_query_clauses, full_non_query_clauses, lbs_opt)
        | Total _with_sampling ->
          let target_pvars =
            Map.key_set @@ PCSP.Problem.partial_sol_targets_of APCSP.problem
          in
          if Set.is_empty target_pvars then begin
            (*ToDo: this case is necessary because [partial_sol_targets_of pfwcsp] is empty if [gen_extra_partial_sols] is false*)
            Debug.print @@ lazy ("send a candidate partial solution to the messenger");
            Common.Messenger.send_boardcast messenger id @@
            PCSP.Problem.CandidateSolution
              (sol, sol_before_red,
               violated_non_query_clauses, full_non_query_clauses, lbs_opt)
          end
          else if config.check_part_sol_with_lbs ||
                  ...
                  Set.exists target_pvars ~f:(fun pvar ->
                      PCSP.Problem.is_qualified_partial_solution ~print:Debug.print
                        (Map.Poly.find_exn (PCSP.Problem.dep_graph_of APCSP.problem) pvar)
  ...
  let validate vs (candidates : (CandSol.t * CandSol.tag) list) =
    ...
    receive_bounds vs;
    Debug.print @@ lazy
      (VersionSpace.str_of_learned_clauses (PCSP.Problem.senv_of APCSP.problem) @@
       VersionSpace.learned_clauses_of vs);
    let clauses =
      Set.union (PCSP.Problem.clauses_of APCSP.problem) @@
      Set.Poly.of_list @@ Hash_set.to_list @@ VersionSpace.learned_clauses_of vs
    in
    let res =
      ...
      | AllExamples ->
        ...
        let rec iter examples = function
          ...
          | (cand, cand_tag) :: candidates ->
            if RLCfg.config.enable && RLCfg.config.show_candidates then begin
              RLConfig.lock ();
              Debug.print_stdout @@ lazy
                (sprintf "candidate: %s" @@ Yojson.Safe.to_string @@ CandSol.to_yojson cand);
              RLConfig.unlock ()
            end;
            let hard_clauses =
              match cand_tag with
              | CandSol.Partial pvar ->
                let pvs = Map.Poly.find_exn (PCSP.Problem.dep_graph_of APCSP.problem) pvar in
                Set.filter clauses ~f:(is_hard pvs)
              | _ -> clauses
            in
            let cexs = find_all_counterexamples hard_clauses (VersionSpace.fenv_of vs) cand in
            if RLCfg.config.enable then begin
              ...
              Debug.print_stdout @@ lazy
                (sprintf "reward: %s/%s" (string_of_int num_sat) (string_of_int total));
            ...
            | CandSol.Partial _ ->
              if config.check_part_sol_with_lbs || Set.is_empty cexs then
                ...
                send_candidate_partial_solution vs cexs cand cand_tag hard_clauses;
              ...
              if Set.is_empty cexs then Second cand
                ...
                let examples' =
                  if with_sampling then
                    ...
                    Set.union examples @@ Set.Poly.map cexs ~f:(fun (ex, srcs) ->
                        ExClause.normalize_params unknowns @@ ExClause.normalize ex, srcs)
                  else examples
                in
                iter examples' candidates
              end
      ...
      | ExamplesUpTo num_of_examples ->
        ...
        *)
        ...
        let examples, spurious_cands =
          Set.fold clauses ~init:(Set.Poly.empty, Set.Poly.empty)
            ~f:(fun (examples, spurious_cands) cl ->
                let examples', spurious_cands' =
                  find_common_counterexamples_for_clause (VersionSpace.fenv_of vs)
                    num_of_examples candidates cl
                in
                let examples' =
                  Set.Poly.map examples' ~f:(fun ex -> ex, [ClauseGraph.Clause cl, false])
                in
                Set.union examples examples', Set.union spurious_cands spurious_cands')
        in
        ...
        | None -> First examples | Some cand -> Second cand
    in
    match res with
    | First examples ->
      Debug.print @@ lazy "******************************************";
