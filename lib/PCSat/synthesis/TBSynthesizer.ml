...
module Make (RLCfg: RLConfig.ConfigType) (Cfg: Config.ConfigType) (APCSP: PCSP.Problem.ProblemType) = struct
  ...
  let run_phase num_iters examples =
    ...
    | SolutionTemplate.Cands (cand, part_cands) ->
      ...
      let cands =
        ...
        (if config.generalize then [CandSol.generalize cand, CandSol.Total true] else []) @
        List.map part_cands ~f:(fun (cand, key) -> CandSol.make cand, CandSol.Partial key)
      in
