...
module Config = struct
  type t = {
    ...
    fn_classifier: FNClassifier.Config.t ext_file
  } [@@ deriving yojson]
  ...
  let instantiate_ext_files cfg =
    ...
    Ok { cfg with classifier = classifier;
                  wf_classifier = wf_classifier;
...
module Make (Cfg : Config.ConfigType) (APCSP: PCSP.Problem.ProblemType) = struct
  ...
  let classifier =
    ...
    Ok (module Classifier.Make (struct let config = cfg end) (APCSP)
        : Classifier.ClassifierType)

  let wf_classifier =
    let open Or_error in
    ExtFile.unwrap config.wf_classifier >>= fun cfg ->
    Ok (module WFClassifier.Make (struct let config = cfg end) (APCSP)
        : WFClassifier.WFClassifierType)

  let fn_classifier =
    let open Or_error in
    ExtFile.unwrap config.fn_classifier >>= fun cfg ->
    Ok (module FNClassifier.Make (struct let config = cfg end) (APCSP)
        : FNClassifier.FNClassifierType)
  ...
  let mk_candidate vs labeling (tvar, sort) =
    ...
    let labeling = match Map.Poly.find labeling pvar with None -> Map.Poly.empty | Some labeling -> labeling in
    let examples = VersionSpace.examples_of vs in
    let senv = Logic.SortMap.to_old_sort_map Logic.ExtTerm.to_old_sort @@ PCSP.Problem.senv_of APCSP.problem in
    Debug.print @@ lazy
      (Printf.sprintf
    ...
    let params =
      ...
      |> LogicOld.SortEnv.of_sorts in
    let open Or_error in
    if wf then
      wf_classifier >>= fun wf_classifier ->
      let (module WFClassifier : WFClassifier.WFClassifierType) = wf_classifier in
      WFClassifier.mk_classifier pvar params table labeling (senv, examples)
    else if fn then
      fn_classifier >>= fun fn_classifier ->
      let (module FNClassifier : FNClassifier.FNClassifierType) = fn_classifier in
      FNClassifier.mk_classifier pvar params table labeling (senv, examples)
    else
      classifier >>= fun classifier ->
      let (module Classifier : Classifier.ClassifierType) = classifier in
  ...
  let run_phase e =
    ...
    print @@ lazy "********************************************";
    let open Or_error in
    List.mapi labelings ~f:(fun i labeling ->
        print @@ lazy (sprintf "Classification for the %s Labeling:" (Ordinal.string_of i));
        ...
        |> Result.all >>= (fun res ->
            ...
            let cand = CandSol.of_old (Map.force_merge_list paramss, Set.Poly.of_list cand) in
            print @@ lazy "  Learned Classifier:";
            print @@ lazy (CandSol.str_of cand);
            print @@ lazy "********************************************";
            Ok cand))
    |> Result.all (* ToDo: errors can be tolerated *) >>= fun cands ->
