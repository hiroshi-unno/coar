...
module Make (Cfg: Config.ConfigType) (APCSP: PCSP.Problem.ProblemType) = struct
  ...
  let mk_candidate vs labeling (tvar, sort) =
    ...
    let params =
      ...
      |> LogicOld.sort_env_list_of_sorts in
    let open Or_error in
    if wf then
      wf_classifier >>= fun (module WFClassifier : WFClassifier.WFClassifierType) ->
      WFClassifier.mk_classifier pvar params table labeling (senv, examples)
    else if fn then
      fn_classifier >>= fun (module FNClassifier : FNClassifier.FNClassifierType) ->
  ...
  let run_phase _iters e =
    ...
    List.mapi labelings ~f:(fun i (labeling, sample_examples) ->
        print @@ lazy (sprintf "Classification for the %s Labeling:" @@ Ordinal.string_of @@ Ordinal.make i);
        PCSP.Problem.senv_of APCSP.problem |> Map.Poly.to_alist
        |> List.map ~f:(mk_candidate (State.version_space_of e) labeling)
        |> Result.all >>= (fun res ->
            Ok (List.n_cartesian_product res
                |> List.map ~f:(fun res ->
    ...
    |> Result.all (* ToDo: errors can be tolerated *) >>= fun candss ->
