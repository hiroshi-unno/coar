...
module Config = struct
  type t =
    ...
    | UnsatCore
    | RL
    | CBSynth of CBSynthesizer.Config.t [@@deriving yojson]
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid TemplateUpdateStrategy Configuration (%s): %s" filename msg
...
module Make (RLCfg: RLConfig.ConfigType) (Cfg: Config.ConfigType)
  ...
  module S =
    (val (match config with
         | Config.Disable ->
           (module struct let update _ _ _ _ = failwith "template update is disabled"
             ...
             let update template_modules _ _ = function
               | None ->
                 Map.Poly.iter template_modules
                   ~f:(fun (module FT : Template.Function.Type) -> FT.restart ())
               | Some pvar_labels_map ->
                 (* ToDo: output -> boolean value *)
                 Map.Poly.iteri template_modules
                   ~f:(fun ~key:pvar ~data:(module FT : Template.Function.Type) ->
             ...
             let update template_modules num_iters _ pvar_labels_map_opt =
               ...
               if RLCfg.config.show_user_time then
                 Debug.print_stdout @@ lazy
                   (sprintf "user time: %s" (string_of_float @@ Timer.get_user_time ()));
               Map.Poly.iteri template_modules
                 ~f:(fun ~key:pvar ~data:(module FT : Template.Function.Type) ->
                     Debug.print_stdout @@ lazy
                       ("### information about " ^ Ident.name_of_tvar @@ FT.name_of ());
                     if RLCfg.config.show_kind then
                       Debug.print_stdout @@ lazy (sprintf "kind: %s" (FT.kind_of ()));
                     if RLCfg.config.show_sort then
                       Debug.print_stdout @@ lazy
                         (sprintf "sort: %s" (LogicOld.Term.str_of_sort @@ FT.sort_of ()));
                     ...
                     (match pvar_labels_map_opt with
                      ...
                      | Some pvar_labels_map ->
                        if RLCfg.config.show_relevance then
                          Debug.print_stdout @@ lazy (sprintf "occurs in unsat core?: %B" @@ Map.Poly.mem pvar_labels_map pvar));
                     FT.show_state ~config:RLCfg.config @@
                     match Option.(pvar_labels_map_opt >>= Fn.flip Map.Poly.find pvar) with None -> Set.Poly.empty | Some labels -> labels);
                 ...
                 ~f:(fun ~key:pvar ~data:(module FT : Template.Function.Type) ->
                     ...
                     | Some pvar_labels_map ->
                       ...
                       | None ->
                         ...
                         if RLCfg.config.show_pcsat_actions then
                           Debug.print_stdout @@ lazy
                             (sprintf "PCSat actions for updating unrelated %s:\n%s"
                                (Ident.name_of_tvar pvar)
                                (String.concat ~sep:"\n" ["end"]));
                         Debug.print_stdout @@ lazy
                           (sprintf "input actions for updating unrelated %s"
                              (Ident.name_of_tvar pvar));
                         Out_channel.flush Out_channel.stdout;
                         FT.rl_action Set.Poly.empty;
                         RLConfig.unlock ()
                       | Some labels ->
                         ...
                         if RLCfg.config.show_pcsat_actions then
                           Debug.print_stdout @@ lazy
                             (sprintf "PCSat actions for updating related %s:\n%s"
                                (Ident.name_of_tvar pvar)
                                (String.concat ~sep:"\n" @@ FT.actions_of labels));
                         Debug.print_stdout @@ lazy
                           (sprintf "input actions for updating related %s"
                              (Ident.name_of_tvar pvar));
                         Out_channel.flush Out_channel.stdout;
                         FT.rl_action labels;
                         RLConfig.unlock())
         ...
         | Config.CBSynth cfg ->
           (module CBSynthTemplateUpdateStrategy.Make (struct let config = cfg end) (APCSP) : TemplateUpdateStrategyType)))
