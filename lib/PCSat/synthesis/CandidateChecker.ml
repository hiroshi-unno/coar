...
module Make (Verbose : Debug.Config.ConfigType) (APCSP : PCSP.Problem.ProblemType) = struct
  ...
  let of_cand_and_ex ?(smt_timeout=None)
      uenv exi_senv (cl:ExClause.t) ((params_senv, cand) : CandSol.t) =
    let clause, uni_senv =
    ...
    let phi = Logic.ExtTerm.to_old_formula exi_senv uni_senv (Logic.Term.subst sub clause) [] in
    ...
    | `Sat model -> (* not all but some clause in cls caused a countermodel *)
      ...
      Debug.print @@ lazy
        ("phi: " ^ (LogicOld.Formula.str_of @@ LogicOld.Formula.mk_neg phi) ^
         "\nmodel: " ^ LogicOld.str_of_model model);
      LogicOld.UTermEnv.update_by_model uenv model;
      let cond =
        LogicOld.Formula.and_of @@ List.filter_map model ~f:(function
            | (tvar, _), Some term ->
              begin try
            ...
            | _ -> None) in
      let sub =
        List.fold model ~init:Map.Poly.empty ~f:(fun ret ->
            function
      ...
      if Fn.non Set.is_empty res then (* note that res never represents true *)begin
        ...
        let dtenv = Z3Smt.Z3interface.z3_dtenv_of_dtenv ctx (PCSP.Problem.dtenv_of APCSP.problem) in
  ...
  let check_candidates ?(smt_timeout=None) vs (cands : CandSol.t list) =
    ...
    if Set.is_empty dpos' && Set.is_empty dneg' && Set.is_empty und' then `Valid
      ...
      | `Unsat conflicts ->
        ...
        `Unsat
      | `Result (dpos', dneg', und') ->
        `Invalid (dpos', dneg', und')
end
