...
let res_invalid = -1
let res_unknown = 0
let res_encode = function Some true -> res_valid | Some false -> res_invalid | None -> res_unknown
let str_of_res r =
  ...
  if Hashtbl.Poly.mem table.aenv atom then ()
    ...
    for qi = 0 to (length_of_quals table) - 1 do
      table.table <- IncrBigArray2.auto_set table.table qi ai
          (res_encode @@ ExAtom.eval_pred ~id fenv qdeps (table.params, table.qarr.(qi)) atom)
    done
  end
let update_with_qualifier ~id (table:table) fenv qdeps qual =
  if Hashtbl.Poly.mem table.qenv qual then ()
    ...
    for ai = 0 to length_of_atoms table - 1 do
      table.table <- IncrBigArray2.auto_set table.table qi ai
          (res_encode @@ ExAtom.eval_pred ~id fenv qdeps (table.params, qual) @@ table.aarr.(ai))
    done
  end
...
let clone t =
  Hashtbl.Poly.fold t ~init:(create ()) ~f:(fun ~key ~data acc ->
      Hashtbl.Poly.add_exn ~key ~data:(clone_table data) acc; acc)
...
let index_of_atom ~id table fenv qdeps atom =
  ...
  | None ->
    ...
    let ai = length_of_atoms table in
    update_with_atom ~id table fenv qdeps atom; ai
(* this may update truth table *)
let index_of_qual ~id table fenv qdeps qual =
  ...
  | None ->
    ...
    let qi = length_of_quals table in
    update_with_qualifier ~id table fenv qdeps qual; qi
...
let num_pos_neg_labeled_atoms alist =
  let pelist, nelist = Map.Poly.partition_tf alist ~f:((=) label_pos(*ToDo*)) in
...
let str_of_atoms t alist =
  let alist =
    List.sort (Map.Poly.to_alist alist) ~compare:(fun (x, _) (y, _) -> Int.compare x y)
  in
  String.concat_map_list ~sep:"\n" alist ~f:(fun (ai, label) ->
      let atom = t.aarr.(ai) in sprintf "%d (%d): %s" ai label (ExAtom.str_of atom))
let str_of_table t (qlist, alist) =
  let qlist = List.sort qlist ~compare:Int.compare in
  let alist = List.sort (Map.Poly.to_alist alist) ~compare:(fun (x, _) (y, _) -> Int.compare x y) in
  if List.is_empty alist then ""
  else
    String.concat_map_list ~sep:"\n" qlist ~f:(fun qi ->
        String.concat_map_list alist ~f:(fun (ai, _) -> str_of_res t.table.{qi, ai}))
...
let str_of (Ident.Pvar name, t) (qlist, alist) =
  ...
  else sprintf "reduced qualifiers (%d):\n%s\n" (List.length @@ reduced_qlist) (str_of_quals t reduced_qlist)
