...
let make exi_senv ps ns phi =
  ...
  with _ ->
    let unknowns, params =
      Formula.term_sort_env_of phi |> Set.partition_tf ~f:(fst >> Map.Poly.mem exi_senv)
    in
    if Set.is_empty unknowns then begin
      ...
      let ps, ns =
        if Set.is_empty params then (* without parameters *)
          ...
          Set.Poly.map ns ~f:(ExAtom.of_old_atom exi_senv neg_phi)
      in
      if Set.exists ps ~f:ExAtom.is_ppapp || Set.exists ns ~f:ExAtom.is_ppapp then
    ...
    end else if Set.is_empty params then
      (* with function variables and without parameters *)
      Some { positive = Set.add (Set.Poly.map ps ~f:of_old_atom)
...
let is_unit (clause: t) = Set.length clause.positive + Set.length clause.negative = 1
(** contradiction *)
let is_empty (clause: t) = Set.(is_empty clause.positive && is_empty clause.negative)
...
let old_atom_of_papp ((pvar, sorts), terms) = Atom.mk_app (Predicate.mk_var pvar sorts) terms
let old_atom_of_uclause (uclause: t) =
  uclause |> papp_of_uclause
  |> (function
      | None ->
        (match ppapp_of_uclause uclause with
         | None -> None
         | Some ((param_senv, phi), papp) ->
           if Formula.is_true phi then Some (param_senv, old_atom_of_papp papp) else None(*ToDo*))
...
let to_old_formula clause =
  ...
  Map.of_set_exn @@ Set.concat_map ~f:Set.of_map @@ Set.union params_pos params_neg,
...
let iterate_senv cl =
  let senv = Set.fold cl.negative ~init:([], Set.Poly.empty) ~f:ExAtom.iterate_senv in
...
let refresh_params senv cl =
  let map =
    Map.of_set_exn @@ Set.Poly.filter_map ~f:(fun x ->
        if Map.Poly.mem senv x then None else Some (x, Ident.mk_fresh_dontcare "")) @@
    tvs_of cl
  in
  rename map cl

let str_of cl =
  let pos = cl.positive |> Set.Poly.map ~f:ExAtom.str_of |> String.concat_set ~sep:" \\/ " in
  let neg = cl.negative |> Set.Poly.map ~f:ExAtom.str_of |> String.concat_set ~sep:" /\\ " in
...
let simplify unknowns pos neg (clause, srcs) =
  ...
  let cl_pos = Set.filter clause.positive ~f:(fun cl ->
      match matched_by unknowns neg cl with
      | Some (_, src) -> srcs := src @ !srcs; false
      | None -> true) in
  let cl_neg = Set.filter clause.negative ~f:(fun cl ->
      match matched_by unknowns pos cl with
      | Some (_, src) -> srcs := src @ !srcs; false
      | None -> true) in
  if Set.exists cl_pos ~f:(matched_by unknowns pos ~check_inst:true >> Option.is_some) (* clause is subsumed by pos *) ||
     Set.exists cl_neg ~f:(matched_by unknowns neg ~check_inst:true >>  Option.is_some) (* clause is subsumed by neg *) ||
     (*ToDo: this never holds?*)Fn.non Set.is_empty @@ Set.inter cl_pos cl_neg (* clause is tautology *) then
    None
  else Some (normalize_params unknowns { positive = cl_pos; negative = cl_neg }, !srcs)
let rec simplify_nepvs nepvs clause =
  if Fn.non Set.is_empty clause.positive ||
     not (Set.for_all clause.negative ~f:(fun atm ->
  ...
  then clause
    ...
    | None -> clause
    | Some neatm ->
      simplify_nepvs nepvs { clause with negative = Set.remove clause.negative neatm }
...
let exists ~f clause = Set.exists ~f clause.positive || Set.exists ~f clause.negative

let equal e1 e2 = Set.equal e1.positive e2.positive && Set.equal e1.negative e2.negative
