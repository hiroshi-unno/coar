...
let of_ground_atom cond exi_senv t =
  of_old_atom exi_senv cond @@ Logic.ExtTerm.to_old_atom exi_senv Map.Poly.empty t []
...
let pvar_of = function
  | PApp ((pvar, _), _) | PPApp (_, ((pvar, _), _)) -> Some pvar | _ -> None
let sorts_of = function
  | PApp ((_, sorts), _) | PPApp (_, ((_, sorts), _)) -> Some sorts | _ -> None
let pvar_sorts_of = function
  | PApp ((pvar, sorts), _) | PPApp (_, ((pvar, sorts), _)) -> Some (pvar, sorts) | _ -> None
let tvs_of = function
...
let cond_map_of ~id fenv cond =
  if Formula.is_true cond then Map.Poly.empty else
    ...
    | `Sat model ->
      List.fold_left model ~init:Map.Poly.empty ~f:(fun ret ((tvar, _), term) ->
          match term with
          | None -> ret
...
let normalize_papp (target, terms) =
  ...
  List.map terms ~f:(fun term ->
      let term' = Normalizer.normalize_term @@ Evaluator.simplify_term term(*ToDo: check*) in
...
let normalize t = match t with
  ...
  | PPApp (pred, papp) as t-> PPApp (normalize_pred (tvs_of t) pred, normalize_papp papp)
...
let eval_pred_aux ~id fenv qdeps (params, qual) (_param_senv, cond, _sorts, args) =
  ...
  let cond' =
    ...
    | Some qdep ->
      ...
      let qdep = QDep.map_atom qdep ~f:(function
          | QDep.LFormula (phi, _) when Stdlib.(phi = qual) -> QDep.mk_atom QDep.LTrue
          | lit -> QDep.mk_atom lit) in
  ...
  Evaluator.check ~cond:(Formula.mk_and cond cond') (Z3Smt.Z3interface.is_valid ~id fenv) phi

(* assume the ExAtom argument is papp or ppapp*)
let eval_pred ~id fenv qdeps (params, qual) = function
  | PApp ((_pvar, sorts), args) ->
    eval_pred_aux ~id fenv qdeps (params, qual) (Map.Poly.empty, Formula.mk_true (), sorts, args)
