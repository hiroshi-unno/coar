...
type heat_map = (unit(*BMIsvm.Model.t*) * (string, int) Hashtbl.Poly.t) option ref
type fenv = FunEnv.t
...
let reduced_terms_of_pvar pvar t = (hspace_of_pvar pvar t).terms(*ToDo*)
...
let add_examples t examples =
  Set.iter examples ~f:(fun (ex, srcs) -> ClauseGraph.add_ex_from t.examples ex srcs)
...
let update_hspace pvar hspace t =
  ...
  | None ->
    Hashtbl.Poly.add_exn t.hspaces ~key:pvar ~data:hspace
  | Some hspace' ->
...
let update_truth_table ~id t =
  Hashtbl.Poly.iteri t.hspaces ~f:(fun ~key:(Ident.Tvar name) ~data ->
      TruthTable.update_map_with_qualifiers ~id t.truth_table t.fenv data.qdeps ((*ToDo*)Ident.Pvar name) (data.params, data.quals));
...
let add_learned_clause id t (uni_senv, ps, ns, phi) =
  match Hash_set.find t.learned_clauses ~f:(fun (_, ps1, ns1, _) ->
      Set.equal ps ps1 && Set.equal ns ns1) with
  | Some (uni_senv1, ps1, ns1, phi1) ->
    ...
    let phi' =
      ...
      |> Evaluator.simplify |> Z3Smt.Z3interface.z3_simplify ~id (LogicOld.get_fenv ())
  ...
  Hashtbl.Poly.update t.upper_bounds key ~f:(function
      ...
      | Some old_term ->
        ...
        Logic.ExtTerm.and_of
          [Logic.ExtTerm.beta_reduction term args;
           Logic.ExtTerm.beta_reduction old_term args])
let add_bound_as_learned_clause ~print id pfwcsp vs pvar (arity, (param_senv, term)) upper =
  print @@ lazy
    (sprintf "*** a new %s bound of [%s]" (if upper then "upper" else "lower") @@
     Ident.name_of_tvar pvar);
  match Map.Poly.find (PCSP.Problem.senv_of pfwcsp) pvar with
  | None -> ()
  | Some _ ->
    let args_log, sargs =
      Map.Poly.find_exn (PCSP.Problem.args_record_of pfwcsp) @@ Ident.tvar_to_pvar pvar
    in
    let params, uni_senv, term =
      ...
      in
      let term =
        Logic.ExtTerm.beta_reduction term @@ List.map ~f:(fst >> Logic.ExtTerm.mk_var) @@
    ...
    in
    let papp = Logic.ExtTerm.(mk_var_app pvar @@ List.map params ~f:(fst >> mk_var)) in
    if upper then begin
      print @@ lazy
        (sprintf "added a new learned clause: %s(%s) => %s"
           (Ident.name_of_tvar pvar)
           (String.concat_map_list ~sep:"," params ~f:(fst >> Ident.name_of_tvar))
...
let update_heat_map (*~print*) ?(enc_bool_to_float = 100.0) ?(ignore_bool_args_variance = false) senv wexs t : unit =
