...
let is_int_psym = function
  ...
  | _ -> false
let mk_act a b psym = assert (is_int_psym psym); (a, b, psym)
  ...
  | LFormula (phi, (a, b, psym)) ->
    ...
    (match Map.Poly.find env phi with
     | Some x ->
       ...
       let t = T_int.mk_add (T_int.mk_mult var (T_int.from_int a)) (T_int.from_int b) in
       Formula.mk_atom @@ Atom.mk_psym_app psym [t; T_int.zero ()]
     | None ->
       print_endline ("phi: " ^ Formula.str_of phi);
       print_endline ("env: " ^ String.concat_map_list ~sep:"," ~f:(fun (phi, x) -> Formula.str_of phi ^ "|->" ^ Ident.name_of_tvar x) @@ Map.Poly.to_alist env);
...
let rec condition_of env = function
  ...
  | CImply (t1, t2) -> Formula.mk_imply (condition_of env t1) (condition_of env t2)
  | CEquiv (t1, t2) ->
    Formula.mk_and
...
let qual_and_deps_of qual conds =
  ...
  let dep =
    and_of @@ List.dedup_and_sort ~compare:Stdlib.compare @@
