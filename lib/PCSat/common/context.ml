...
let version_of version (vs:VersionSpace.t) : version =
  {graph = vs.examples; truth_table = vs.truth_table; problem = version.problem; senv = version.senv}
...
let remove_unused old_version new_version =
  ...
  Map.Poly.iteri old_senv ~f:(fun ~key:(Tvar v) ~data ->
      let tvar = Tvar v in
      let pvar = Pvar v in
      if not (Map.Poly.mem new_senv tvar) || Stdlib.(<>) (Map.Poly.find_exn new_senv tvar) data
      then Hashtbl.Poly.remove old_version.truth_table pvar
      else if Hashtbl.Poly.mem old_version.truth_table pvar then
        Hashtbl.Poly.set ~key:pvar ~data:(Hashtbl.Poly.find_exn old_version.truth_table pvar)
