...
module Make (Cfg: Config.ConfigType) (APCSP: PCSP.Problem.ProblemType) = struct
  ...
  let mk_classifier pvar (params : sort_env_list) table labeling _examples =
    ...
    let rec inner () =
        ...
        Set.fold ~init:(Map.Poly.empty, Map.Poly.empty) pos_neg_ex
          ~f:(fun (key_constr_map, key_tvar_update_list_map) (polarity, ((pvar, sorts), args)) ->
              ...
              let eval_qual (key, (qi, _params, _phi)) =
                let ai = TruthTable.index_of_atom ~id tt fenv qdeps (ExAtom.normalize_params atom) in
              ...
              let constr =
                ...
                |> Logic.Term.subst (Map.Poly.singleton (Ident.pvar_to_tvar pvar) template)
      ...
      match Z3Smt.Z3interface.check_sat_unsat_core Map.Poly.empty key_constr_map with
      | `Sat model -> Debug.print @@ lazy "sat";
        ...
        Ok [Map.Poly.empty(* parameters of parametric candidate*), (pvar, (params, phi))]
      | `Unsat unsat_keys ->
