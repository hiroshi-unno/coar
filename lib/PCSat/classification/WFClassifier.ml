...
module Config = struct
  type t =
    ...
    | DT of DT_WFClassifier.Config.t [@@deriving yojson]
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid WFClassifier Configuration (%s): %s" filename msg
end
...
module Make (Cfg: Config.ConfigType) (Problem: PCSP.Problem.ProblemType) =
  (val (match Cfg.config with
       | TB cfg ->
         (module TB_Classifier.Make (struct let config = cfg end) (Problem): WFClassifierType)
       | DT cfg ->
         (module DT_WFClassifier.Make (struct let config = cfg end) (Problem): WFClassifierType)))