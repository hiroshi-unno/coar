...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid Generator Configuration (%s): %s" filename msg

  module type ConfigType = sig val config : t end
end

module Make (Cfg : Config.ConfigType) (APCSP: Problem.ProblemType) : GeneratorType = struct
  ...
  let pcsp =
    if config.normalize_pcsp then begin
      Debug.print @@ lazy "normalizing pfwCSP for qualifiers extraction";
      let cls =
        Problem.clauses_of APCSP.problem
        |> Set.Poly.map ~f:(Clause.refresh_pvar_args (Problem.senv_of APCSP.problem))
      in
      ...
      Set.Poly.map cls ~f:(fun (uni_senv, ps, ns, phi) ->
          ...
          Debug.print @@ lazy
            (sprintf "normalizing %s clause (out of %d)"
               (Ordinal.string_of @@ Ordinal.make !count) (Set.length cls));
          (* assume that [phi] is alpha-renamed *)
          let phi =
            Normalizer.normalize_let @@
            Logic.ExtTerm.to_old_formula (Problem.senv_of APCSP.problem) uni_senv phi []
          in
          let uni_senv =
          ...
          let phi =
            ...
            Evaluator.simplify @@
            (*Formula.elim_ite @@*)
            (*Formula.elim_let_equivalid @@*) (*Formula.elim_let*) phi
          in
          ...
          let uni_senv_phi, _, phi =
            Qelim.qelim bounds (Problem.senv_of APCSP.problem) (uni_senv, [], phi)
          in
          ...
          if Set.length ps + Set.length ns = 1 then phi
            ...
            Set.Poly.map (Set.union ps ns) ~f:(fun atm ->
                ...
                if Set.is_empty @@ Set.inter fvs_phi fvs_atm
                then LogicOld.Formula.mk_false ()
                else
                  (*print_endline ("before QE: " ^ Formula.str_of @@ Logic.ExtTerm.to_old_formula (Problem.senv_of APCSP.problem) uni_senv phi []);
                    Out_channel.flush stdout;*)
                  let uni_senv', _, phi' =
                    let bounds =
                      Map.of_set_exn @@
                      Set.Poly.map ~f:(fun x -> x, Map.Poly.find_exn senv x) fvs_atm
                    in
                    Qelim.qelim bounds (Problem.senv_of APCSP.problem) (uni_senv, [], phi)
                  in
    ...
    end else APCSP.problem

  let rec module_of_domain = function
    | Config.Null ->
      (module Null : QualifierType)
    | Interval ->
      (module Interval : QualifierType)
    | Octagon  ->
      (module Octagon : QualifierType)
    | Octahedron ->
      (module Octahedron : QualifierType)
    | Polyhedron ub ->
      (module Polyhedron.Make (struct let upper_bound = ub end) : QualifierType)
    | Polynomial ->
      (module Polynomial : QualifierType)
    ...
    | NN dummy_param ->
      (module NN.Make (struct let dummy_param = dummy_param end) : QualifierType)
    | Union ds ->
  ...
  let extract pcsp =
    ...
    let cls =
      Set.Poly.map (Problem.formulas_of pcsp) ~f:(fun (uni_senv, phi) ->
          Logic.ExtTerm.to_old_formula (Problem.senv_of pcsp) uni_senv phi [])
    in
    let count = ref 0 in
    Set.fold cls ~init:Set.Poly.empty ~f:(fun acc phi ->
        ...
        List.fold atomics ~init:acc ~f:(fun acc atomic ->
            ...
            if Fn.non Set.is_empty @@
              Set.inter (Set.Poly.map ~f:fst tvs) (Map.key_set @@ Problem.senv_of pcsp) ||
               Set.is_subset tvs ~of_:fvs(* tvs and bvs has no common element *) then acc
            else
              let acc =
                List.fold pos_papp_tvs ~init:acc ~f:(fun acc (pa, pftv) ->
                    if Set.is_subset tvs ~of_:(Set.union pftv fvs)
                    (*&& (Set.is_empty fvs || match atomic with Atom.App (_, [t; _], _) -> Fn.non Term.is_bool_sort (Term.sort_of t) | _ -> false)*) then
                      (*let _ = print_endline @@ "pos" ^ string_of_int @@ Atom.ast_size atomic in
                        let _ = print_endline @@ "[atomic] " ^ (Atom.str_of atomic) in*)
                      let atm =
                        Formula.aconv_tvar @@
                        Formula.exists (Set.to_list fvs) @@ Evaluator.simplify @@
                        (*Formula.mk_and cond @@*) Formula.mk_atom @@ Atom.negate atomic
                      in
              ...
              List.fold neg_papp_tvs ~init:acc ~f:(fun acc (pa, pftv) ->
                  ...
                  (*&& (Set.is_empty fvs || match atomic with Atom.App (_, [t; _], _) -> Fn.non Term.is_bool_sort (Term.sort_of t) | _ -> false)*) then
                    ...
                    let atm =
                      Formula.aconv_tvar @@
                      Formula.forall (Set.to_list fvs) @@ Evaluator.simplify @@
                      (*Formula.mk_imply cond @@*) Formula.mk_atom atomic
                    in
    ...
    |> List.map ~f:(fun lst ->
        let pvar, sorts =
          ...
          | Some (Atom.App (Predicate.Var (pvar, sorts), _, _), _) -> pvar, sorts
                  ...
                  try
                    let tvars =
                      List.map terms' ~f:(function
                          | Term.Var (x, _, _) -> x
                          | term -> failwith @@
                            sprintf "invalid term: %s @ Generator.extract" (Term.str_of term))
                    in
                    let ids =
                      let t_index_map =
                        Map.Poly.of_alist_exn @@ List.mapi tvars ~f:(fun i x -> x, i)
                      in
                      Set.Poly.map (Formula.tvs_of atomic) ~f:(Map.Poly.find_exn t_index_map)
                    in
                    (*Debug.print @@ lazy ("qualifier before sub: " ^ Formula.str_of atomic);*)
                    let phi =
                      let tsub = Map.Poly.of_alist_exn @@ List.zip_exn tvars terms in
                      Normalizer.normalize @@ Evaluator.simplify @@ Formula.subst tsub atomic
                    in
                    (*Debug.print @@ lazy ("qualifier after sub: " ^ Formula.str_of phi);*)
                    if Set.is_empty @@ Formula.fvs_of phi then None else Some (ids, phi)
                  with _ -> None
                end
              | atom, _ -> failwith @@
                sprintf "fail with %s @ Generator.extract" (Atom.str_of ~priority:20 atom))
        in
        pvar, Set.union bool_quals extracted_quals)
    |> List.map ~f:(fun (pa, quals) ->
        Debug.print @@ lazy
          (sprintf "eliminating quantifiers in qualifiers for %s" (Ident.name_of_pvar pa));
        pa,
        Set.Poly.filter_map ~f:(fun (ids, phi) ->
            if config.filter_out_quantified && Formula.is_bind phi
            then None else Some (ids, phi)) @@
        Set.fold quals ~init:Set.Poly.empty ~f:(fun acc (ids, phi) ->
            ...
            let phi' =
              ...
              Z3Smt.Z3interface.qelim ~id:(Problem.id_of pcsp) (Problem.fenv_of pcsp) phi
            in
            (*Debug.print @@ lazy ("qualifier after qelim: " ^ Formula.str_of phi');*)
            if (*ToDo*)Formula.is_bind phi' then acc else
              Set.union acc @@ Set.Poly.map ~f:(fun atm -> ids(*ToDo*), Formula.mk_atom atm) @@
  ...
  let qualifier_propagation (quals_map: (Ident.pvar, (int Set.Poly.t * Formula.t) Set.Poly.t) Map.Poly.t) =
    ...
    let pairs =
      ...
      Set.fold cls ~init:Set.Poly.empty ~f:(fun acc (_uni_senv, ps, ns, _) ->
          ...
          Debug.print @@ lazy
            (sprintf "propagating qualifiers across %s clause (out of %d) with %d atoms"
               (Ordinal.string_of @@ Ordinal.make !count) (Set.length cls) (Set.length atms));
                ...
                let sub =
                  List.fold2_exn ids1 ids2 ~init:Map.Poly.empty ~f:(fun acc id1 id2 ->
                      ...
                      Map.Poly.update acc x2
                        ~f:Set.(function None -> Poly.singleton x1 | Some xs -> add xs x1))
                in
                Set.add acc (Ident.tvar_to_pvar pvar1, Ident.tvar_to_pvar pvar2,
                             sub, Set.Poly.of_list ids1, Set.Poly.of_list ids2)))
    in
    let rec inner quals_map =
      let quals_map' =
        let count = ref 0 in
        Set.fold pairs ~init:quals_map ~f:(fun acc (pvar1, pvar2, sub, ids1_set, ids2_set) ->
            count := !count + 1;
            ...
            | Some (_, _, quals) ->
              if Set.is_empty quals then acc
                ...
                let header = "###_" in
                let sub = Map.Poly.map sub ~f:(Set.Poly.map ~f:(fun (Ident.Tvar s) -> Ident.Tvar (header ^ s))) in
                Debug.print @@ lazy
                  (sprintf "propagating %d qualifiers from %s to %s (%d/%d) (#sub = %d)"
                     (Set.length quals) (Ident.name_of_pvar pvar2) (Ident.name_of_pvar pvar1)
                     !count (Set.length pairs) (Map.Poly.length sub));
                let qs =
                  let sub1, sub2 =
                    Map.Poly.partition_map sub ~f:(function xs ->
                        if Set.is_singleton xs then First (Set.choose_exn xs) else Second xs)
                  in
                  Set.concat_map quals ~f:(fun (ids, qual) ->
                      if Fn.non Set.is_empty ids && Set.is_subset ids ~of_:ids2_set then
                        let init = Set.Poly.singleton @@ Formula.rename sub1 qual in
                        ...
                        Map.Poly.fold sub2 ~init ~f:(fun ~key ~data acc ->
                            ...
                            if Set.exists ids ~f:(x_i >> Ident.tvar_equal key) then
                              Set.concat_map data ~f:(fun x ->
                                  Set.Poly.map acc ~f:(Formula.rename (Map.Poly.singleton key x)))
                            else acc)
                      else Set.Poly.empty)
                in
                let qs =
                  Set.Poly.map qs ~f:(fun (ids, q) ->
                      let ren =
                        Map.of_set_exn @@ Set.Poly.map ids ~f:(fun i ->
                            let xi = x_i i in
                            Ident.Tvar (header ^ Ident.name_of_tvar xi), xi)
                      in
                ...
                Map.Poly.update acc pvar1 ~f:(function
                    ...
                    | Some (added, processed, work) ->
                      Set.union added (Set.diff qs (Set.union processed work)),
                      processed, work)
              end)
      in
      if Map.Poly.for_all quals_map' ~f:(fun (added, _, _) -> Set.is_empty added)
      then Map.Poly.map quals_map' ~f:(fun (_, processed, work) -> Set.union processed work)
      else inner @@ Map.Poly.map quals_map' ~f:(fun (added, processed, work) ->
          Set.Poly.empty, Set.union processed work, added)
    in
    inner @@ Map.Poly.map quals_map ~f:(fun quals -> Set.Poly.empty(*added*), Set.Poly.empty(*processed*), quals(*work*))

  let extracted_qualifiers =
    if config.extract_pcsp then begin
      Debug.print @@ lazy "*** extracting qualifiers";
      let extracted =
        List.fold (extract pcsp) ~init:Map.Poly.empty ~f:(fun map (key, d) ->
            Map.Poly.update map key ~f:(function None -> d | Some d' -> Set.union d d'))
      in
      Debug.print @@ lazy "*** propagating qualifiers";
      let propagated = if true then qualifier_propagation extracted else extracted in
      Debug.print @@ lazy "done";
      propagated
    end else Map.Poly.empty

  let div_mod_filter (_, phi) =
    Fn.non Set.is_empty @@ Set.inter (Set.Poly.of_list [T_int.Mod; T_int.Div])
