...
module Make (Config: sig val approx_level: int val csvm: float end) = struct
  ...
  in
    let params, quals = svm_half_spaces_of Config.approx_level Config.csvm sorts pos_examples neg_examples in
    Set.Poly.map ~f:(fun qual -> params, Normalizer.normalize qual) quals
  let str_of_domain = "SVM with approx_level=" ^ string_of_int Config.approx_level ^ " csvm=" ^ string_of_float Config.csvm
end
