...
module Make (Config: sig val dummy_param: int end) = struct
  let qualifiers_of _pvar sorts labeled_atoms (_senv, clause_examples) =
    ...
    let _pos_examples, _neg_examples, _und_examples =
      let pos_examples, neg_examples, und_examples = ClauseGraph.pos_neg_und_examples_of clause_examples in
      ExClauseSet.instantiate pos_examples,
      ExClauseSet.instantiate neg_examples,
      ExClauseSet.instantiate und_examples
    in
    let params, quals = nn_half_spaces_of Config.dummy_param sorts pos_examples neg_examples in
