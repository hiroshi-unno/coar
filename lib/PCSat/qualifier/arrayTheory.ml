...
let update_term_map ?(d=0)
  ...
  if d = 0 then term_map
    ...
    | T_array.SArray (si, se) ->
      let si_terms = match Map.Poly.find term_map si with Some si_terms-> si_terms | None -> Set.Poly.empty in
      let se_terms = match Map.Poly.find term_map se with Some se_terms-> se_terms | None -> Set.Poly.empty in
      let store_terms =
        Set.union terms @@
        Set.fold terms ~init:Set.Poly.empty ~f:(fun ret (t, conds) ->
            Set.fold si_terms ~init:ret ~f:(fun ret (si_term, conds1) ->
                Set.fold se_terms ~init:ret ~f:(fun ret (se_term, conds2) ->
                    Set.add ret @@
                    (T_array.mk_store si se t si_term se_term, conds @ conds1 @ conds2))))
      in
      let select_terms =
        Set.union se_terms @@
        Set.fold terms ~init:Set.Poly.empty ~f:(fun ret (t, conds) ->
            Set.union ret @@ Set.Poly.map si_terms ~f:(fun (si_term, conds1) ->
...
let qualifiers_of sort (terms : (Term.t * Formula.t list) Set.Poly.t) =
  ...
  match sort with
  | T_array.SArray _ ->
    Set.Poly.map ~f:Formula.mk_atom @@ Set.Poly.of_list @@ T_bool.mk_eqs @@
    List.filter_map ~f:(fun (t, _) -> if Term.is_var t then Some t else None) @@
    Set.to_list terms
  | _ -> Set.Poly.empty
