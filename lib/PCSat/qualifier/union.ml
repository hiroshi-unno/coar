...
module type Q = sig
  val qualifiers_of: Ident.pvar -> Sort.t list -> (ExAtom.t * int) Set.Poly.t -> (sort_env_map * VersionSpace.examples) -> (sort_env_list * Formula.t) Set.Poly.t
  ...
  let qualifiers_of pvar sorts labeled_atoms examples =
    ...
    List.map Config.domains ~f:(fun (module Q) ->
        Set.Poly.map ~f:(fun (params', phi) ->
            params,
            let map = Map.Poly.of_alist_exn @@ List.map2_exn params' params ~f:(fun (x, _) (y, s) -> x, Term.mk_var y s) in
