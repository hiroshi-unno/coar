...
let int_real_terms_of term_map params =
  Map.Poly.fold term_map ~init:Set.Poly.empty ~f:(fun ~key:sort ~data:terms ret ->
      match sort with
      | T_int.SInt | T_real.SReal ->
        Set.union ret @@
        Set.Poly.filter_map terms ~f:(fst >> function
          | Term.Var (x, _, _) when List.exists params ~f:(fst >> Stdlib.(=) x) -> None
          | term -> Some term)
      | _ -> ret)
let bool_quals_of terms params =
  ...
  |> Set.to_list |> List.map ~f:(fun (qual, dep) -> qual, (qual, dep))
  |> List.unzip
...
let add_term_map term_map term =
  ...
  match Map.Poly.find term_map sort with
  | Some terms -> Map.Poly.set term_map ~key:sort ~data:(Set.add terms (term, []))
    ...
    let quals' =
      ...
      Set.Poly.map quals ~f:(fun qual ->
          Formula.map_term qual ~f:(function
              | Term.FunApp (T_int.Mod, [t1; t2], info) as t when T_int.is_int t2 ->
  ...
  Set.concat @@ Set.Poly.filter_map quals ~f:(fun phi ->
      if Formula.is_atom phi then
        ...
        | Atom.App (Predicate.Psym (T_bool.Eq | T_bool.Neq), [t1; t2], _), _
          when Term.is_int_sort @@ Term.sort_of t1 && AffineTerm.is_affine t1 ->
...
let mk_mod_quals params =
  Set.Poly.of_list @@ List.filter_map params ~f:(fun (v, s) ->
      if Fn.non Term.is_int_sort s then None
        ...
        Option.some @@
        Formula.eq (T_int.mk_mod (Term.mk_var v s) (T_int.mk_int (Z.of_int 2)))
          (T_int.zero ()))
...
let reduce_dedup_quals =
  Set.Poly.map ~f:(Evaluator.simplify >> Normalizer.normalize >> Evaluator.simplify)
...
let qualifiers_of ?(add_mod2_quals=false) ~fenv depth hspace =
  ...
  let quals =
    ...
    if add_mod2_quals then Set.union (mk_mod_quals hspace.params) quals else quals
  in
  let term_map =
    let rec aux d term_map =
      if d > depth then term_map
        ...
        Map.Poly.fold term_map ~init:term_map ~f:(fun ~key:sort ~data:terms term_map ->
            ...
            | _ -> term_map)
    in
    let term_map = split_params_by_theory hspace.params in
    let term_map = Set.fold terms ~init:term_map ~f:add_term_map in
    ...
    Map.Poly.fold term_map ~init:(quals, hspace.qdeps)
      ~f:(fun ~key:sort ~data:terms (quals, qdeps) ->
          ...
          | _ ->
            ...
            qdeps(*ToDo*))
  in
  { hspace with depth = depth; quals = reduce_dedup_quals quals;
                qdeps = qdeps; terms = int_real_terms_of term_map hspace.params }
