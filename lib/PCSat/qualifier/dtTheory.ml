    ...
    | T_dt.SDT dt ->
      Set.fold terms ~init:term_map ~f:(fun term_map (term, conds) ->
          ...
          List.fold_left sel_terms ~init:term_map ~f:(fun term_map (sel_term, conds) ->
              ...
              | Some group ->
                Map.Poly.set term_map ~key:sel_sort ~data:(Set.add group (sel_term, conds))
              | None ->
                Map.Poly.add_exn term_map ~key:sel_sort ~data:(Set.Poly.singleton (sel_term, conds))))
    | _ -> term_map
...
let mk_eqs (ts : (Term.t * Formula.t list) list) =
  List.concat_mapi ts ~f:(fun i (t, conds) ->
      List.foldi ts ~init:[] ~f:(fun i1 ret (t1, conds1) ->
          ...
          if i1 <= i || Stdlib.(Term.sort_of t <> Term.sort_of t1) ||
             Stdlib.(t = t1) || is_cyclic t t1 then ret
...
let qualifiers_of sort (terms : (Term.t * Formula.t list) Set.Poly.t) =
  ...
  | T_dt.SDT dt ->
    let testers =
      List.concat_map (Datatype.conses_of dt) ~f:(fun cons ->
          Set.to_list @@ Set.Poly.map terms ~f:(fun (t, conds) ->
              let qual =
    ...
    in
    let eqs =
      Set.filter terms ~f:(fun (t, _) -> Term.is_var t || T_dt.is_sdt @@ Term.sort_of t)
      |> Set.to_list |> mk_eqs
    in
    let qdep_env =
