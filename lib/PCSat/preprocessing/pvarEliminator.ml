...
module Make (Verbose : Debug.Config.ConfigType)
            (Config : sig val resolution_threshold : int val propagate_lb_ub : bool end) = struct
  ...
  let resolve qelim exi_senv pv clauses =
    ...
    let cls =
      ...
      ClauseSetOld.simplify (Map.key_set exi_senv) (Z3Smt.Z3interface.is_valid ~id:!id (LogicOld.get_fenv ())) @@
  ...
  let subst_preds unknowns psub cls =
    ...
    in
    cls1, Set.Poly.filter_map cls2 ~f:(ClauseOld.simplify unknowns (Z3Smt.Z3interface.is_valid ~id:!id (LogicOld.get_fenv ())))
    ...
    let psub =
      ...
      Map.of_set_exn @@ Set.Poly.filter_map eliminable_pvs ~f:(fun pvar ->
          ...
          if Set.is_empty pvar_lbs_ubs then None
            ...
            if ok_lb && Z3Smt.Z3interface.is_valid ~id:!id (LogicOld.get_fenv ()) lb then begin
              Debug.print @@ lazy
            ...
            end else if ok_ub && Z3Smt.Z3interface.is_valid ~id:!id (LogicOld.get_fenv ()) @@ Formula.mk_neg ub then begin
              Debug.print @@ lazy
              ...
              None
            else if ok_lb && ok_ub && Z3Smt.Z3interface.is_valid ~id:!id (LogicOld.get_fenv ()) @@ Formula.mk_iff lb ub then begin
              Debug.print @@ lazy
  ...
  let elim_pvs ?(bpvs=Set.Poly.empty) pcsp =
    ...
    let rec inner iter pcsp0 =
      ...
      let eliminable_pvs =
        ...
        |> Set.filter ~f:(fun t ->
            not (Set.mem bpvs t ||
                 PCSP.Problem.is_fn_pred pcsp0 t || PCSP.Problem.is_ne_pred pcsp0 t ||
                 PCSP.Problem.is_wf_pred pcsp0 t || PCSP.Problem.is_nwf_pred pcsp0 t ||
                 PCSP.Problem.is_adm_pred pcsp0 t || PCSP.Problem.is_integ_pred pcsp0 t))
      ...
      let pcsp1 =
        ...
        if PCSP.Problem.is_cnf pcsp0 then pcsp0 else
        PCSP.Problem.map_if_raw_old pcsp0 ~f:(fun inp ->
            ...
            |> Set.Poly.filter_map ~f:(fun cl ->
                ...
                ClauseOld.simplify (Map.key_set exi_senv)
                  (Z3Smt.Z3interface.is_valid ~id:!id (LogicOld.get_fenv ())) cl >>= fun cl ->
                let senv, phi = ClauseOld.to_formula @@ ClauseOld.reduce_sort_map cl in
                Some (Map.Poly.map ~f:Logic.ExtTerm.to_old_sort senv, phi)))
      in
      if Map.Poly.equal Stdlib.(=) (PCSP.Problem.senv_of pcsp0) (PCSP.Problem.senv_of pcsp1)
      then pcsp1
      else begin
