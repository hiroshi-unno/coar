...
let state_of param labels : state =
  (param.nd, param.nc, param.depth, param.ext, Option.map ~f:Z.to_int param.ubec, Option.map ~f:Z.to_int param.ubed, Option.map ~f:Z.to_int param.ubcc, Option.map ~f:Z.to_int param.ubcd,
...
module Make (Cfg : Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let gen_template ~tag hspace =
    ...
    let temp_params, hole_qualifiers_map, tmpl, constr_of_expr_dt_bound,
      ...
      Generator.gen_int_bool_dt_fun
        ...
        (List.init !param.nd ~f:(fun _ -> !param.nc), !param.depth, !param.ext,
  ...
  let init_expr (param, actions) =
    Debug.print @@ lazy ("************* initializing number_of_expr of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let decrease_cond_conj (param, actions) =
    ...
    { param with nc = max (param.nc - 1) 0 }, actions @ ["decrease_cond_conj"]
  let init_cond_conj (param, actions) =
    Debug.print @@ lazy ("************* initializing number_of_cond_conj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_expr_cond_conj (param, actions) =
    if
      ...
      && param.nd * param.nc > (param.depth + 1) * (param.depth + 1) * (param.depth + 1)
    then increase_depth_ext (param, actions)
    else if (param.nd + param.nc) mod 2 = 0 || (match config.max_number_of_cond_conj with None -> false | Some max -> param.nc >= max)
