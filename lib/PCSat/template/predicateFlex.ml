...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid PredicateFlex Configuration (%s): %s" filename msg
end
...
let state_of param labels : state =
  (param.shp, param.depth, Option.map ~f:Z.to_int param.ubc, Option.map ~f:Z.to_int param.ubd,
...
module Make (Cfg : Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let str_of () =
    sprintf
      ("shape : [%s]\n" ^^
       ...
       "seeds : %s")
      (String.concat_map_list ~sep:";" !param.shp ~f:string_of_int) !param.depth
  ...
  let adjust_quals ~tag quals = ignore tag; quals
  let init_quals _ _ = ()
  let update_hspace ~tag hspace =
    ignore tag; Qualifier.AllTheory.qualifiers_of ~fenv:Arg.fenv ~add_mod2_quals:true !param.depth hspace
  let gen_template ~tag hspace =
    ...
    let temp_params, hole_qualifiers_map, tmpl,
      ...
      Generator.gen_dnf
        ...
        (List.map ~f:(fun t -> t, Term.sort_of t(*ToDo*)) @@ Set.to_list hspace.terms)
    ...
    Debug.print @@ lazy
      (sprintf "[%s] cnstr_of_coeffs:\n  %s"
         (Ident.name_of_tvar Arg.name) (Formula.str_of cnstr_of_coeffs));
    Debug.print @@ lazy
      (sprintf "[%s] cnstr_of_consts:\n  %s"
         (Ident.name_of_tvar Arg.name) (Formula.str_of cnstr_of_consts));
    Debug.print @@ lazy
      (sprintf "[%s] cnstr_of_qualifiers:\n  %s"
         (Ident.name_of_tvar Arg.name) (Formula.str_of cnstr_of_quals));
    (*Debug.print @@ lazy
  ...
  let decrease_disj (param, actions) =
    Debug.print @@ lazy ("************* decreasing number_of_disj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with shp = match param.shp with [] -> [] | _ :: shp -> shp }, actions @ ["decrease_disj"]
  let increase_conj disj_idx (param, actions) =
    Debug.print @@ lazy ("************* increasing number_of_conj of " ^ Ordinal.string_of (Ordinal.make disj_idx) ^ " disjunct of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with shp = List.mapi param.shp ~f:(fun i nc -> if i = disj_idx then nc + 1 else nc) },
    actions @ ["increase_conj@" ^ string_of_int disj_idx]
  let decrease_conj disj_idx (param, actions) =
    Debug.print @@ lazy ("************* decreasing number_of_conj of " ^ Ordinal.string_of (Ordinal.make disj_idx) ^ " disjunct of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with shp = List.mapi param.shp ~f:(fun i nc -> if i = disj_idx then max (nc - 1) 0 else nc) },
  ...
  let decrease_coeff (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubc = Option.map param.ubc ~f:(fun ubc -> Z.(max (ubc - one) zero)) },
    actions @ ["decrease_coeff"]
  let init_coeff (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let decrease_const (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubd = Option.map param.ubd ~f:(fun ubd -> Z.(max (ubd - one) zero)) },
    actions @ ["decrease_const"]
  let init_const (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_depth_disj_conj (param, actions) =
    if
      ...
      && List.fold param.shp ~init:0 ~f:(+) > (param.depth + 1) * (param.depth + 1) * (param.depth + 1)
    then begin
      Debug.print @@ lazy ("************* initializing shape of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      Debug.print @@ lazy ("************* increasing depth of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      { param with shp = init.shp; depth = param.depth + 1 },
      actions @ ["init_shape"; "increase_depth"]
    end else if config.disj_first then
      (if Random.bool ()(*ToDo*)
       then increase_disj (param, actions)
       else increase_conj (Random.int (List.length param.shp)(*ToDo*)) (param, actions))
    else
      (if Random.bool ()(*ToDo*)
       then increase_conj (Random.int (List.length param.shp)(*ToDo*)) (param, actions)
       else increase_disj (param, actions))

  let rec inner param_actions = function
    | [] -> param_actions
    | DepthDisjConj :: labels ->
      inner (if config.fix_shape then param_actions else increase_depth_disj_conj param_actions) labels
    | Coeff :: labels ->
    ...
    | Qual :: labels ->
      inner param_actions(*ToDo*) labels
    | QDep :: labels ->
  ...
  let rl_action labels =
    ...
    let rec action param_actions =
      ...
      | s ->
        ...
        | Some res ->
          action (increase_conj (int_of_string res) param_actions)
        | None ->
