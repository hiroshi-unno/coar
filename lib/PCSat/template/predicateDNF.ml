...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid PredicateDNF Configuration (%s): %s" filename msg
end
...
let state_of param labels : state =
  (param.nd, param.nc, param.depth, Option.map ~f:Z.to_int param.ubc, Option.map ~f:Z.to_int param.ubd,
...
module Make (Cfg : Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let str_of () =
    sprintf
      ("number of disjuncts (max: %s) : %d\n" ^^
       ...
       "seeds : %s")
      (SolSpace.str_of_tvar_and_flag Arg.name SolSpace.ND Arg.sol_space) !param.nd
      (SolSpace.str_of_tvar_and_flag Arg.name SolSpace.NC Arg.sol_space) !param.nc
      (SolSpace.str_of_tvar_and_flag Arg.name SolSpace.Depth Arg.sol_space) !param.depth
  ...
  let gen_template ~tag hspace =
    ...
    let temp_params, hole_qualifiers_map, tmpl,
      ...
      Generator.gen_dnf
        ...
        (List.init !param.nd ~f:(fun _ -> !param.nc), !param.ubc, !param.ubd, !param.s)
  ...
  let init_disj (param, actions) =
    Debug.print @@ lazy ("************* initializing number_of_disj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let decrease_conj (param, actions) =
    Debug.print @@ lazy ("************* decreasing number_of_conj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with nc = max (param.nc - 1) 0 }, actions @ ["decrease_conj"]
  let init_conj (param, actions) =
    Debug.print @@ lazy ("************* initializing number_of_conj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_depth_disj_conj (param, actions) =
    ...
    then begin
      ...
      Debug.print @@ lazy ("************* initializing number_of_conj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      Debug.print @@ lazy ("************* increasing depth of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      { param with nd = init.nd; nc = init.nc; depth = param.depth + 1 },
      actions @ ["init_disj"; "init_const"; "increase_depth"]
    end else if config.disj_first then
      (if
        ((param.nd + param.nc) - 2) mod config.incr_interval < config.incr_interval - 1
