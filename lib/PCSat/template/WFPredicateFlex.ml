...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Ok x -> instantiate_ext_files x >>= fun x ->
        Ok (ExtFile.Instance x)
      | Error msg ->
        error_string @@ sprintf "Invalid WFPredicateFlex Configuration (%s): %s" filename msg
end
...
let state_of param labels : state =
  (param.shp, param.nl, param.depth, Option.map ~f:Z.to_int param.ubrc, Option.map ~f:Z.to_int param.ubrd, Option.map ~f:Z.to_int param.ubdc, Option.map ~f:Z.to_int param.ubdd,
...
module Make (Cfg : Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let update_hspace ~tag hspace =
    ignore tag; Qualifier.AllTheory.qualifiers_of ~fenv:Arg.fenv !param.depth hspace
  let gen_template ~tag hspace =
    ...
    Debug.print @@ lazy
      (sprintf "[%s] cnstr_of_rfun_coeffs:\n  %s"
         (Ident.name_of_tvar @@ Arg.name) (Formula.str_of cnstr_of_rfun_coeffs));
    Debug.print @@ lazy
      (sprintf "[%s] cnstr_of_rfun_const:\n  %s"
         (Ident.name_of_tvar @@ Arg.name) (Formula.str_of cnstr_of_rfun_const));
    Debug.print @@ lazy
      (sprintf "[%s] cnstr_of_disc_coeffs:\n  %s"
         (Ident.name_of_tvar @@ Arg.name) (Formula.str_of cnstr_of_disc_coeffs));
    Debug.print @@ lazy
      (sprintf "[%s] cnstr_of_disc_const:\n  %s"
         (Ident.name_of_tvar @@ Arg.name) (Formula.str_of cnstr_of_disc_const));
    let tmpl =
      Logic.(Term.mk_lambda (of_old_sort_env_list ExtTerm.of_old_sort hspace.params)) @@
  ...
  let increase_disc_conj piece_idx (param, actions) =
    ...
    { param with shp = List.mapi param.shp ~f:(fun i ndc -> if i = piece_idx then ndc + 1 else ndc) },
  ...
  let decrease_disc_conj piece_idx (param, actions) =
    ...
    { param with shp = List.mapi param.shp ~f:(fun i ndc -> if i = piece_idx then max (ndc - 1) 0 else ndc) },
    actions @ ["decrease_disc_conj@" ^ string_of_int piece_idx]
  let init_shape (param, actions) =
    Debug.print @@ lazy ("************* initializing shape of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_lexico_piece_conj (param, actions) =
    if param.nl >= List.length param.shp then begin
      Debug.print @@ lazy ("************* initializing number_of_rfun_lexicos of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      Debug.print @@ lazy ("************* increasing number_of_rfun_pieces of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      Debug.print @@ lazy ("************* increasing number_of_disc_conj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      Debug.print @@ lazy ("************* increasing depth of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      { param with
        shp = List.length param.shp :: List.map param.shp ~f:(fun ndc -> ndc + 1);(*ToDo*)
        nl = init.nl; depth = param.depth + 1 },
      actions @
      List.mapi param.shp ~f:(fun i _ -> "increase_disc_conj@" ^ string_of_int i) @
      ["increase_rfun_pieces"] @
      List.map param.shp ~f:(fun _ -> "increase_disc_conj@0") @
      ["init_rfun_lexicos"; "increase_depth"]
