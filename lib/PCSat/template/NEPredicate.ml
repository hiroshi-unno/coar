...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid NEPredicate Configuration (%s): %s" filename msg
end
...
module Make (Cfg : Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let str_of () =
    sprintf "depth: %d\next: %d\nubc: %d\nubd: %d" !param.depth !param.ext !param.ubc !param.ubd
  ...
  let gen_template ~tag hspace =
    ...
    let temp_params, tmpl, ne_constr, scope_constr, cnstr_of_coeffs, cnstr_of_consts =
      Generator.gen_ne_template
        (Set.to_list quals)
        (Set.to_list hspace.terms)
        (!param.depth, !param.ext, !param.ubc, !param.ubd) params
    in
  ...
  let decrease_coeff (param, actions) =
    Debug.print @@ lazy ("************* decreasing coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubc = max (param.ubc - 1) 0 }, actions @ ["decrease_coeff"]
  let init_coeff (param, actions) =
    Debug.print @@ lazy ("************* initializing coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let decrease_const (param, actions) =
    Debug.print @@ lazy ("************* decreasing const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubd = max (param.ubd - 1) 0 }, actions @ ["decrease_const"]
  let init_const (param, actions) =
    Debug.print @@ lazy ("************* initializing const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
