...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid IntFunction Configuration (%s): %s" filename msg
end
...
let state_of param labels : state =
  (param.shp, param.depth, param.ext, Option.map ~f:Z.to_int param.ubec, Option.map ~f:Z.to_int param.ubed, Option.map ~f:Z.to_int param.ubcc, Option.map ~f:Z.to_int param.ubcd,
...
module Make (Cfg : Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let param =
    ref { shp = config.shape;
          ...
          es = (match config.expr_seeds with
          ...
          cs = (match config.cond_seeds with
              | None -> Set.Poly.singleton Z.zero
              | Some ds -> Set.Poly.of_list @@ List.map ds ~f:Z.of_int) }
  let init =
  ...
  let adjust_quals ~tag quals =
    ...
    Set.Poly.filter_map quals ~f:(fun phi ->
        ...
        | Formula.Atom (Atom.App (Predicate.Psym (T_bool.Eq | T_int.Geq), [t1; t2], _), _) ->
        ...
        | phi ->
          ...
          in
          if Formula.is_bind qual || Set.is_empty (Formula.fvs_of qual) then None else Some qual)
  let init_quals _ _ = ()
  ...
  let gen_template ~tag hspace =
    ...
    let temp_params, hole_qualifiers_map, tmpl, constr_of_expr_dt_bound,
      ...
      Generator.gen_int_bool_dt_fun
        ...
        (!param.shp, !param.depth, !param.ext,
         !param.ubec, !param.ubed, !param.es, !param.ubcc, !param.ubcd, !param.cs)
    ...
    Debug.print @@ lazy
      (sprintf "[%s] function template:\n  %s"
         (Ident.name_of_tvar @@ Arg.name) (Term.str_of tmpl));
    Debug.print @@ lazy
    ...
    [ExprCoeff, Logic.ExtTerm.of_old_formula cnstr_of_expr_coeffs;
     ...
     ExprCondConjDepthExt, Logic.ExtTerm.of_old_formula constr_of_expr_dt_bound],
  ...
  let decrease_expr (param, actions) =
    Debug.print @@ lazy ("************* decreasing number_of_expr of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with shp = match param.shp with [] -> [] | _ :: shp -> shp }, actions @ ["decrease_expr"]
  let increase_cond_conj expr_idx (param, actions) =
    ...
    { param with shp = List.mapi param.shp ~f:(fun i nc -> if i = expr_idx then nc + 1 else nc) }, actions @ ["increase_cond_conj"]
  let decrease_cond_conj expr_idx (param, actions) =
    Debug.print @@ lazy ("************* decreasing number_of_cond_conj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with shp = List.mapi param.shp ~f:(fun i nc -> if i = expr_idx then max (nc - 1) 0 else nc) }, actions @ ["decrease_cond_conj"]
  ...
  let decrease_ext (param, actions) =
    Debug.print @@ lazy ("************* decreasing ext of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ext = max (param.ext - 1) 0 }, actions @ ["decrease_ext"]
  let init_ext (param, actions) =
    Debug.print @@ lazy ("************* initializing ext of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_expr_coeff threshold (param, actions) =
    ...
    | _, _ ->
      ...
      { param with ubec = Option.map param.ubec ~f:(fun ubec -> Z.(ubec + one)) },
      actions @ ["increase_expr_coeff"]
  let decrease_expr_coeff (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_expr_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubec = Option.map param.ubec ~f:(fun ubec -> Z.(max (ubec - one) zero)) },
    actions @ ["decrease_expr_coeff"]
  let init_expr_coeff (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_expr_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_expr_const threshold (param, actions) =
    ...
    | _, _ ->
      ...
      { param with ubed = Option.map param.ubed ~f:(fun ubed -> Z.(ubed + one)) },
      actions @ ["increase_expr_const"]
  let decrease_expr_const (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_expr_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubed = Option.map param.ubed ~f:(fun ubed -> Z.(max (ubed - one) zero)) },
    actions @ ["decrease_expr_const"]
  let init_expr_const (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_expr_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_cond_coeff threshold (param, actions) =
    ...
    | _, _ ->
      ...
      { param with ubcc = Option.map param.ubcc ~f:(fun ubcc -> Z.(ubcc + one)) },
      actions @ ["increase_cond_coeff"]
  let decrease_cond_coeff (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_cond_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubcc = Option.map param.ubcc ~f:(fun ubcc -> Z.(max (ubcc - one) zero)) },
    actions @ ["decrease_cond_coeff"]
  let init_cond_coeff (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_cond_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_cond_const threshold (param, actions) =
    ...
    | _, _ ->
      ...
      { param with ubcd = Option.map param.ubcd ~f:(fun ubcd -> Z.(ubcd + one)) },
      actions @ ["increase_cond_const"]
  let decrease_cond_const (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_cond_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubcd = Option.map param.ubcd ~f:(fun ubcd -> Z.(max (ubcd - one) zero)) },
    actions @ ["decrease_cond_const"]
  let init_cond_const (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_cond_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubcd = init.ubcd }, actions @ ["init_cond_const"]
  let increase_depth_ext (param, actions) =
    if (param.depth + param.ext) mod 2 = 0
    then increase_depth (param, actions)
  ...
  let increase_expr_cond_conj (param, actions) =
    if
      ...
      && List.fold param.shp ~init:1 ~f:( * ) > (param.depth + 1) * (param.depth + 1) * (param.depth + 1)
    then increase_depth_ext (param, actions)
    else
      match
        match config.max_number_of_cond_conj with
        | None -> Some (Random.int (List.length param.shp))
        | Some max -> List.find_mapi param.shp ~f:(fun idx nc -> if nc >= max then None else Some idx)
      with
      | None -> increase_expr (param, actions)
      | Some idx ->
        if (*ToDo*)(List.length param.shp + List.sum (module Int) param.shp ~f:Fn.id) mod 2 = 0
  ...
  let rec inner param_actions = function
    ...
    | ExprCondConjDepthExt :: labels ->
      inner (if config.fix_shape then increase_depth_ext param_actions else increase_expr_cond_conj param_actions) labels
    | ExprCoeff :: labels ->
      inner (increase_expr_coeff config.threshold_expr_coeff param_actions) labels
    | ExprConst :: labels ->
      inner (increase_expr_const config.threshold_expr_const param_actions) labels
    | CondCoeff :: labels ->
      inner (increase_cond_coeff config.threshold_cond_coeff param_actions) labels
    | CondConst :: labels ->
      inner (increase_cond_const config.threshold_cond_const param_actions) labels
    | QDep :: labels ->
      inner param_actions labels
