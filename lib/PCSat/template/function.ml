...
module type Type = sig
  ...
  val show_state: ?config:RLConfig.t -> parameter_update_type Set.Poly.t -> unit
  ...
  val gen_template:
    tag:(Ident.tvar * Ident.tvar) option -> hspace ->
    (parameter_update_type * Logic.term) * (parameter_update_type * Logic.term) list * Logic.sort_env_map * (Ident.tvar * (Ident.tvar * (LogicOld.sort_env_list * LogicOld.Formula.t)) list) list
...
let qdep_constr_of (qdeps : (LogicOld.Formula.t , QDep.t) Map.Poly.t) env =
  QDep,
  Map.map qdeps ~f:(PCSatCommon.QDep.condition_of env) (* TODO: not empty map *)
  |> Map.Poly.to_alist |> List.unzip |> snd |> LogicOld.Formula.and_of
