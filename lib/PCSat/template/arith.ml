...
let cast = function
  ...
  | sort1, sort2 ->
    failwith ("not supported: " ^ Term.str_of_sort sort1 ^ ", " ^
              Term.str_of_sort sort2)

let sum_of sort term ts =
  let coeffs, terms =
    List.unzip @@ List.map ts ~f:(fun (t, s) ->
...
let atom_of ?(eq=false) ts =
  ...
  let num_ts = List.filter ts ~f:(function _, T_int.SInt | _ , T_real.SReal -> true | _, _ -> false) in
  num_ts, if eq then eq_of sort num_ts else ineq_of sort num_ts

let conj_with_bool_of ts =
  let sort = sort_of ts in
  let bool_ts = List.filter ts ~f:(function _, T_bool.SBool -> true | _, _ -> false) in
  let num_ts = List.filter ts ~f:(function _, T_int.SInt | _ , T_real.SReal -> true | _, _ -> false) in
  let rec aux = function
    | [] -> let params, templ = ineq_of sort num_ts in [params], Formula.mk_atom templ
    | (t, _) :: ts ->
