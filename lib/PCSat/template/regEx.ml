...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid RegEx Configuration (%s): %s" filename msg
end
...
module Make (Cfg : Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let gen_template ~tag hspace =
    ...
    let ts = List.map (Set.to_list hspace.consts) ~f:T_regex.mk_str_to_re in
    let tmpl =
      let switch ts =
        let x = Ident.mk_fresh_parameter () in
        let xt = Term.mk_var x T_int.SInt in
        temp_params := Map.Poly.add_exn !temp_params ~key:x ~data:Logic.ExtTerm.SInt;
        cnstr_of_temp_params :=
          Formula.and_of @@ !cnstr_of_temp_params ::
      ...
      let rec aux d =
        if d <= 0 then
          switch @@
          T_regex.mk_empty () :: T_regex.mk_full () :: T_regex.mk_eps () :: ts
        else
          switch @@
          T_regex.mk_empty () :: T_regex.mk_full () :: T_regex.mk_eps () :: ts @
          [T_regex.mk_complement (aux (d - 1)); T_regex.mk_star (aux (d - 1));
           T_regex.mk_plus (aux (d - 1)); T_regex.mk_opt (aux (d - 1));
    ...
    let tmpl =
      if false then tmpl
        ...
        List.fold (Term.elim_ite tmpl) ~init:(T_regex.mk_empty ())
          ~f:(fun acc (cond, t) -> T_bool.ifte cond t acc) in
    Debug.print @@ lazy
    ...
    in
    (Depth, tmpl), [Dummy, Logic.ExtTerm.of_old_formula !cnstr_of_temp_params],
  ...
  let rec inner param_actions = function
    | [] -> param_actions
    | Depth :: labels ->
      inner (increase_depth param_actions) labels
    | (Dummy | QDep) :: labels ->
