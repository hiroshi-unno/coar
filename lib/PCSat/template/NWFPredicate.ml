...
let state_of param labels : state =
  (param.np, param.ndc, param.nl, param.depth, Option.map ~f:Z.to_int param.ubrc, Option.map ~f:Z.to_int param.ubrd, Option.map ~f:Z.to_int param.ubdc, Option.map ~f:Z.to_int param.ubdd,
...
module Make (Cfg : WFPredicate.Config.ConfigType) (Arg : ArgType) : Function.Type = struct
  ...
  let param =
    ref { np = config.number_of_rfun_pieces;
          ...
          ds = match config.disc_seeds with
  ...
  let adjust_quals ~tag quals =
    ...
    if Stdlib.(idxl <> idxr) then quals
      ...
      let params_x, params_y = params_of_idx_l idxl, params_of_idx_r idxl idxr in
      ...
      Set.concat_map quals ~f:(fun phi ->
          let qual1 = Z3Smt.Z3interface.qelim ~id Arg.fenv @@ Formula.exists params_y phi in
          let qual2 = Z3Smt.Z3interface.qelim ~id Arg.fenv @@ Formula.exists params_x phi in
          Set.union
            (if Formula.is_bind qual1 || Set.is_empty (Formula.fvs_of qual1)
             then Set.Poly.empty else Set.Poly.of_list [qual1; rename_x_y qual1])
            (if Formula.is_bind qual2 || Set.is_empty (Formula.fvs_of qual2)
             then Set.Poly.empty else Set.Poly.of_list [rename_y_x qual2; qual2]))

  let rcs, dcs, qds, qualmapl, qualmapr =
    let init_qualmap () = Hashtbl.Poly.map Arg.tag_infos ~f:(fun _ -> Set.Poly.empty) in
    Hashtbl.Poly.(create (), create (), create (), init_qualmap (), init_qualmap ())
  let reset_paramvars () = Hashtbl.Poly.(clear rcs; clear dcs; clear qds)
  ...
  let rec init_quals tag quals =
    ...
    let subst_y =
      Map.Poly.of_alist_exn @@ List.mapi (params @ params_y) ~f:(fun i (x, _) -> x, x_i i)
    in
    ...
    Hashtbl.Poly.update qualmapl idxl
      ~f:(function Some quals -> Set.union quals qualsl | None -> qualsl);
    Hashtbl.Poly.update qualmapr idxr
      ~f:(function Some quals -> Set.union quals qualsr | None -> qualsr)

  let update_hspace ~tag hspace =
    ignore tag;
    let qualsl = Hashtbl.Poly.find_exn qualmapl @@ fst @@ Option.value_exn tag in
    let qualsr =
    ...
    in
    let hspace = { hspace with quals = if true(*ToDo*) then Set.Poly.empty else hspace.quals } in
    let hspace = Qualifier.AllTheory.qualifiers_of ~fenv:Arg.fenv !param.depth hspace in
    { hspace with quals = Set.Poly.union_list [hspace.quals; qualsl; qualsr] }

  let gen_template ~tag hspace =
    let idxl, idxr = Option.value_exn tag in
    let real_name = Ident.mk_nwf_tvar Arg.name idxl idxr in
    let temp_params, hole_qualifiers_map, tmpl,
        cnstr_of_rfun_coeffs, cnstr_of_rfun_const,
        cnstr_of_disc_coeffs, cnstr_of_disc_const =
      Generator.gen_simplified_nwf_predicate
        (*config.use_ifte*)
        (Set.to_list hspace.quals)
        (Set.to_list hspace.terms)
        (rcs, dcs, qds)
        (List.init !param.np ~f:(fun _ -> !param.ndc), !param.nl,
         !param.ubrc, !param.ubrd, !param.ubdc, !param.ubdd, !param.ds)
        (config.norm_type,
        ...
        (LogicOld.sort_env_list_of_sorts ~pre:"" Arg.sorts,
         ...
         idxr, params_of_idx_r idxl idxr)
    in
    Debug.print @@ lazy
  ...
  let decrease_rfun_pieces (param, actions) =
    Debug.print @@ lazy ("************* decreasing number_of_rfun_pieces of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with np = max (param.np - 1) 0 }, actions @ ["decrease_rfun_pieces"]
  let init_rfun_pieces (param, actions) =
    Debug.print @@ lazy ("************* initializing number_of_rfun_pieces of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let decrease_disc_conj (param, actions) =
    Debug.print @@ lazy ("************* decreasing number_of_disc_conj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ndc = max (param.ndc - 1) 0 }, actions @ ["decrease_disc_conj"]
  let init_disc_conj (param, actions) =
    Debug.print @@ lazy ("************* initializing number_of_disc_conj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let decrease_rfun_lexicos (param, actions) =
    Debug.print @@ lazy ("************* decreasing number_of_rfun_lexicos of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with nl = max (param.nl - 1) 0 }, actions @ ["decrease_rfun_lexicos"]
  let init_rfun_lexicos (param, actions) =
    Debug.print @@ lazy ("************* initializing number_of_rfun_lexicos of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let decrease_depth (param, actions) =
    Debug.print @@ lazy ("************* decreasing depth of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with depth = max (param.depth - 1) 0 }, actions @ ["decrease_depth"]
  let init_depth (param, actions) =
    Debug.print @@ lazy ("************* initializing depth of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_rfun_coeff threshold (param, actions) =
    ...
    | _, _ ->
      ...
      { param with ubrc = Option.map param.ubrc ~f:(fun ubrc -> Z.(ubrc + one)) },
      actions @ ["increase_rfun_coeff"]
  let decrease_rfun_coeff (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_rfun_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubrc = Option.map param.ubrc ~f:(fun ubrc -> Z.(max (ubrc - one) zero)) },
    actions @ ["decrease_rfun_coeff"]
  let init_rfun_coeff (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_rfun_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_rfun_const threshold (param, actions) =
    ...
    | _, _ ->
      ...
      { param with ubrd = Option.map param.ubrd ~f:(fun ubrd -> Z.(ubrd + one)) },
      actions @ ["increase_rfun_const"]
  let decrease_rfun_const (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_rfun_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubrd = Option.map param.ubrd ~f:(fun ubrd -> Z.(max (ubrd - one) zero)) },
    actions @ ["decrease_rfun_const"]
  let init_rfun_const (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_rfun_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_disc_coeff threshold (param, actions) =
    ...
    | _, _ ->
      ...
      { param with ubdc = Option.map param.ubdc ~f:(fun ubdc -> Z.(ubdc + one)) },
      actions @ ["increase_disc_coeff"]
  let decrease_disc_coeff (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_disc_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubdc = Option.map param.ubdc ~f:(fun ubdc -> Z.(max (ubdc - one) zero)) },
    actions @ ["decrease_disc_coeff"]
  let init_disc_coeff (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_disc_coeff of " ^ Ident.name_of_tvar Arg.name ^ "***************");
  ...
  let increase_disc_const threshold (param, actions) =
    ...
    | _, _ ->
      ...
      { param with ubdd = Option.map param.ubdd ~f:(fun ubdd -> Z.(ubdd + one)) },
      actions @ ["increase_disc_const"]
  let decrease_disc_const (param, actions) =
    Debug.print @@ lazy ("************* decreasing upper_bound_disc_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubdd = Option.map param.ubdd ~f:(fun ubdd -> Z.(max (ubdd - one) zero)) },
    actions @ ["decrease_disc_const"]
  let init_disc_const (param, actions) =
    Debug.print @@ lazy ("************* initializing upper_bound_disc_const of " ^ Ident.name_of_tvar Arg.name ^ "***************");
    { param with ubdd = init.ubdd }, actions @ ["init_disc_const"]
  let increase_lexico_piece_conj (param, actions) =
    if param.nl >= param.np then begin
      Debug.print @@ lazy ("************* initializing number_of_rfun_lexicos of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      Debug.print @@ lazy ("************* increasing number_of_rfun_pieces of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      Debug.print @@ lazy ("************* increasing number_of_disc_conj of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      Debug.print @@ lazy ("************* increasing depth of " ^ Ident.name_of_tvar Arg.name ^ "***************");
      { param with
        np = param.np + 1; ndc = param.ndc + 1;
        nl = init.nl; depth = param.depth + 1 },
      actions @ ["increase_rfun_pieces"; "increase_disc_conj"; "init_rfun_lexicos"; "increase_depth"]
    end else increase_rfun_lexicos (param, actions)

  let rec inner param_actions = function
    | [] -> param_actions
    | LexicoPieceConj :: labels ->
      inner (if config.fix_shape then param_actions else increase_lexico_piece_conj param_actions) labels
    | RFunCoeff :: labels ->
      inner (increase_rfun_coeff config.threshold_rfun_coeff param_actions) labels
    | RFunConst :: labels ->
      inner (increase_rfun_const config.threshold_rfun_const param_actions) labels
    | DiscCoeff :: labels ->
      inner (increase_disc_coeff config.threshold_disc_coeff param_actions) labels
    | DiscConst :: labels ->
      inner (increase_disc_const config.threshold_disc_const param_actions) labels
  ...
  let rl_action labels =
    if not @@ Set.mem labels TimeOut then
...
let make ~(id:int option) ~(name:Ident.tvar) config arg =
  ...
  | None ->
    ...
    |> (fun m -> Hashtbl.Poly.add_exn modules ~key:(id, name) ~data:m; m)
