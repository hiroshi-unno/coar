...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid Labeler Configuration (%s): %s" filename msg
  ...
  let rec str_of_strategy = function
    | SAT -> "SAT solving"
    | ORACLE_SAT s ->
      sprintf "Oracle SAT solving [%s]" @@ str_of_strategy s
    | POS_BIASED_SAT -> "Positively biased SAT solving"
    | NEG_BIASED_SAT -> "Negatively biased SAT solving"
    | RAND_MAX_SAT (cut_off_lb, cut_off_ub) ->
      sprintf "Randomly weighted MAX-SAT solving (%f, %f)" cut_off_lb cut_off_ub
    | BMI_MAX_SAT (num, bound, ratio, perturbation, cut_off_lb, cut_off_ub, enc_bool_to_float, ignore_bool_args_variance) ->
      sprintf "BMI weighted MAX-SAT solving (%d, %d, %f, %s, %f, %f, %f, %b)"
        num bound ratio (str_of_perturbation perturbation) cut_off_lb cut_off_ub
        enc_bool_to_float ignore_bool_args_variance
    | ORACLE_MAX_SAT s ->
      sprintf "Oracle MAX-SAT solving [%s]" (str_of_strategy s)
    | POS_BIASED_MAX_SAT (b, w) ->
      sprintf "Positively biased MAX-SAT solving (%b, %s)" b (string_of_float w)
    | NEG_BIASED_MAX_SAT (b, w) ->
      sprintf "Negatively biased MAX-SAT solving (%b, %s)" b (string_of_float w)
    | Mixed ss ->
      String.bracket @@
...
module Make (RLCfg: RLConfig.ConfigType) (Cfg : Config.ConfigType)
  ...
  let rec learn_clauses_to_remove_nondet_functions pos res = function
    | [] -> res
    | (pvar, sorts) :: fnpvs ->
      let sample = Set.filter pos ~f:(fun ((pvar', _), _) -> Stdlib.(pvar = pvar')) in
      if Set.is_empty sample then learn_clauses_to_remove_nondet_functions pos res fnpvs
      else
        let map =
        ...
        let res' =
          Map.Poly.fold map ~init:res ~f:(fun ~key:args ~data:rets res ->
              Set.fold_distinct_pairs (Set.Poly.of_list rets) ~init:res ~f:(fun acc r1 r2 ->
                  Set.add acc @@
                  ExClause.{
    ...
    | (pvar, sorts) :: wfpvs ->
      ...
      if Set.is_empty sample then learn_clauses_to_remove_cycles pos res wfpvs
        ...
        | Some ((_pvar, sorts), terms) ->
          let res' = Set.add res (ExClause.mk_unit_neg (ExAtom.PApp ((pvar, sorts), terms))) in
  ...
  let rec learn_clauses_to_remove_cycles_for_nwf pos res = function
    ...
    | (pvar, sorts, sorts_l, sorts_r) :: nwfpvs ->
      let sample = Set.filter pos ~f:(fun ((pvar', _), _) -> Stdlib.(pvar = pvar')) in
      if Set.is_empty sample then learn_clauses_to_remove_cycles_for_nwf pos res nwfpvs
      else
        let has_self_loop (_, terms) =
        ...
        | None ->
          ...
          |> (fun (graph, node_map_rev) -> graph, Components.scc_list graph, node_map_rev)
          |> NWFLoopDetector.detect ~print:Debug.print res pvar (sorts, sorts_l, sorts_r)
  ...
  let abstract map cls =
    PropLogic.Formula.and_of @@ Set.to_list @@ Set.Poly.map cls ~f:(abstract_clause map)
  ...
  let mk_labels pos neg und =
    ...
    [Set.Poly.filter_map und ~f:(fun atm ->
         if Set.mem pos atm || Set.mem neg atm then (*ToDo: reachable?*)None
      ...
      | RAND_MAX_SAT (cut_off_lb, _cut_off_ub) ->
        let soft =
          ...
          List.filter_map (Map.Poly.to_alist map) ~f:(fun (atom, prop) ->
              ...
              Debug.print @@ lazy (sprintf "%s has value %f" (ExAtom.str_of atom) heat);
      ...
      | BMI_MAX_SAT (num, bound, ratio, perturbation, cut_off_lb, _cut_off_ub, enc_bool_to_float, ignore_bool_args_variance) ->
        let soft =
          ...
          let exs1 =
            ...
            VersionSpace.examples_of vs |> Set.Poly.map ~f:(fun cl -> cl, dw) |> Set.to_list
          in
          let exs2 =
            let clauses = Set.elements @@ PCSP.Problem.clauses_of APCSP.problem in
            let rand_samp =
              List.init num ~f:(fun _ ->
                  ...
                  let sub = Map.Poly.mapi (Clause.sort_env_of clause) ~f:(fun ~key:_ ~data ->
                      ...
                      | _(*ToDo*) -> Logic.ExtTerm.mk_dummy (Logic.ExtTerm.to_old_sort) data) in
                  Clause.reduce_sort_map @@
                  Clause.subst (PCSP.Problem.senv_of APCSP.problem) sub clause)
            in
            let weight =
              if num = 0 then 0.0
              else ratio *. (float_of_int @@ List.length exs1) /. (float_of_int num)
            in
            let unknowns = Map.key_set @@ PCSP.Problem.senv_of APCSP.problem in
            List.filter_map rand_samp ~f:(fun cl ->
                ...
                match ExClause.make (PCSP.Problem.senv_of APCSP.problem)
                        pos_atms neg_atms phi with
                | None -> None
                | Some cl -> Some (ExClause.normalize_params unknowns cl, weight))
          in
          VersionSpace.update_heat_map ~enc_bool_to_float ~ignore_bool_args_variance
            (PCSP.Problem.senv_of APCSP.problem) (exs1 @ exs2) vs;
          ...
          | Some oracle ->
            let soft =
              List.filter_map (Map.Poly.to_alist map) ~f:(fun (atom, prop) ->
                  ...
                  ExAtom.pvar_of atom >>= fun pvar ->
                  Map.Poly.find (Map.of_set_exn @@ snd oracle) pvar >>= fun pred ->
                  match ExAtom.eval_pred ~id (VersionSpace.fenv_of vs)
                          Map.Poly.empty pred atom with
                  | Some true ->
          ...
          if strengthen_bias then
            Set.fold ~init:weights cls ~f:(fun weights clause ->
                List.map weights ~f:(fun (atm, weight) ->
                    let size = Set.length clause.ExClause.positive + Set.length clause.ExClause.negative in
                    atm,
                    if pos_bias && Set.mem clause.ExClause.positive atm ||
                       not pos_bias && Set.mem clause.ExClause.negative atm then
                      weight +. w /. float_of_int size
                    else weight))
          else weights
        in
        let soft =
    ...
    let rec outer_loop ps_ns_list cls n =
      ...
      let rec inner_loop lcls(* learned clauses *) sample =
        ...
        check_sat_with iters vs wfpvs map
          (Set.Poly.union_list [ lcls; sample; block_prev_assignments ]) config.strategy
        >>= function
        | None ->
          Debug.print @@ lazy
        ...
        | Some ((pos, neg), sample_examples) ->
          ...
          in
          if Set.is_empty lcls' then (*sat*) Ok (Some ((pos, neg, sample_examples), lcls))
      ...
      if n <= 0 then Ok (ps_ns_list, cls)
        ...
        | Some ((pos, neg, sample_examples), lcls) ->
          outer_loop ((pos, neg, sample_examples) :: ps_ns_list) (Set.union cls lcls) (n - 1)
    in
  ...
  let check_sat iters vs () =
    ...
    let pos_atms, neg_atms, undecided_inst =
      ...
      ExClauseSet.exatoms_of_uclauses @@ (*ExClauseSet.normalize @@*)
    ...
    if Fn.non Set.is_empty conflicts then begin
  ...
  let unit_propagation vs () =
    ...
    let dpos, dneg, undecided = VersionSpace.pos_neg_und_examples_of vs in
    let undecided =
      Set.Poly.map ~f:(ExClause.simplify_nepvs (PCSP.Problem.nepvs_of APCSP.problem))
        undecided
    in
    ...
    let res =
      ExClauseSet.unit_propagation
        (Map.key_set @@ PCSP.Problem.senv_of APCSP.problem) dpos dneg undecided
    in
    Debug.print @@ lazy "<<< unit propagation";
    match res with
    | `Unsat conflicts ->
    ...
    | `Result (dpos', dneg', undecided') ->
      Debug.print @@ lazy
        (VersionSpace.str_of_examples @@
         Set.Poly.(map ~f:fst dpos', map ~f:fst dneg', map ~f:fst undecided'));
      let new_examples =
