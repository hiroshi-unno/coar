...
let rec of_dt = function
  ...
  | DecisionTree.Node (q, t1, t2) ->
    ...
    Ok PropLogic.Formula.(mk_or (mk_and atm subtree1) (mk_and (mk_neg atm) subtree2))
  | DecisionTree.Select ((_, _, t)(* the head is the selected branch *) :: _) -> of_dt t
...
let rec concretize quals = function
  ...
  | UnaryOp (Neg, phi, _) ->
    LogicOld.Formula.mk_neg (concretize quals phi)
