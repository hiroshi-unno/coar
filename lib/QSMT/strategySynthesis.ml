...
module PreFormula = struct
  ...
  let rec term_of_formula term_of_atom = function
    ...
    | Bind (bind, (var, sort), f) -> Term.mk_bind (match bind with Forall -> Logic.Forall | Exists -> Logic.Exists) var sort (term_of_formula term_of_atom f)
...
module StrategySynthesis (A : sig
  ...
  and deptree = ForallDep of PreFormula.var * depforest | ExistsDep of PreFormula.var * depforest
  type ssforest = sstree list
  and sstree = ForallSS of PreFormula.var * ssforest | ExistsSS of PreFormula.var * (A.term * ssforest) list (*this list must be nonempty*)
  ...
  and merge_sstree = function
    ...
    | ExistsSS (v, l), ExistsSS (v', l') ->
      ...
      let rec unify_term_forest_list l = function
        | [] -> l
        | (t', ssf')::l' ->
          let lt, lf = List.partition_tf ~f:(fun (t, _) -> A.term_equal t t') l in
          (match lt with
           | [] -> (t', ssf')::unify_term_forest_list l l'
           | [(t, ssf)] -> (t, merge_ssforest ssf ssf')::unify_term_forest_list lf l'
           | _ -> assert false) in
  ...
  let rec winning_formula (position : int list) (s : ssforest) = function
    | Atom a -> Atom a, []
    | And l ->
      let fs, vars = List.mapi ~f:(fun i f -> winning_formula (i::position) s f) l |> List.unzip in
  ...
  let rec css (position : int list) (f : A.atom formula) model model' (s : ssforest) =
    match f with
    ...
    | Bind(Exists, v, f) ->
      ...
      let us, gs = remove_exists v s
                   |> List.mapi ~f:(fun i (t, s') ->
                       let model' = Map.Poly.add_exn model' ~key:tvar ~data:(A.eval_term model' t) in
                       let u, g = css (i::position) f model model' s' in
                       (u, subst_formula tvar t g)) |> List.unzip in
    ...
    | Some model ->
      let rec build_strategy = function
        ...
        | Bind (b, v, f) ->
          ...
          [match b with Forall -> ForallSS (v, s) | Exists -> ExistsSS (v, [(t, s)])], g in
  ...
  let nondet_strategy_synthesis f =
    ...
    | Some _ as s ->
      ...
      let rec loop p s u f nf =
        ...
        | Some _ as u' -> loop (match p with SAT -> UNSAT | UNSAT -> SAT) (merge_ssforest_option u u') s nf f
        | None -> p, Option.value_exn s in
      loop SAT s u f nf
  ...
  let rec loseCHC vars s =
    ...
    | And l ->
      ...
      BoolTerm.or_of fmls, List.concat chc, merge_map_list strategies, List.concat params
    | Or l ->
      let fmls, chc, strategies, params = List.map ~f:(loseCHC vars s) l |> List.unzip4 in
    ...
    | Bind(Exists, v, f) ->
      ...
      | [] -> assert false
      | (_, _, strategy, _)::l' ->
        let newstrategy = List.fold l' ~init:strategy ~f:(fun s (fml, _, s', _) ->
            Map.Poly.merge s s' ~f:(fun ~key:_ -> function `Both ((senv, t), (senv', t')) ->
                assert (Map.Poly.equal Stdlib.(=) senv senv');
                Some (senv, Term.mk_apps (BoolTerm.mk_ite A.sort) [fml; t; t']) | _ -> assert false)) in
...
module LIA = struct
  ...
  let term_of_term (a, b, c) =
    ...
    if Z.Compare.(b = Z.one) then
      Term.mk_apps (IntTerm.mk_add ()) [a'; c']
    else
      Term.mk_apps (IntTerm.mk_add ()) [Term.mk_apps (IntTerm.mk_div ()) [a'; b']; c']
       ...
       | Some (c, rest) ->
         if Z.(Compare.(c = zero)) then
           ...
           Or (list_init_z a ~f:(fun i ->
               ...
               let gt0 = Gt0 Affine.(mult a rest + mult c (t + of_z Z.(a * b - i))) in
  ...
  let rec formula_of_term = function
    ...
    | App(App(TyApp(Con(r, _), BoolTerm.SBool, _), t1, _), t2, _) when (match r with ExtTerm.Eq | ExtTerm.Neq -> true | _ -> false) ->
      Option.(formula_of_term t1 >>= fun a1 ->
              ...
              | ExtTerm.Eq ->
                let tt = And ([a1; a2]) in
                let ff = And ([(neg_formula neg_atom a1); (neg_formula neg_atom a2)]) in
                return (Or [tt; ff])
              | ExtTerm.Neq ->
  ...
  let term_of_atom = function
    | Gt0 a -> Term.mk_apps (ExtTerm.mk_gt ()) [Affine.term_of a; IntTerm.zero ()]
    | Div (d, a) ->
      if Z.(Compare.(d = one)) then
        BoolTerm.mk_bool true
      else
        Term.mk_apps (BoolTerm.mk_eq IntTerm.SInt) [Term.mk_apps (IntTerm.mk_mod ()) [Affine.term_of a; IntTerm.mk_int d]; IntTerm.zero ()]
  let ub model tvar f =
    atoms f
  ...
  let delta model tvar f =
    div model tvar f |> List.map ~f:(fun (d, (c, _)) -> Z.(d /| (Z.gcd (abs c) d))) |> lcm_of_list
  let lub model tvar f =
    let delta = delta model tvar f in
    ub model tvar f |> List.map ~f:(fun (a, t) ->
        let a = Z.(~-a) in
        let ev_tvar = match Map.Poly.find model tvar with Some v -> v | None -> default_value in
        let t_minus_one = Affine.(t - of_z Z.one) in
        let b = Z.(erem (eval_affine model t_minus_one /< a - ev_tvar) delta) in
        assert Z.(Compare.(>=) b zero);
        let term = (t_minus_one, a, Z.(~-b)) in
        term) |> List.min_elt ~compare:(fun t1 t2 -> Z.compare (eval_term model t1) (eval_term model t2))
  let glb model tvar f =
    let delta = delta model tvar f in
    lb model tvar f |> List.map ~f:(fun (a, t) ->
        let t = Affine.(~-t) in
        let ev_tvar = match Map.Poly.find model tvar with Some v -> v | None -> default_value in
        let b = Z.(erem (ev_tvar - eval_affine model t /< a - one) delta) in
        assert Z.(Compare.(>=) b zero);
        let term = (t, a, Z.(b + one)) in
        term) |> List.max_elt ~compare:(fun t1 t2 -> Z.compare (eval_term model t1) (eval_term model t2))
  ...
  let check_sat f senv = 
    ...
    | Some model -> 
      ...
      let remove_dontcare (tvar, v) =
        match v with
        | None -> (tvar, Z.zero)
        | Some (term) -> (tvar, let_int term) in
      let model = List.map ~f:remove_dontcare model |> Map.Poly.of_alist_exn in
      Some model
end
...
module LRA = struct
      ...
      (match find_var_term tvar s with
       ...
       | Some (c, rest) ->
         if Q.(c = zero) then
           ...
           Atom (Eq0 (RTerm.(rest + RTerm.mult c t))))
    | Gt0 s ->
      (match find_var_term tvar s with
       | None -> Atom (Gt0 s)
       | Some (c, rest) ->
         if Q.(c = zero) then
           ...
           Atom (Gt0 (RTerm.(rest + RTerm.mult c t))))
  let eval_term m t =
    Map.Poly.fold t ~init:Q.zero ~f:(fun ~key ~data sum ->
        match key with
        | None -> Q.(sum + data)
        | Some tvar -> let v = match Map.Poly.find m tvar with Some v -> v | None -> default_value in Q.(sum + data * v))
  let rename_var_term v v' t =
    match Map.Poly.find t (Some v) with
    | None -> t
    | Some c -> Map.Poly.remove t (Some v) |> Map.Poly.add_exn ~key:(Some v') ~data:c
  ...
  let term_of_atom = function
    | Eq0 t -> Term.mk_apps (ExtTerm.mk_eq RealTerm.SReal) [RTerm.term_of t; RealTerm.rzero ()]
    | Gt0 t -> Term.mk_apps (ExtTerm.mk_gt ()) [RTerm.term_of t; RealTerm.rzero ()]
      ...
      (match find_var_term tvar t with
       ...
       | Some (c, rest) ->
         if Q.(c = zero) then None
           ...
           if Q.((Map.Poly.find_exn model tvar) = (eval_term model s)) then Some s else None)
    | Gt0 _ -> None
      ...
      (match find_var_term tvar t with
       ...
       | Some (c, rest) ->
         if Q.(c >= zero) then Set.Poly.empty
           ...
           let s = RTerm.div rest Q.(-c) in
           if Q.((Map.Poly.find_exn model tvar) < (eval_term model s)) then Set.Poly.singleton s else Set.Poly.empty)
  ...
  let select model tvar f =
    ...
    | Some t -> t
    | None ->
      let (ub, lb) = ub_and_lb model tvar f in begin
  ...
  let check_sat f senv = 
    ...
    | Some model -> 
      ...
      let remove_dontcare (tvar, v) =
        match v with
        | None -> (tvar, Q.zero)
        | Some (term) -> (tvar, let_real term) in
      let model = List.map ~f:remove_dontcare model |> Map.Poly.of_alist_exn in
      Some model
end
  ...
  let check_sat f senv = 
    ...
    | Some model -> 
      ...
      let remove_dontcare (tvar, v) =
        match v with
        | None -> (tvar, true)
        | Some (term) -> (tvar, let_bool term) in
      let model = List.map ~f:remove_dontcare model |> Map.Poly.of_alist_exn in
      Some model
end
