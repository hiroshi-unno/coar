...
module Make (Config: Config.ConfigType): SolverType = struct
  ...
  let pvar_eliminator = let open Or_error in
    ExtFile.unwrap config.pvar_eliminator >>= fun cfg -> Ok (PCSat.PvarEliminator.make cfg)
  ...
  let print_pcsp pcsp =
    ...
    pvar_eliminator >>= fun (module PvarEliminator) ->
    let _, pcsp = PvarEliminator.preprocess ~bpvs:Set.Poly.empty pcsp in
    return @@ print_endline @@
    (match config.smt_format with SMT2 -> Smtlib2.of_pcsp | String -> PCSP.Problem.str_of) @@
    pcsp
