...
module type TermType = sig
  ...
  val con_of: (Logic.term -> Z3.Expr.expr -> Logic.term) -> Z3.Expr.expr -> Logic.term option
end
...
module IntTerm : TermType = struct
  ...
  let nullary_con_of expr =
    if Arithmetic.is_int_numeral expr then
      mk_int (Arithmetic.Integer.get_big_int expr)
      |> Option.some
    else None
  ...
  let con_of mk_op_app expr =
    let term =
      ...
      else if Arithmetic.is_div expr || Arithmetic.is_idiv expr then mk_op_app (mk_div ()) expr
      else assert false
    in Option.some term
end
...
module RealTerm : TermType = struct
  ...
  let nullary_con_of expr =
    if Arithmetic.is_real expr then
      mk_real (Arithmetic.Real.get_ratio expr)
      |> Option.some
    else None
  ...
  let con_of mk_op_app expr =
    let term =
      ...
      else if Arithmetic.is_div expr || Arithmetic.is_idiv expr then mk_op_app (mk_rdiv ()) expr
      else assert false
    in Option.some term
end

module RealIntTerm : TermType = struct
  open RealIntTerm
  ...
  let of_con ctx args = function
    ...
    | sym ->
      if Map.Poly.mem Logic.RealTerm.sym_sort_map sym then RealTerm.of_con ctx args sym
      else if Map.Poly.mem Logic.IntTerm.sym_sort_map sym then IntTerm.of_con ctx args sym
      else assert false
  ...
  let con_of mk_op_app expr =
    let term =
      if Arithmetic.is_int2real expr then mk_op_app (mk_to_real ()) expr
      else if Arithmetic.is_real2int expr then mk_op_app (mk_to_real ()) expr
...
module ExtTerm : TermType = struct
  ...
  let of_con ctx args = function
    ...
    | PDiv -> begin
        match args with
        | exp1 :: exp2 :: [] ->
          Boolean.mk_eq ctx (Arithmetic.Integer.mk_mod ctx exp2 exp1) (Arithmetic.Integer.mk_numeral_i ctx 0)
        | _ -> assert false
      end
    | IsInt -> begin
        match args with
    ...
    | sym ->
      if Map.Poly.mem Logic.BoolTerm.sym_sort_map sym then BoolTerm.of_con ctx args sym
      else if Map.Poly.mem Logic.RealIntTerm.sym_sort_map sym then RealIntTerm.of_con ctx args sym
      else assert false

  let nullary_con_of expr =
    match BoolTerm.nullary_con_of expr with
    | Some x -> Some x
    | None -> RealIntTerm.nullary_con_of expr
    ...
    let term =
      ...
      (** convert to Term ignoring mismatch of sort of ite/eq
        * The type will be checked after an ast is build *)
        ...
        | Some x -> x
        | None -> begin
            match RealIntTerm.con_of mk_op_app expr with
            | Some x -> x
            | _ -> assert false
          end end
    in Option.some term
...
module Make (Term : TermType) : sig
  ...
  let rec sym_of t =
    if is_con t then let sym, _ = let_con t in sym
    else if is_tyapp t then let t, _, _ = let_tyapp t in sym_of t
    else assert false
  ...
  let rec of_term ctx env term =
    if is_var term then
      ...
      let term, _s, _ = let_tyapp term in of_term ctx env term
  ...
  and of_term_app ctx env term args =
    ...
    else if is_con t1 then let sym, _ = let_con t1 in Term.of_con ctx args sym
    else if is_tyapp t1 then let t, _, _ = let_tyapp t1 in Term.of_con ctx args (sym_of t)
  ...
  and op_of_expr senv expr =
    let mk_op_app op expr =
      ...
      | _ :: _ as lst ->
        List.fold ~f:(fun acc exp -> Logic.Term.mk_app acc (term_of senv exp)) ~init:op lst
      | _ -> assert false
    in
    match Term.con_of mk_op_app expr with
    | Some x -> x
    | None -> assert false
