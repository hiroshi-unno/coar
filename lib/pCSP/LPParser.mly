...
%type <(Ast.LogicOld.Formula.t * Ast.LogicOld.Formula.t) list > parser_main_logic_program
%%
...
atom:
  ...
  | term EQ term { Formula.mk_atom (T_bool.mk_eq $1 $3) }
  ...
  | TOP { Formula.mk_atom (Atom.mk_true ()) }
  | BOT { Formula.mk_atom (Atom.mk_false ()) }
  | VAR { Formula.mk_atom (Atom.of_bool_var (Ident.Tvar $1) )}
  | PVAR LPAREN terms RPAREN {
   let sorts = List.map Term.sort_of $3 in
