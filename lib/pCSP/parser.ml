...
let from_smt2_file ~print ~inline ?(skolem_pred=false)
  ...
  let phis, envs =
    filename |> In_channel.create |> Lexing.from_channel |> SMT.(Parser.program Lexer.token)
    |> SMT.Smtlib2.toplevel ~print ~inline [] { uni_senv = uni_senv; exi_senv = exi_senv; kind_map = kind_map; fenv = fenv; dtenv = dtenv }
  in
  let phis = Typeinf.typeinf ~print ~to_sus:true ~instantiate_num_to_int:true phis in
        ...
        let kind_map =
          ...
          let fnpvs =
            Set.Poly.filter_map psenv ~f:(fun (Ident.Pvar n, _) ->
                if String.is_prefix ~prefix:"FN_" n then Some (Ident.Tvar n) else None)
          in
          let wfpvs =
            Set.Poly.filter_map psenv ~f:(fun (Ident.Pvar n, _) ->
                if String.is_prefix ~prefix:"WF_" n then Some (Ident.Tvar n) else None)
          in
          Map.Poly.empty |> Kind.add_kinds fnpvs Kind.FN |> Kind.add_kinds wfpvs Kind.WF
        in
        Params.make ~kind_map @@
        Map.of_set_exn @@ Set.Poly.map psenv ~f:(fun (pvar, sargs) ->
            Ident.pvar_to_tvar pvar,
            Sort.mk_fun @@ List.map sargs ~f:ExtTerm.of_old_sort @ [ExtTerm.SBool])
      in
      (*Problem.normalize @@*) Problem.of_formulas ~params @@
      Set.Poly.of_list @@ List.rev_map phis ~f:(fun phi ->
