  ...
  let sub, phis_rest, phis_cond =
    let rec loop (sub, phis_rest, phis_cond) =
      let sub', phis_rest' =
        Set.partition_map phis_rest ~f:(fun phi ->
            try
              ...
              if Term.is_var t1 && Set.mem args_set (fst @@ fst @@ Term.let_var t1) && Set.is_empty @@ Set.inter (Term.fvs_of t2) args_set then
                First (fst @@ Term.let_var t1, t2)
              else if Term.is_var t2 && Set.mem args_set (fst @@ fst @@ Term.let_var t2) && Set.is_empty @@ Set.inter (Term.fvs_of t1) args_set then
                First (fst @@ Term.let_var t2, t1)
              else Second phi
            with _ -> Second phi)
      in
      if Set.is_empty sub' then sub, phis_rest, phis_cond
        ...
        let sub', phiss_rest =
          List.unzip @@ List.map ~f:(fun res ->
              let arg = fst @@ List.hd_exn res in
              match List.map ~f:snd res with
              | [] -> assert false
              | t :: ts ->
                (arg, t), Set.Poly.of_list @@ List.map ts ~f:(Formula.eq (uncurry Term.mk_var arg))) @@
          List.classify (fun x y -> Stdlib.(fst x = fst y)) @@ Set.to_list sub'
        in
        let subst = Map.Poly.of_alist_exn @@ List.map ~f:(fun ((x, _), t) -> x, t) sub' in
        loop (Set.union sub @@ Set.Poly.of_list sub',
              Set.Poly.map ~f:(Formula.subst subst) phis_rest',
              Set.union phis_cond @@
              Set.Poly.map ~f:(Formula.subst subst) @@ Set.Poly.union_list phiss_rest)
    in
...
let rec term_sort_env_of_command = function
  ...
  | Seq (c1, c2) | Choice (c1, c2) -> Set.union (term_sort_env_of_command c1) (term_sort_env_of_command c2)
...
let rec simplify ~print s cfa =
  if contract_edges ~print cfa || contract_vertex_1_1 ~print s cfa ||
     contract_vertex_1_n ~print s cfa || contract_vertex_n_1 ~print s cfa then
    simplify ~print s cfa
  else cfa
...
let rec cut_points_of g res =
  if DFS.has_cycle g then
    let vertices_self_loop = G.fold_vertex (fun s vs ->
        if G.mem_edge g s s then Set.add vs s else vs) g Set.Poly.empty
    in
    Set.iter vertices_self_loop ~f:(G.remove_vertex g);
    let res = Set.union res vertices_self_loop in
    let vertices_deg_leq_1 = G.fold_vertex (fun s vs ->
        if G.in_degree g s + G.out_degree g s <= 1 then Set.add vs s else vs) g Set.Poly.empty
    in
    Set.iter vertices_deg_leq_1 ~f:(G.remove_vertex g);
    let vertices_indeg1_outdeg1 = G.fold_vertex (fun s vs ->
        match G.pred g s, G.succ g s with
        | [pred], [succ] (*when String.(pred <> s && succ <> s)*) ->
          Set.add vs (pred, s, succ)
        | _ -> vs) g Set.Poly.empty
    in
    Set.iter vertices_indeg1_outdeg1 ~f:(fun (pred, s, succ) ->
        if not @@ G.mem_edge g pred succ then G.add_edge g pred succ;
        G.remove_vertex g s);
    if Set.is_empty vertices_self_loop && Set.is_empty vertices_deg_leq_1 && Set.is_empty vertices_indeg1_outdeg1 then
      let _, v = G.fold_vertex (fun s (deg0, s0) ->
          let deg = G.in_degree g s + G.out_degree g s in
          if deg > deg0 then (deg, s) else (deg0, s0)) g (-1, "")
      in
      G.remove_vertex g v;
...
let analyze ~print ((s, e, c, trans) as lts) =
  ...
  | Some s ->
    ...
    print @@ lazy (sprintf "cut points: %s" @@ String.concat_set ~sep:", " cut_points);
