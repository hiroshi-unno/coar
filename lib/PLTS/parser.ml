...
let parse_from_lexbuf ~print lexbuf =
  let _phis(* assert distict ...*), envs =
    ...
    |> SMT.Smtlib2.toplevel ~print ~inline:false [] { uni_senv = Map.Poly.empty; exi_senv = Map.Poly.empty; kind_map = Map.Poly.empty; fenv = Map.Poly.empty; dtenv = Map.Poly.empty }
  in
  ...
  let inits, nexts =
    ...
    |> Map.Poly.partition_mapi ~f:(fun ~key ~data:(args, ret, def, is_rec, prop) ->
        ...
        if String.is_prefix f ~prefix:"init_" then
          let pc, args = match args with (pc, _sloc) :: args -> pc, args | [] -> failwith "" in
          let src, rel =
            ...
            | [pc'; src; rel] ->
              ...
              Ident.name_of_tvar @@ fst @@ fst @@ Term.let_var src, T_bool.let_formula rel
            | _ -> failwith ""
          in
          ...
          let pc1, args1 = match args1 with (pc, _sloc) :: args -> pc, args | [] -> failwith "" in
          let pc2, args2 = match args2 with (pc, _sloc) :: args -> pc, args | [] -> failwith "" in
          let res =
            ...
            |> Set.Poly.map ~f:(fun phi ->
                ...
                match args with
                | [pc1'; src; pc2'; dst; rel] ->
                  assert (Ident.tvar_equal pc1 (fst @@ fst @@ Term.let_var pc1'));
                  assert (Ident.tvar_equal pc2 (fst @@ fst @@ Term.let_var pc2'));
  ...
  in
  let init = match Map.Poly.data inits with [init] -> init | _ -> failwith "parse_from_lexbuf" in
