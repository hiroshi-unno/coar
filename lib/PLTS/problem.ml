...
type plts =
  ...
  (string * sort_env_list * sort_env_list * (string * string * Formula.t) Core.Set.Poly.t) list
type mode = Term | NonTerm
type t = plts * mode

let init = "__init__"
let proc_loc proc loc = proc ^ "@" ^ loc
let lts_of ~print ((_locs, (init_proc, init_args, init_loc, init_rel), nexts) : plts) =
  ...
  let trans =
    ...
    List.concat_map nexts ~f:(fun (proc, args1, args2, trans) ->
        Set.to_list @@ Set.Poly.map trans ~f:(fun (src, dst, rel) ->
            proc_loc proc src,
            LTS.Problem.seq @@
            LTS.Problem.commands_of_formula ~print args2 rel @
            LTS.Problem.subst args1 (List.map args2 ~f:(uncurry Term.mk_var)),
            proc_loc proc dst))
  in
  Some init, None, None, trans

let lts_mode_of = function Term -> LTS.Problem.Term | NonTerm -> LTS.Problem.NonTerm
