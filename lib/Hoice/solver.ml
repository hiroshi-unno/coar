    ...
    | (msg :: tail) as result ->
      if Str.string_match (Str.regexp "^(error \\\"$") msg 0 then
        let rec read_error errors = function
          | [] ->
            failwith @@ sprintf "can't read error from %s" @@
            String.concat ~sep:"\n" result
          | "\")" :: tail -> errors, tail
        ...
        let rec read_warning = function
          | [] ->
            failwith @@ sprintf "can't read warning from %s" @@
            String.concat ~sep:"\n" result
          | "; |===|" :: tail -> tail
  ...
  let solve ?(print_sol=false) pcsp =
    ...
    let query_name, exi_senv =
      try
        ...
        query_name,
        Map.Poly.add_exn (PCSP.Problem.senv_of pcsp)
    ...
    Set.iter (PCSP.Problem.clauses_of pcsp) ~f:(fun (uni_senv, ps, ns, phi) ->
        ...
        in
        let head =
          Formula.and_of @@ Set.to_list @@ Set.Poly.map ~f:Formula.mk_atom @@
    ...
    let inputs =
      inputs @ if !exists_query then
        [ sprintf "(assert (forall () (not (%s))))" query_name ] else []
    in
