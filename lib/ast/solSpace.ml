...
let str_of t =
  ...
  |> String.concat_map_list ~sep:"\n" ~f:(fun (Ident.Tvar v, constrs) ->
         v ^ ".("
         ^ String.concat_map_list ~sep:"," constrs ~f:(fun (flag, i) ->
               match i with
               | Some i ->
                   sprintf "%s = %d"
                     (match flag with
  ...
  List.fold space_constrs ~init:Map.Poly.empty ~f:(fun acc (tvar, flag, i) ->
      ...
      | Some space ->
          Map.Poly.set acc ~key:tvar
            ~data:(Map.Poly.set space ~key:flag ~data:(Some i))
      | None ->
          Map.Poly.set acc ~key:tvar ~data:(Map.Poly.singleton flag (Some i)))
