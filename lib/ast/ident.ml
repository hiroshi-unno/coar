...
let is_ne_tvar (Tvar p) = String.is_prefix p ~prefix:(sprintf "NE%c" ne_split_char)
...
let str_of_penv tenv = String.concat_map_list ~sep:", " tenv ~f:(fun (Pvar x, _)  -> x)
