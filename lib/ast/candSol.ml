...
type tag = Total of bool(* with sample*) | Partial of Ident.tvar

let make cand : t =
  Map.Poly.empty,
  Set.Poly.map (Map.to_set cand) ~f:(fun (tvar, (sort, phi)) -> (tvar, sort), phi)

let of_old_bind (Ident.Pvar n, (args, phi)) =
  let args_new = List.map args ~f:(fun (tvar, sort) -> tvar, ExtTerm.of_old_sort sort) in
...
let to_old_bind ((Ident.Tvar n, _sort), term) =
  ...
  let phi = ExtTerm.to_old_formula Map.Poly.empty (Map.of_list_exn args) term' [] in
...
let of_subst params_senv theta : t =
  params_senv,
  Set.Poly.of_list @@ List.map ~f:(fun (var, term)->
...
let str_of (params_senv, cand) : string =
  String.concat_map_set ~sep:"\n" cand ~f:(fun ((Ident.Tvar tvar, _sort), term) ->
...
let to_yojson (params_senv, cand) =
  ...
  { candidates =
      Set.Poly.to_list @@ Set.Poly.map cand ~f:(fun ((Ident.Tvar tvar, _sort), term) ->
          ...
          in
          tvar,
          List.map params' ~f:(fun (tvar, sort) -> Ident.name_of_tvar tvar, ExtTerm.str_of_sort sort),
          str_term') }
...
let replace_int_con t =
  ...
  let rec replace_int_con' = function
    | Logic.App (Logic.App (Logic.Con (Logic.IntTerm.Mult,_),
                            Logic.Con (Logic.IntTerm.Int _, _), _),
                 Logic.Var (_, _), _) as t' -> t'
