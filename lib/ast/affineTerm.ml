...
let is_affine term =
  Set.for_all ~f:(function
      | T_int.Add | T_int.Sub | T_int.Neg | T_int.Mult | T_int.Int _-> true | _ -> false) @@
  ...
  Term.iter_term t ~f:(fun t ->
      if Option.is_none !ret then
        ...
        | Term.FunApp (T_int.Mult, [t1; t2], _) when is_v t2 ->
          ret := Some (Z.to_int(*ToDo*) @@ Value.int_of @@ Evaluator.eval_term t1)
        | _ -> ());
  match !ret with Some i -> i | None -> 1

let extract_term_from_affine is_unused_term affine =
  let ret =
    Term.map_term true affine ~f:(fun t11 -> if is_unused_term t11 then T_int.zero () else t11)
  in
