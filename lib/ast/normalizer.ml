...
let rec int_monomials_of (coeff : Value.t) = function
  ...
  | FunApp (T_int.Abs, [t], _) ->
    Map.Poly.singleton (Map.Poly.singleton (T_int.mk_abs (normalize_term t)) 1) coeff
  ...
  | FunApp (T_int.Div, [t1; t2], _) ->
    Map.Poly.singleton
      (Map.Poly.singleton (T_int.mk_div (normalize_term t1) (normalize_term t2)) 1)
      coeff
  | FunApp (T_int.Mod, [t1; t2], _) ->
    Map.Poly.singleton
      (Map.Poly.singleton (T_int.mk_mod (normalize_term t1) (normalize_term t2)) 1)
      coeff
  | FunApp (T_int.Rem, [t1; t2], _) ->
    Map.Poly.singleton
      (Map.Poly.singleton (T_int.mk_rem (normalize_term t1) (normalize_term t2)) 1)
      coeff
  ...
  | FunApp (FVar (x, sorts), ts, _) ->
    Map.Poly.singleton
      (Map.Poly.singleton (Term.mk_fvar_app x sorts (List.map ~f:normalize_term ts)) 1)
      coeff
  | FunApp (T_bool.IfThenElse, [t1; t2; t3], _) ->
    let mono = T_bool.mk_if_then_else (normalize_term t1) (normalize_term t2) (normalize_term t3) in
  ...
  | FunApp (fun_sym, terms, _) as term ->
    ...
    failwith @@ sprintf "[int_monomials_of] not supported: %s : (%s [%s])"
      (Term.str_of term) (Term.str_of_funsym fun_sym)
...
and real_monomials_of (coeff : Value.t) = function
  ...
  | FunApp (T_real.RDiv, [t1; t2], _) ->
    Map.Poly.singleton
      (Map.Poly.singleton (T_real.mk_rdiv (normalize_term t1) (normalize_term t2)) 1)
      coeff
  ...
  | FunApp (fun_sym, terms, _) ->
    failwith @@ sprintf "[real_monomials_of] not supported: (%s [%s])"
...
and normalize_datatype_eq psym cons_name dt ts t2  = (* psym must be eq/neq *)
  ...
  | Some cons ->
    ...
    | T_bool.Neq ->
      let is_not_cons = Formula.mk_atom @@ T_dt.mk_is_not_cons dt cons_name t2 in
...
and normalize_term ?(drop_coeff = false) = function
  ...
  | term ->
    ...
    | T_int.SInt ->
      ...
      |> NonLinear.int_simplify |> (if drop_coeff then NonLinear.div_by_gcd else Fn.id)
...
and normalize_psym psym terms =
  ...
  | T_bool.Neq, [t1; t2] when
      Term.is_bool_sort @@ Term.sort_of t1 && Fn.non Term.is_var t2 &&
  ...
  | T_bool.Neq, [t1; t2] when
      Term.is_bool_sort @@ Term.sort_of t1 && Fn.non Term.is_var t1 &&
  ...
  | (T_bool.Eq | T_bool.Neq), [t1; t2] -> (* assume that [t1] and [t2] are let-normalized *)
    ...
    (match Term.sort_of t1, Term.sort_of t2 with
     ...
     | T_real.SReal, T_real.SReal ->
       [normalize_term ~drop_coeff:true @@ T_real.mk_rsub t1 t2; T_real.rzero ()]
     | _ ->
       ...
       | t1, t2 ->
         if String.compare (Term.str_of t1) (Term.str_of t2) <= 0 then [t1; t2] else [t2; t1])
  (* the return value does not use Gt, Leq, and Lt *)
  | T_int.Geq, [t1; t2] ->
    T_int.Geq, [normalize_term ~drop_coeff:true @@ T_int.mk_sub t1 t2; T_int.zero ()]
  | T_int.Gt, [t1; t2] ->
  ...
  | T_real.RGeq, [t1; t2] ->
    T_real.RGeq, [normalize_term ~drop_coeff:true @@ T_real.mk_rsub t1 t2; T_real.rzero ()]
  | T_real.RGt, [t1; t2] ->
    T_real.RGt, [normalize_term ~drop_coeff:true @@ T_real.mk_rsub t1 t2; T_real.rzero ()]
...
and normalize_atom = function
  ...
  | Atom.App (Predicate.Psym psym, terms, _) ->
    uncurry Atom.mk_psym_app @@ normalize_psym psym @@ List.map terms ~f:normalize_term
...
and normalize_let_atom ?(info=LogicOld.Dummy) = function
  ...
  | Atom.App (pred, ts, info) ->
    let ts' = List.map ts ~f:normalize_let_term in
    let body = Formula.mk_atom @@ Atom.mk_app pred (List.map ts' ~f:Term.body_of_let) ~info in
...
let rec homogenize = function
  ...
  | Formula.UnaryOp (Not, phi, info) -> Formula.UnaryOp (Not, homogenize phi, info)
  | Formula.BinaryOp (op, phi1, phi2, info) ->
...
let linear_int_monomials_of coeff term =
  ...
  with _ -> failwith @@ "[linear_int_monomials_of] " ^ Term.str_of term ^ " not supported"
let linear_real_monomials_of coeff term =
  try Linear.to_linear @@ NonLinear.real_simplify @@ real_monomials_of coeff term
  with _ -> failwith @@ "[linear_real_monomials_of] " ^ Term.str_of term ^ " not supported"
