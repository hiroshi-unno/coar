...
module Priority = struct
  ...
  let add_paren outer_priority inner_priority inner_str =
    if outer_priority > inner_priority then String.paren inner_str else inner_str
end
...
module Value = struct
  ...
  let pow = bop (fun n1 n2 -> Z.pow n1 (Z.to_int(*ToDo*) n2)) (fun _ _ -> failwith "Value.pow for real numbers not implemented" (*ToDo*))
...
module Sort = struct
  ...
  let rec of_args_ret_pure args ret =
    ...
    | [] -> ret
    | hd :: tl ->
      SArrow (hd, (mk_fresh_empty_open_opsig (), of_args_ret_pure tl ret, Pure))
...
let str_of_pred_sort_env_set str_of_sort penv =
  String.concat_map_set ~sep:", " penv ~f:(fun (pvar, sorts) ->
      String.paren @@ sprintf "%s: %s" (Ident.name_of_pvar pvar) @@
...
let str_of_sort_subst str_of_sort sub =
  String.concat_map_list ~sep:", " (Map.Poly.to_alist sub) ~f:(fun (svar, sort) ->
      String.paren @@ sprintf "%s |-> %s" (Ident.name_of_svar svar) (str_of_sort sort))
...
let mk_fresh_sort_env_list = List.map ~f:(fun sort -> Ident.mk_fresh_tvar (), sort)
...
module Type = struct
  module type TermType = sig
    ...
    val body_of_let : t -> t
    val number_of_pvar_apps : ?ex:((Ident.tvar, int) Map.Poly.t) -> bool -> t -> int
    val ast_size : t -> int
  ...
  module type FormulaType = sig
    ...
    val fvar_apps_of : t -> term Set.Poly.t
    val filtered_terms_of : f:(term -> bool) -> t -> term Set.Poly.t
    val atoms_of : ?nrec:bool -> ?pos:bool -> t -> atom Set.Poly.t * atom Set.Poly.t
    val let_env_of : ?map:termSubst -> t -> termSubst
    val let_sort_env_of : ?map:sort_env_map -> t -> sort_env_map
    val find_div_mod : t -> (term * term) Set.Poly.t
    ...
    val pnf_of : t -> t
    val skolemize : bool -> t -> sort_env_map * sort_env_map * t
  ...
  module type T_sequenceType = sig
    ...
    type fun_sym += SeqEpsilon | SeqSymbol of string | SeqConcat of bool | SeqLeftQuotient of bool | SeqRightQuotient of bool
    type pred_sym += IsPrefix of bool | NotIsPrefix of bool | SeqInRegExp of bool * string Grammar.RegWordExp.t | NotSeqInRegExp of bool * string Grammar.RegWordExp.t
...
= struct
  ...
  let rec str_of_triple (o, s, e) =
    (if Sort.is_pure e then
     ...
     if Sort.is_empty_closed_opsig o then
       ...
       String.paren @@ sprintf "%s |> %s / %s" (str_of_opsig o) (str_of_sort s) (str_of_cont e))
  and str_of_sort = function
    | Sort.SVar svar -> Ident.name_of_svar svar
    | Sort.SArrow (s1, (o, s2, e)) ->
      sprintf "%s -> %s"
        (if Sort.is_arrow s1 then String.paren @@ str_of_sort s1 else str_of_sort s1)
        (str_of_triple (o, s2, e))
    | Sort.SPoly (svs, s) ->
      if Set.is_empty svs then failwith "[str_of_sort.SPoly]"(*str_of_sort s*)
    ...
    | T_dt.SUS (name, args) ->
      if List.is_empty args then name
        ...
        sprintf "%s %s"
          (String.paren @@ String.concat_map_list ~sep:", " args ~f:Term.str_of_sort)
          name
    | T_array.SArray (s1, s2) ->
      sprintf "%s -a> %s"
        (if Term.is_array_sort s1 then String.paren @@ str_of_sort s1 else str_of_sort s1)
        (str_of_sort s2)
  ...
  let rec short_name_of_sort = function
    ...
    | Sort.SPoly (svs, s) ->
      if Set.is_empty svs then failwith "[short_name_of_sort.SPoly]"(*short_name_of_sort s*)
      else
        sprintf "forall %s. %s"
          (if Set.is_singleton svs then
             Ident.name_of_svar @@ Set.choose_exn svs
           else
             String.paren @@ String.concat_map_set ~sep:"," ~f:Ident.name_of_svar svs)
  ...
  and svs_of_sort = function
    ...
    | Sort.SArrow (s1, (o, s2, e)) ->
      Set.Poly.union_list [svs_of_sort s1;
                           svs_of_opsig o; svs_of_sort s2; svs_of_cont e]
    ...
    | T_string.SString | T_sequence.SSequence _ | T_regex.SRegEx -> Set.Poly.empty
  ...
  let rec evs_of_cont = function
    ...
    | Sort.Eff (o1, s1, e1, o2, s2, e2) ->
      Set.Poly.union_list [evs_of_opsig o1; evs_of_sort s1; evs_of_cont e1;
                           evs_of_opsig o2; evs_of_sort s2; evs_of_cont e2]
    | _ -> failwith "svs_of_cont"
  and evs_of_sort = function
    | Sort.SVar _ -> Set.Poly.empty
    | Sort.SArrow (s1, (o, s2, e)) ->
      Set.Poly.union_list [evs_of_sort s1;
                           evs_of_opsig o; evs_of_sort s2; evs_of_cont e]
  ...
  let rec rvs_of_cont = function
    | Sort.EVar _ | Sort.Pure -> Set.Poly.empty
    | Sort.Eff (o1, s1, e1, o2, s2, e2) ->
      Set.Poly.union_list [rvs_of_opsig o1; rvs_of_sort s1; rvs_of_cont e1;
                           rvs_of_opsig o2; rvs_of_sort s2; rvs_of_cont e2]
    | _ -> failwith "rvs_of_cont"
  and rvs_of_sort = function
    | Sort.SVar _ -> Set.Poly.empty
    | Sort.SArrow (s1, (o, s2, e)) ->
      Set.Poly.union_list [rvs_of_sort s1;
                           rvs_of_opsig o; rvs_of_sort s2; rvs_of_cont e]
  ...
  let rec subst_sorts_sort map = function
    ...
    | T_tuple.STuple sorts -> T_tuple.STuple (List.map sorts ~f:(subst_sorts_sort map))
    | T_ref.SRef sort -> T_ref.SRef (subst_sorts_sort map sort)
    | T_dt.SDT dt -> T_dt.SDT (Datatype.subst_sorts map dt)
    | T_dt.SUS (name, args) -> T_dt.SUS (name, List.map args ~f:(subst_sorts_sort map))
    | T_array.SArray (s1, s2) ->
  ...
  and subst_conts_sort map = function
    ...
    | T_ref.SRef sort ->
      T_ref.SRef (subst_conts_sort map sort)
    | T_dt.SDT dt ->
  ...
  let rec subst_opsigs_sort map = function
    ...
    | T_ref.SRef sort ->
      T_ref.SRef (subst_opsigs_sort map sort)
    | T_dt.SDT dt ->
  ...
  let rec fresh_conts_sort = function
    ...
    | T_ref.SRef sort ->
      T_ref.SRef (fresh_conts_sort sort)
    | T_dt.SDT dt ->
      T_dt.SDT (Datatype.fresh_conts_sort dt)
    | T_dt.SUS (name, sorts) ->
  ...
  let rec fresh_rvs_cont = function
    ...
    | Sort.Eff (o1, s1, e1, o2, s2, e2) ->
      Sort.Eff (fresh_rvs_opsig o1, fresh_rvs_sort s1, fresh_rvs_cont e1,
                fresh_rvs_opsig o2, fresh_rvs_sort s2, fresh_rvs_cont e2)
    | _ -> failwith "fresh_rvs_cont"
  and fresh_rvs_sort = function
    | Sort.SVar _ as s -> s
    | Sort.SArrow (s1, (o, s2, e)) ->
      Sort.SArrow (fresh_rvs_sort s1,
                   (fresh_rvs_opsig o, fresh_rvs_sort s2, fresh_rvs_cont e))
    | Sort.SPoly (svs, s) ->
      Sort.SPoly (svs, fresh_rvs_sort s)
    | T_bool.SBool | T_int.SInt | T_int.SRefInt | T_real.SReal as s -> s
    | T_tuple.STuple sorts ->
      T_tuple.STuple (List.map ~f:fresh_rvs_sort sorts)
    | T_ref.SRef sort ->
      T_ref.SRef (fresh_rvs_sort sort)
    | T_dt.SDT dt ->
      T_dt.SDT (Datatype.fresh_rvs_sort dt)
    | T_dt.SUS (name, sorts) ->
      T_dt.SUS (name, (List.map ~f:fresh_rvs_sort sorts))
    | T_array.SArray (s1, s2) ->
  ...
  and fresh_rvs_opsig = function
    | Sort.OpSig (map, _) ->
      Sort.OpSig (ALMap.map map ~f:fresh_rvs_sort, Some (Ident.mk_fresh_rvar ()))
  ...
  let rec pat_match_sort sort1 sort2 =
    if Stdlib.(sort1 = sort2) then Map.Poly.empty, Map.Poly.empty, Map.Poly.empty
      ...
      | T_ref.SRef sort1, T_ref.SRef sort2 ->
        pat_match_sort sort1 sort2
      ...
      | T_dt.SDT dt1, T_dt.SUS (name2, sorts2) when String.(dt1.name = name2) ->
        let omaps, smaps, emaps =
          List.unzip3 @@ List.map2_exn (Datatype.params_of dt1) sorts2 ~f:pat_match_sort
        in
        Map.force_merge_list omaps,
        Map.force_merge_list smaps,
        Map.force_merge_list emaps
      | T_dt.SUS (name1, sorts1), T_dt.SDT dt2 when String.(name1 = dt2.name) ->
        let omaps, smaps, emaps =
          List.unzip3 @@ List.map2_exn sorts1 (Datatype.params_of dt2) ~f:pat_match_sort
        in
        Map.force_merge_list omaps,
        Map.force_merge_list smaps,
        Map.force_merge_list emaps
      | T_array.SArray (s11, s12), T_array.SArray (s21, s22) ->
        ...
        Map.force_merge_list emaps
      | _ ->
        failwith (sprintf "[pat_match_sort] %s and %s does not match" (str_of_sort sort1) (str_of_sort sort2))
  ...
  let add_dummy_term tvar sort =
    if not (is_dummy_term tvar (Some sort)) &&
       (T_dt.is_sus sort || Sort.is_arrow sort) then
      set_dummy_term_senv @@ get_dummy_term_senv () @ [tvar, sort]
  ...
  let rec mk_dummy = function
    ...
    | T_real.SReal ->
      T_real.rzero ()
    | T_ref.SRef sort ->
      T_ref.mk_ref sort @@ mk_dummy sort
    | T_array.SArray (s1, s2) ->
      T_array.mk_const_array s1 s2 @@ mk_dummy s2
    | T_tuple.STuple sorts ->
      T_tuple.mk_tuple_cons sorts @@ List.map ~f:mk_dummy sorts
    | T_dt.SDT _ | T_dt.SUS (_, _) as sort ->
      T_dt.mk_dummy sort
    | T_string.SString ->
      mk_fsym_app (T_string.StrConst "") []
    | T_sequence.SSequence true ->
      T_sequence.mk_eps ()
    | T_sequence.SSequence false ->
  ...
  let str_of_funsym = function
    ...
    | T_num.Value (v, svar) -> sprintf "value(%s:%s)" v (Ident.name_of_svar svar)
    ...
    | FVar (var', sorts) -> begin
        ...
        | Some t ->
          try
            ...
            FVar (var, sorts)
          with _ ->
            failwith @@ sprintf "[subst_fun_sym] %s |-> %s"
              (Ident.name_of_tvar var') (str_of t)
      end
  ...
  let subst_sorts_fun_sym map = function
    ...
    | (T_num.Value (_, svar) | T_num.NAdd svar | T_num.NSub svar | T_num.NMult svar |
       T_num.NDiv svar | T_num.NPower svar | T_num.NNeg svar) as fsym ->
    ...
    | T_dt.DTCons (name, arg_sorts, dt) ->
      T_dt.DTCons (name, List.map arg_sorts ~f:(Term.subst_sorts_sort map),
  ...
  let subst_conts_fun_sym map = function
    ...
    | T_dt.DTCons (name, arg_sorts, dt) ->
      T_dt.DTCons (name, List.map arg_sorts ~f:(Term.subst_conts_sort map),
  ...
  let subst_opsigs_fun_sym map = function
    ...
    | (T_num.Value (_, svar) | T_num.NAdd svar | T_num.NSub svar | T_num.NMult svar |
       T_num.NDiv svar | T_num.NPower svar | T_num.NNeg svar) as fsym ->
      T_num.fsym_of_num_fsym fsym @@ Term.subst_opsigs_sort map @@ Sort.SVar svar
    | T_bool.Formula phi ->
    ...
    | T_ref.Ref sort ->
      T_ref.Ref (Term.subst_opsigs_sort map sort)
    | T_ref.Deref sort ->
      T_ref.Deref (Term.subst_opsigs_sort map sort)
    | T_ref.Update sort ->
      T_ref.Update (Term.subst_opsigs_sort map sort)
    | T_dt.DTCons (name, arg_sorts, dt) ->
      T_dt.DTCons (name, List.map arg_sorts ~f:(Term.subst_opsigs_sort map),
  ...
  let neg_fsym = function
    | T_int.Add -> T_int.Sub | T_int.Sub -> T_int.Add
    | T_int.Mult -> T_int.Div | T_int.Div -> T_int.Mult
    | T_real.RAdd -> T_real.RSub | T_real.RSub -> T_real.RAdd
    | T_real.RMult -> T_real.RDiv | T_real.RDiv -> T_real.RMult
    | fsym -> failwith @@ sprintf "[T_int.neg_fsym] %s" (Term.str_of_funsym fsym)

  (** Morphism *)
  let rec para ~f = function
    | Var (tvar, sort, _) -> f#fvar tvar sort
    | FunApp (fsym(*ToDo: can be formula*), args, _) -> f#fapp fsym @@ (args, List.map args ~f:(para ~f))
    | LetTerm (tvar, sort, def, body, _) ->
      f#flet tvar sort (def, para ~f def) (body, para ~f body)
  let rec fold ~f = function
    | Var (tvar, sort, _) -> f#fvar tvar sort
    | FunApp (fsym(*ToDo: can be formula*), args, _) -> f#fapp fsym @@ List.map args ~f:(fold ~f)
    | LetTerm (tvar, sort, def, body, _) ->
  ...
  let map_term _ ~f =
    fold ~f:(object
      ...
      method fapp fsym args =
        ...
        | T_bool.Formula phi, [] ->
          f @@ Term.mk_fsym_app (T_bool.Formula (Formula.map_term ~f phi)) args
        | _, _ -> f @@ Term.mk_fsym_app fsym args
      method flet tvar sort def body = f @@ Term.mk_let_term tvar sort def body
    end)
  let iter_term ~f t = ignore @@ map_term true t ~f:(fun t -> f t; t); ()
  let map_atom ~f =
    fold ~f:(object
  ...
  let map_atom_polarity ~f =
    fold ~f:(object
      ...
      method fapp fsym args polarity =
        match fsym, args with
        | T_bool.Formula phi, [] ->
          Term.mk_fsym_app (T_bool.Formula (Formula.map_atom_polarity ~f phi polarity)) []
        | _, _ ->
          Term.mk_fsym_app fsym (List.map args ~f:(fun arg -> arg polarity))
      method flet tvar sort def body polarity =
        Term.mk_let_term tvar sort (def polarity) (body polarity)
    end)
  let iter_atom ~f t = ignore @@ map_atom t ~f:(fun atm -> f atm; Formula.mk_atom atm); ()
  ...
  let rec str_of ?(priority=Priority.lowest) t0 =
    para ~f:(object
      method fvar tvar sort _priority =
        ...
        | sort ->
          if false
          then sprintf "%s:%s" (Ident.name_of_tvar tvar) (short_name_of_sort sort)
      ...
      method fapp fsym (ts, args) priority =
        ...
        | FVar (x, _), ts ->
          if List.length ts = 0 then Ident.name_of_tvar x
            ...
            String.concat_map_list ~sep:" " ts ~f:(fun t -> t (Priority.fun_app+1))
        | T_bool.Formula phi, [] ->
          Priority.add_paren priority Priority.lowest(*ToDo*) @@
          String.angle_bracket @@ Formula.str_of ~priority:Priority.lowest phi
        | T_bool.IfThenElse, [cond; then_; else_] ->
          Priority.add_paren priority Priority.ite @@
          sprintf "if %s then %s else %s" (cond Priority.ite(*ToDo*)) (then_ Priority.lowest(*ToDo*)) (else_ Priority.ite(*ToDo*))
        | T_int.Int n, [] ->
        ...
        | T_real.Alge (t, n), [] ->
          sprintf "root-obj(%s %s)" (str_of ~priority:Priority.lowest t) (string_of_int n)
        | (T_real_int.ToReal | T_real_int.ToInt), [t] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "%s %s" (str_of_funsym fsym) (t (Priority.fun_app+1))
        | (T_int.Neg | T_real.RNeg | T_num.NNeg _), [t] ->
          Priority.add_paren priority Priority.neg_deref @@
          sprintf "%s%s" (str_of_funsym fsym) (t (Priority.neg_deref+1))
        | (T_int.Abs | T_real.RAbs), [t] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "%s %s" (str_of_funsym fsym) (t (Priority.fun_app+1))
        | (T_int.Add | T_real.RAdd | T_num.NAdd _ |
           T_int.Sub | T_real.RSub | T_num.NSub _), [t1; t2] ->
          Priority.add_paren priority Priority.add_sub @@
          sprintf "%s %s %s"
            (t1 Priority.add_sub)
            (str_of_funsym fsym)
            (t2 (Priority.add_sub+1))
        | (T_int.Mult | T_real.RMult | T_num.NMult _ |
           T_int.Div | T_real.RDiv | T_num.NDiv _ |
           T_int.Mod | T_int.Rem), [t1; t2] ->
          Priority.add_paren priority Priority.mult_div_mod @@
          sprintf "%s %s %s" (t1 Priority.mult_div_mod) (str_of_funsym fsym) (t2 (Priority.mult_div_mod+1))
        | (T_int.Power | T_real.RPower | T_num.NPower _), [t1; t2] ->
          ...
          sprintf "%s %s %s"
            ...
            (str_of_funsym fsym)
            (t2 Priority.append_power)
        | (T_int.Add | T_real.RAdd | T_num.NAdd _ |
           T_int.Mult | T_real.RMult | T_num.NMult _ |
           T_int.Div | T_real.RDiv | T_num.NDiv _ |
           T_int.Mod | T_int.Rem |
           T_int.Power | T_real.RPower | T_num.NPower _), _ ->
          failwith "add, sub, mult, div, mod, rem, power are binary"
        | T_num.Value _, _ ->
          str_of_funsym fsym
        | T_tuple.TupleCons sorts, ts ->
          Priority.add_paren priority Priority.comma @@ String.concat ~sep:"," @@
          List.map2_exn ts sorts ~f:(fun t s -> t (Priority.comma+1)(*ToDo*) ^ ":" ^ str_of_sort s)
        ...
        | T_ref.Update _sort, [t1; t2] ->
          ...
          sprintf "%s := %s" (t1 (Priority.assign+1)) (t2 Priority.assign)
        | T_dt.DTCons ("::", _, dt), [_; _]
          ...
          let rec aux args = function
            ...
            | t -> args, Some t
          in
          let t1, t2 = match ts with [t1; t2] -> t1, t2 | _ -> assert false in
          (match aux [t1] t2 with
           | args, None ->
             Priority.add_paren priority Priority.lowest(*ToDo*) @@
             String.bracket @@ String.concat_map_list ~sep:"; " args ~f:(str_of ~priority:(Priority.seq+1)(*ToDo*))
           | [t1], Some t2 ->
        ...
        | T_dt.DTSel (_, _, _) as fsym, [t1] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "%s %s" (str_of_funsym fsym) (t1 (Priority.fun_app+1))
        | T_array.ASelect _, [t1; t2] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "%s%s" (t1 Priority.highest) (String.bracket @@ t2 Priority.lowest)
        | T_array.AStore _, [t1; t2; t3] ->
          Priority.add_paren priority Priority.fun_app @@
          String.paren @@ sprintf "store %s %s %s" (t1 (Priority.fun_app+1)) (t2 (Priority.fun_app+1)) (t3 (Priority.fun_app+1))
        | T_array.AConst _, [t1] ->
          Priority.add_paren priority Priority.highest(*ToDo*) @@
          sprintf "ARR%s" @@ String.brace @@ t1 Priority.lowest
        ...
        | T_sequence.SeqConcat fin, [t1; t2] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "concat_%s %s %s" (if fin then "fin" else "inf")
            (t1 (Priority.fun_app+1)) (t2 (Priority.fun_app+1))
        | T_sequence.SeqLeftQuotient fin, [t1; t2] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "leftQuot_%s %s %s" (if fin then "fin" else "inf")
            (t1 (Priority.fun_app+1)) (t2 (Priority.fun_app+1))
        | T_sequence.SeqRightQuotient fin, [t1; t2] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "rightQuot_%s %s %s" (if fin then "fin" else "inf")
            (t1 (Priority.fun_app+1)) (t2 (Priority.fun_app+1))
        ...
        | T_regex.RegConcat, [t1; t2] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "concat %s %s" (t1 (Priority.fun_app+1)) (t2 (Priority.fun_app+1))
        | T_regex.RegUnion, [t1; t2] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "union %s %s" (t1 (Priority.fun_app+1)) (t2 (Priority.fun_app+1))
        | T_regex.RegInter, [t1; t2] ->
          Priority.add_paren priority Priority.fun_app @@
          sprintf "inter %s %s" (t1 (Priority.fun_app+1)) (t2 (Priority.fun_app+1))
        | f, ts ->
          failwith ("unknown function application: " ^
                    String.concat ~sep:" " @@
                    str_of_funsym f :: List.map ts ~f:(fun t -> t (Priority.fun_app+1)))
      method flet tvar _sort (_, def) (_, body) priority =
        Priority.add_paren priority Priority.let_forall_exists @@
        sprintf "let %s = %s in %s"
          (Ident.name_of_tvar tvar) (def Priority.let_forall_exists(*ToDo*)) (body Priority.let_forall_exists(*ToDo*))
  ...
  let rec is_numerical_compound term =
    ...
    match term with
    | Var _ -> false
    | FunApp ((T_int.Int _| T_real.Real _ | T_real.Alge _ | FVar (_, _)), _, _) ->
      false
    | FunApp (_, _, _) -> true
  ...
  let rec is_pathexp = function
    ...
    | FunApp ((T_int.Neg | T_real.RNeg | T_num.NNeg _ |
               T_real_int.ToReal | T_real_int.ToInt), [t], _) ->
      is_pathexp t
    | FunApp (T_dt.DTSel (sel_name, dt, _),
              [FunApp (T_dt.DTCons (cons_name, _, _), _, _)], _) ->
      begin match Datatype.look_up_cons dt cons_name with
        | Some cons ->
          not @@ List.exists (Datatype.sels_of_cons cons)
  ...
  let svs_of =
    let let_svar = function
      | Sort.SVar svar -> Set.Poly.singleton svar | _ -> Set.Poly.empty
    in
    fold ~f:(object
      ...
      method flet _ sort def body = Set.Poly.union_list [def; body; let_svar sort]
    end)
  let term_sort_env_of ?(bvs = Set.Poly.empty) t =
    fold ~f:(object
      method fvar x sort bvs =
        if Set.mem bvs x then Set.Poly.empty else Set.Poly.singleton (x, sort)
    ...
    if is_pathexp t then Set.Poly.singleton t
      ...
      | FunApp (fsym, args, _) ->
        ...
        (match fsym with
         ...
         | _ -> Set.Poly.empty) :: List.map args ~f:(pathexps_of ~bvs)
  ...
  let rec number_of_pvar_apps ?(ex=Map.Poly.empty) is_pos = function
    ...
    | LetTerm (x, _, def, body, _) ->
      ...
      number_of_pvar_apps ~ex:(Map.Poly.set ~key:x ~data:def_size ex) is_pos body
    | Var (x, _, _) ->
      begin match Map.Poly.find ex x with Some i -> i | None -> 0 end
    | FunApp (_, ts, _) ->
  ...
  let ast_size t =
    let rec ast_size ~ex = function
      ...
      | FunApp (sym, args, _) ->
        ...
        + (match sym with T_bool.Formula phi -> Formula.ast_size phi - 1 | _ -> 0)
      | LetTerm (var, _, def, body, _) ->
        ast_size ~ex:(Map.Poly.set ex ~key:var ~data:(ast_size ~ex def)) body
    in ast_size ~ex:Map.Poly.empty t
  ...
  let rec sort_of = function
    ...
    | FunApp (T_int.Neg, _, _) | FunApp (T_int.Abs, _, _)
    | FunApp (T_int.Add, _, _) | FunApp (T_int.Sub, _, _)
    | FunApp (T_int.Mult, _, _) | FunApp (T_int.Div, _, _)
    | FunApp (T_int.Mod, _, _) | FunApp (T_int.Rem, _, _) -> T_int.SInt
    | FunApp (T_real.Real _, _, _) | FunApp (T_real.Alge _, _, _)
    | FunApp (T_real_int.ToReal, _, _)
    | FunApp (T_real.RNeg, _, _) | FunApp (T_real.RAbs, _, _)
    | FunApp (T_real.RAdd, _, _) | FunApp (T_real.RSub, _, _)
    | FunApp (T_real.RMult, _, _) | FunApp (T_real.RDiv, _, _) -> T_real.SReal
    ...
    | FunApp (T_ref.Update _, [_; _], _) -> Datatype.sort_of (Datatype.mk_unit_dt ())
    | FunApp (T_string.StrConst _, _, _) -> T_string.SString
    | FunApp (T_sequence.(SeqEpsilon | SeqSymbol _), [], _) -> T_sequence.SSequence true
    | FunApp (T_sequence.(SeqConcat fin | SeqLeftQuotient fin | SeqRightQuotient fin), [_; _], _) -> T_sequence.SSequence fin
    | FunApp (T_regex.(RegEmpty | RegFull | RegEpsilon), [], _) -> T_regex.SRegEx
    | FunApp (T_regex.(RegStr | RegComplement | RegStar | RegPlus | RegOpt), [_], _) -> T_regex.SRegEx
    | FunApp (T_regex.(RegConcat | RegUnion | RegInter), [_; _], _) -> T_regex.SRegEx
  ...
  let rec alpha_rename_let ?(map=Map.Poly.empty) = function
    ...
    | FunApp (fsym, ts, info) ->
      FunApp (alpha_rename_let_fun_sym ~map fsym, List.map ts ~f:(alpha_rename_let ~map),
              info)
    | LetTerm (var, sort, def, body, info) ->
  ...
  let rec let_sort_env_of ?(map=Map.Poly.empty) = function
    | Var _ -> map
    | FunApp (T_bool.Formula phi, [], _) ->
      Formula.let_sort_env_of ~map phi
    | FunApp (_, ts, _) ->
  ...
  let insert_let_term tvar sort def info term =
    if Set.mem (tvs_of term) tvar then
      ...
      LetTerm (tvar', sort, def, rename (Map.Poly.singleton tvar tvar') term, info)
    else term
  ...
  let rec elim_ite = function
    | Var (var, sort, info) ->
      [Formula.mk_true (), Var (var, sort, info)]
    | FunApp (T_bool.IfThenElse, [t1; t2; t3], _) ->
      ...
      List.map (elim_ite t3) ~f:(fun (phi, t) -> Formula.mk_and ncond phi, t)
    | FunApp (T_bool.Formula phi, [], info) ->
    ...
    | FunApp (fsym(* ToDo *), [t1], _) ->
      List.map (elim_ite t1) ~f:(fun (phi, t) -> phi, Term.mk_fsym_app fsym [t])
  ...
  let rec extend_pred_params pvar extended_params = function
    ...
    | FunApp (sym, args, info) ->
      let sym =
        match sym with
        | T_bool.Formula phi ->
          T_bool.Formula (Formula.extend_pred_params pvar extended_params phi)
        | sym -> sym
      in
      FunApp (sym, List.map ~f:(extend_pred_params pvar extended_params) args, info)
    | LetTerm (var, sort, def, body, info) ->
      LetTerm (var, sort,
  ...
  let rec replace_div_mod map = function
    ...
    | FunApp (sym, args, info) ->
      let sym =
        match sym with
        | T_bool.Formula phi -> T_bool.Formula (Formula.replace_div_mod map phi)
        | sym -> sym
      in
  ...
  let rec unify bvs t1 t2 =
    ...
    | FunApp (T_bool.Formula (Formula.Atom (atm1, _)), [], _),
      ...
      Option.some @@ Map.Poly.empty
    | Var (x, _, _), t
      when not (Set.mem bvs x) && not (Set.mem (tvs_of t) x) ->
    ...
    | FunApp (funsym, [Var (x, _, _); t3], _), t2
      when not (Set.mem bvs x) &&
           ...
           not (Set.mem (tvs_of t3) x) && not (Set.mem (tvs_of t2) x) ->
      Option.some @@ Map.Poly.singleton x (Term.mk_fsym_app (neg_fsym funsym) [t2; t3])
    | FunApp (funsym, [t3; Var (x, _, _)], _), t2
      when not (Set.mem bvs x) &&
           Stdlib.(funsym = T_int.Add || funsym = T_real.RAdd (*|| funsym = T_int.Mult*)) &&
           not (Set.mem (tvs_of t3) x) && not (Set.mem (tvs_of t2) x) ->
      Option.some @@ Map.Poly.singleton x (Term.mk_fsym_app (neg_fsym funsym) [t2; t3])
    | FunApp (funsym, [t3; Var (x, _, _)], _), t2
      when not (Set.mem bvs x) &&
           Stdlib.(funsym = T_int.Sub || funsym = T_real.RSub (*|| funsym = T_int.Div*)) &&
           not (Set.mem (tvs_of t3) x) && not (Set.mem (tvs_of t2) x) ->
      Option.some @@ Map.Poly.singleton x (Term.mk_fsym_app funsym [t3; t2])
    | t1, FunApp (funsym, [Var (x, _, _); t3], _)
      when not (Set.mem bvs x) &&
           Stdlib.(funsym = T_int.Add || funsym = T_real.RAdd ||
                   funsym = T_int.Sub || funsym = T_real.RSub) &&
           not (Set.mem (tvs_of t3) x) && not (Set.mem (tvs_of t2) x) ->
      Option.some @@ Map.Poly.singleton x (Term.mk_fsym_app (neg_fsym funsym) [t1; t3])
  ...
  let pattern_match bvs t1 t2 =
    if Set.is_empty @@ Term.tvs_of t1 && Stdlib.(t1 = t2) then
      ...
      | FunApp (T_int.Int n1, [], _), FunApp (T_int.Int n2, [], _)
        when Z.Compare.(=) n1 n2 -> (* ToDo: reachable? *)
        Option.some @@ Map.Poly.empty
      | FunApp (T_real.Real r1, [], _), FunApp (T_real.Real r2, [], _)
        when Q.(=) r1 r2 -> (* ToDo: reachable? *)
        Option.some @@ Map.Poly.empty
      | FunApp (T_bool.Formula (Formula.Atom (atm1, _)), [], _),
        FunApp (T_bool.Formula (Formula.Atom (atm2, _)), [], _)
        when (Atom.is_true atm1 && Atom.is_true atm2) ||
...
= struct
  ...
  let subst_sorts_psym map = function
    ...
    | T_dt.NotIsCons (name, dt) -> T_dt.NotIsCons (name, Datatype.subst_sorts map dt)
    | psym -> psym(* ToDo *)
  let subst_conts_psym map = function
    ...
    | T_dt.NotIsCons (name, dt) -> T_dt.NotIsCons (name, Datatype.subst_conts map dt)
    | psym -> psym(* ToDo *)
  let subst_opsigs_psym map = function
    ...
    | T_dt.NotIsCons (name, dt) -> T_dt.NotIsCons (name, Datatype.subst_opsigs map dt)
    | psym -> psym(* ToDo *)
  let negate_psym = function
    ...
    | T_sequence.SeqInRegExp (fin, regexp) -> T_sequence.NotSeqInRegExp (fin, regexp)
    | T_sequence.NotSeqInRegExp (fin, regexp) -> T_sequence.SeqInRegExp (fin, regexp)
  ...
  let str_of = function
    ...
    | Fixpoint (fp, Ident.Pvar pvar, params, phi) ->
      String.paren @@ sprintf "%s%s%s. %s" (str_of_fop fp) pvar
  ...
  let tvs_of = function
    ...
    | Fixpoint (_, pvar, bounds, body) ->
      Set.diff (Formula.tvs_of body)
        (Set.add (Set.Poly.of_list @@ List.map ~f:fst bounds) (Ident.pvar_to_tvar pvar))
  let pvs_of = function
    | Var (pvar, _) -> Set.Poly.singleton pvar
    | Psym _ -> Set.Poly.empty
    | Fixpoint (_, pvar, bounds, body) ->
      Set.diff (Formula.pvs_of body)
        (Set.add (Set.Poly.map ~f:Ident.tvar_to_pvar @@ Set.Poly.of_list @@ List.map ~f:fst bounds) pvar)
  let fvs_of atm =
    Set.union (tvs_of atm)
      (Set.Poly.map ~f:Ident.pvar_to_tvar(*ToDo*) @@ pvs_of atm)
  let svs_of = function
    | Var (_, sorts) ->
      Set.Poly.of_list @@
      List.filter_map sorts ~f:(function Sort.SVar svar -> Some svar | _ -> None)
    | Psym _ -> Set.Poly.empty
    | Fixpoint (_, _, bounds, body) ->
      Set.union (Formula.svs_of body)
        (Set.Poly.of_list @@
         List.filter_map ~f:(function Sort.SVar svar -> Some svar | _ -> None) @@
  ...
  let aconv_pvar = function
    ...
    | Fixpoint (fp, pvar, params, body) ->
      ...
      let body =
        Formula.aconv_pvar @@ Formula.rename_pvars (Map.Poly.singleton pvar pvar') body
      in
      Fixpoint (fp, pvar', params, body)
  ...
  let extend_pred_params x extended_params = function
    | Fixpoint (fp, pvar, args, body) ->
      Fixpoint (fp, pvar, args, Formula.extend_pred_params x extended_params body)
    | x -> x
end
...
= struct
  ...
  let mk_pvar_app ?(info=Dummy) pvar sorts = mk_app ~info (Predicate.mk_var pvar sorts)
  ...
  let sort_env_of ?(bpvs = Set.Poly.empty) atm =
    ...
    Set.Poly.map ~f:(fun (pvar, sorts) ->
        Ident.pvar_to_tvar(*ToDo*) pvar, Sort.mk_fun (sorts @ [T_bool.SBool])) @@
  ...
  let funsyms_of = function
    | True _ | False _ -> Set.Poly.empty
    | App (_, terms, _) -> Set.Poly.union_list @@ List.map ~f:Term.funsyms_of terms
  let pathexps_of ?(bvs = Set.Poly.empty) = function
    | True _ | False _ -> Set.Poly.empty
    | App (Predicate.Fixpoint (_, _, senv, body), args, _) ->
      Set.Poly.union_list @@
      Set.diff (Formula.pathexps_of ~bvs body)
  ...
  let filtered_terms_of ~f = function
    ...
    | App (Predicate.Fixpoint (_, _, _, body), args, _) ->
      Set.Poly.union_list @@
      Formula.filtered_terms_of ~f body :: List.map args ~f:(Term.filtered_terms_of ~f)
    | App (_, args, _) ->
  ...
  let atoms_of ?(pos=true) = function
    ...
    | App (_pred(*ToDo*), terms, _) as atom ->
      let poss1, negs1 = List.unzip @@ List.map terms ~f:(Term.atoms_of ~pos) in
      if pos then
        Set.Poly.union_list @@ Set.Poly.singleton atom :: poss1, Set.Poly.union_list negs1
      else
        Set.Poly.union_list poss1, Set.Poly.union_list @@ Set.Poly.singleton atom :: negs1
  ...
  let number_of_pvar_apps ?(ex=Map.Poly.empty) is_pos = function
    ...
    | App (Predicate.Fixpoint _, _, _) -> ignore ex; assert false (* This function does not support fixpoint fomulas appropriately *)
  ...
  let ast_size = function
    ...
    | App (Predicate.Fixpoint (_, _, _, phi), terms, _) ->
      Formula.ast_size phi + (Integer.sum_list @@ List.map ~f:Term.ast_size terms)
  ...
  let subst map = function
    ...
    | App (Var (pvar, []), [], _) when Map.Poly.mem map (Ident.pvar_to_tvar pvar) ->
  ...
  let subst_preds map = function
    ...
    | App (Predicate.Var (pvar, sort), args, info) ->
      ...
      (match Map.Poly.find map pvar with
       ...
       | None ->
         Formula.mk_atom (App (Predicate.Var (pvar, sort), args, info)))
    | App (Psym sym, args, info) ->
    ...
    | App (Predicate.Fixpoint (fp, pvar, params, phi), terms, info) ->
      ...
      Formula.mk_atom (App (Predicate.Fixpoint (fp, pvar, params, phi), terms, info))
  let subst_neg pvar = function
    | True info -> True info
    | False info -> False info
  ...
  let elim_ite = function
    | App (Psym (T_bool.Eq | T_bool.Neq |
                 ...
                 T_real.RLeq | T_real.RGeq | T_real.RLt | T_real.RGt as psym),
           [t1; t2], _) ->
  ...
  let extend_pred_params pvar (extended_params: sort_env_list) = function
    ...
    | App (pred, args, info) ->
      ...
      let args = List.map ~f:(Term.extend_pred_params pvar extended_params) args in
      App (pred, args, info)
    | x -> x
...
= struct
  ...
  (** Sorts *)
  let subst_sorts_bounds map = List.map ~f:(fun (x, s) -> x, Term.subst_sorts_sort map s)
  let subst_conts_bounds map = List.map ~f:(fun (x, s) -> x, Term.subst_conts_sort map s)
  let subst_opsigs_bounds map = List.map ~f:(fun (x, s) -> x, Term.subst_opsigs_sort map s)
  ...
  let flip_binop = function And -> Or | Or -> And | Iff -> Xor | Xor -> Iff | Imply -> failwith "Imply not supported"
  ...
  let rec mk_randoms rands f =
    match rands with
    | [] -> f
    | (var, rand) :: tl -> mk_random rand [var, Rand.sort_of rand] (mk_randoms tl f)
  ...
  let rec negate = function
    | Atom (atom, info) as phi -> (try Atom (Atom.negate atom, info) with _ -> mk_neg phi)
  ...
  let xor_of = function
    ...
    | Atom (Atom.True _, _) :: phi :: [] | phi :: Atom (Atom.True _, _) :: [] ->
      mk_neg phi
    | Atom (Atom.False _, _) :: phi :: [] | phi :: Atom (Atom.False _, _) :: [] ->
      phi
    | phi1 :: phi2 :: [] ->
  ...
  let rec fold ~f = function
    ...
    | LetRec (funcs, phi, _) ->
      let funcs =
        List.map funcs ~f:(fun (fix, pvar, senv, phi) -> fix, pvar, senv, fold ~f phi)
      in
      f#fletrec funcs (fold ~f phi)
    | LetFormula (tvar, sort, def, body, _) -> f#flet tvar sort def (fold ~f body)
  let map_term ~f =
    fold ~f:(object
  ...
  let map_atom_polarity ~f =
    fold ~f:(object
      ...
      method fbind binder senv p1 polarity = mk_bind binder senv (p1 polarity)
      method fletrec funcs p1 polarity =
        let funcs =
          List.map funcs ~f:(fun (fix, pvar, args, phi) -> fix, pvar, args, phi polarity)
        in
        mk_letrec funcs (p1 polarity)
      method fimply p1 p2 polarity = mk_imply (p1 @@ not polarity) (p2 polarity)
      ...
      method flet tvar sort def body polarity =
        mk_let_formula tvar sort
          (Term.map_atom_polarity ~f def polarity) (body polarity)
    end)
  let iter_atom ~f phi =
    ignore @@ map_atom phi ~f:(fun atm -> f atm; Formula.mk_atom atm); ()

  (** Printing *)
  let str_of ?(priority=Priority.lowest) =
    let rec aux ?(priority=Priority.lowest) ~next = function
      | Atom (atom, _) ->
        next @@ Atom.str_of ~priority atom
      | UnaryOp (Not, phi, _) ->
        aux ~priority:(Priority.fun_app+1(*ToDo*)) phi ~next:(fun s ->
            next @@ Priority.add_paren priority Priority.fun_app @@
            sprintf "not %s" s)
      ...
      | BinaryOp (Imply, phi1, phi2, _) ->
        aux phi1 ~priority:Priority.imply_iff_xor ~next:(fun s1 ->
            aux phi2 ~priority:Priority.imply_iff_xor ~next:(fun s2 ->
                next @@ Priority.add_paren priority Priority.imply_iff_xor @@
                sprintf "%s => %s" s1 s2))
      ...
      | Bind (Forall, params, phi, _) ->
        aux phi ~priority:Priority.let_forall_exists ~next:(fun s ->
            next @@ Priority.add_paren priority Priority.let_forall_exists @@
            sprintf "forall %s. %s"
      ...
      | LetRec (funcs, body, _) ->
        aux body ~priority:Priority.let_forall_exists ~next:(fun s ->
            ...
            sprintf "let rec %s in %s"
              (String.concat_map_list ~sep:" and " funcs ~f:(fun (fix, pvar, args, body) ->
                   let var_names =
      ...
      | LetFormula (var, sort, def, phi, _) ->
        aux phi ~priority:Priority.let_forall_exists ~next:(fun s ->
            next @@ Priority.add_paren priority Priority.let_forall_exists @@
            sprintf "letf %s:%s = %s in %s"
              (Ident.name_of_tvar var) (Term.short_name_of_sort sort) (Term.str_of def)
              s)
    in aux ~priority ~next:Fn.id
  ...
  let is_quantifier_free =
    fold ~f:(object
      ...
      method fletrec funcs r1 = List.for_all funcs ~f:(fun (_, _, _, r) -> r) && r1
      method flet _tvar _sort def body = Term.is_quantifier_free def && body
    end)
  ...
  let tvs_of =
    fold ~f:(object
      ...
      method fletrec funcs r1 =
        Set.diff
          (Set.Poly.union_list @@ r1 :: List.map funcs ~f:(fun (_, _, args, body) ->
               Set.diff body (Set.Poly.of_list @@ List.map ~f:fst args)))
          (Set.Poly.of_list @@ List.map funcs ~f:(fun (_, pvar, _, _) -> Ident.pvar_to_tvar pvar))
      method flet tvar _sort def body =
  ...
  let pvs_of =
    fold ~f:(object
      ...
      method fletrec funcs r1 =
        Set.diff
          (Set.Poly.union_list @@
           r1 :: List.map funcs ~f:(fun (_, _, args, body) ->
               Set.diff body
  ...
  let fvs_of phi =
    ...
    Set.Poly.map ~f:Ident.pvar_to_tvar(*ToDo*) @@ pvs_of phi
  let svs_of =
    fold ~f:(object
      ...
      method fletrec funcs r1 =
        Set.Poly.union_list @@
        r1 :: List.map funcs ~f:(fun (_, _, senv, body) ->
            Set.union
              (Set.Poly.of_list @@
               List.filter_map ~f:(function Sort.SVar svar -> Some svar | _ -> None) @@
               List.map ~f:snd senv)
      ...
      method flet _ sort def body =
        Set.Poly.union_list [Term.svs_of def; body; match sort with Sort.SVar svar -> Set.Poly.singleton svar | _ -> Set.Poly.empty]
    end)
  let term_sort_env_of ?(bvs = Set.Poly.empty) phi =
    fold ~f:(object
      ...
      method fbind _ senv r1 bvs = Set.diff (r1 bvs) (Set.Poly.of_list senv)
      method fletrec funcs r1 bvs =
        Set.Poly.union_list @@
        r1 bvs :: List.map funcs ~f:(fun (_, _, args, body) ->
  ...
  let rec pathexps_of ?(bvs = Set.Poly.empty) = function
    ...
    | LetRec (funcs, phi, _) ->
      Set.Poly.union_list @@
      pathexps_of ~bvs phi :: List.map funcs ~f:(fun (_, _, args, body) ->
          let bvs = Set.union bvs (Set.Poly.of_list @@ List.map ~f:fst args) in
  ...
  let rec filtered_terms_of ~f = function
    ...
    | LetRec (funcs, phi, _) ->
      ...
      List.map funcs ~f:(fun (_, _, args, body) ->
          Set.diff (filtered_terms_of ~f body)
  ...
  let rec let_env_of ?(map=Map.Poly.empty) = function
    ...
    | UnaryOp (_, phi, _) ->
      let_env_of phi ~map
    | BinaryOp (_, phi1, phi2, _) ->
      let_env_of phi2 ~map:(let_env_of phi1 ~map)
  ...
  let nesting_level =
    fold ~f:(object
      ...
      method fbind _ _ r1 = r1
      method fletrec funcs r1 =
        Integer.max_list @@ r1 :: List.map funcs ~f:(fun (_, _, _, r) -> 1 + r)
  ...
  let rec number_of_quantifiers = function
    ...
    | LetRec (funcs, body, _) ->
      let nums = List.map funcs ~f:(fun (_, _, _, phi) -> number_of_quantifiers phi) in
      Integer.sum_list @@ number_of_quantifiers body :: nums
    | LetFormula _ -> failwith "[number_of_quantifiers] 'LetFormula' not supported"
  let rec number_of_pvar_apps ?(ex=Map.Poly.empty) is_pos = function
    | Atom (atom, _) -> Atom.number_of_pvar_apps ~ex is_pos atom
    | UnaryOp (Not, phi, _) -> number_of_pvar_apps ~ex (not is_pos) phi
    | BinaryOp (Imply, phi1, phi2, _) ->
      number_of_pvar_apps ~ex (not is_pos) phi1 + number_of_pvar_apps ~ex is_pos phi2
    | BinaryOp ((Iff | Xor), _, _, _) -> assert false
    | BinaryOp ((And | Or), phi1, phi2, _) ->
      number_of_pvar_apps ~ex is_pos phi1 + number_of_pvar_apps ~ex is_pos phi2
  ...
  let rec count_pvar_apps = function
    ...
    | LetRec (_, _, _) -> assert false
    | LetFormula _ -> failwith "[count_pvar_apps] 'LetFormula' not supported yet"
  let rec ast_size = function
    ...
    | LetRec (funcs, phi, _) ->
      1 + (Integer.sum_list @@ List.map funcs ~f:(fun (_, _, _, phi) -> ast_size phi)) + ast_size phi
    | LetFormula (var, sort, def, body, info) ->
      Term.ast_size @@ LetTerm (var, sort, def, T_bool.of_formula body, info)
  let rec occur_times ?(map=Map.Poly.empty) x = function
    | Formula.Atom (atom, _) ->
      Atom.occur_times ~map x atom
    | Formula.BinaryOp (_, phi1, phi2, _) ->
      occur_times ~map x phi1 + occur_times ~map x phi2
    | Formula.UnaryOp (_, phi1, _) ->
      occur_times ~map x phi1
    | Formula.Bind (_, _, phi1, _) ->
      occur_times ~map x phi1
    | Formula.LetRec _ ->
      failwith "[occur_times_in_formula]unsupported letrec"
  ...
  let bind_fvs binder fml =
    let bounds =
      ...
      |> List.map ~f:(fun tvar -> tvar, T_int.SInt(* TODO: infer the sorts *))
    in
  ...
  let quantify_except ?(exists=true) args phi =
    let senv =
      Set.to_list @@ Set.filter ~f:(fst >> Set.mem args >> not) @@
  ...
  let rec rename_pvars map = function
    | Atom (atom, info) ->
      Atom (Atom.rename_pvars map atom, info)
    | UnaryOp (Not, phi, info) ->
      UnaryOp (Not, rename_pvars map phi, info)
    | BinaryOp (op, phi1, phi2, info) ->
      BinaryOp (op, rename_pvars map phi1, rename_pvars map phi2, info)
    | Bind (binder, bounds, body, info) ->
      Bind (binder, bounds, rename_pvars map body, info)
    | LetRec (funcs, body, info) ->
      let map' = Map.remove_keys map (List.map funcs ~f:(fun (_, pvar, _, _) -> pvar)) in
      let funcs = List.map funcs ~f:(fun (fix, pvar, args, body) -> fix, pvar, args, rename_pvars map' body) in
  ...
  let rec rename_sorted_pvars map = function
    | Atom (atom, info) ->
      Atom (Atom.rename_sorted_pvars map atom, info)
    | UnaryOp (Not, phi, info) ->
    ...
    | LetRec (funcs, body, info) ->
      let map' = Map.remove_keys map (List.map funcs ~f:(fun (_, pvar, args, _) -> Ident.name_of_pvar pvar, List.map ~f:snd args)) in
      let funcs = List.map funcs ~f:(fun (fix, pvar, args, body) -> fix, pvar, args, rename_sorted_pvars map' body) in
      LetRec (funcs, rename_sorted_pvars map body, info)
    | LetFormula (var, sort, def, body, info) ->
      of_bool_term @@ Term.rename_sorted_pvars map @@
      LetTerm (var, sort, def, T_bool.of_formula body, info)
  let rec alpha_rename_let ?(map=Map.Poly.empty) = function
    | Atom (atom, info) ->
  ...
  let refresh_except args phi =
    Formula.refresh_tvar
      (Map.of_set_exn @@ Set.filter ~f:(fst >> Set.mem args >> not) @@
       Formula.term_sort_env_of phi, phi)
  (* assume that [phi] is let-normalized *)
  ...
  let rec subst map phi =
    let rec aux ~next map = function
      | Atom (Atom.App (Var (pvar, []), [], _), _)
        ...
        (match Map.Poly.find map (Ident.pvar_to_tvar pvar(*ToDo*)) with
          | Some t -> next @@ of_bool_term @@ if Term.is_var t then t else Term.subst map t
          | None -> next phi)
  ...
  let rec subst_preds map = function
    | Atom (atom, _) ->
      Atom.subst_preds map atom
    | UnaryOp (op, phi, info) ->
      UnaryOp (op, subst_preds map phi, info)
    | BinaryOp (op, phi1, phi2, info) ->
    ...
    | LetRec (funcs, body, info) ->
      let map' = Map.remove_keys map (List.map funcs ~f:(fun (_, pvar, _, _) -> pvar)) in
      let funcs = List.map funcs ~f:(fun (fp, pvar, args, phi) -> fp, pvar, args, subst_preds map' phi) in
  ...
  let rec subst_neg pvar = function
    ...
    | Atom (atom, info) ->
      Atom (Atom.subst_neg pvar atom, info)
    | UnaryOp (Not, phi, info) ->
  ...
  let rec aconv_pvar = function
    ...
    | LetRec (funcs, phi, info) ->
      let map =
        Map.Poly.of_alist_exn @@ List.map funcs ~f:(fun (_, pvar, _, _) ->
  ...
  let complete_psort map = map_atom ~f:(Atom.complete_psort map >> mk_atom)(*ToDo:support LetRec*)
  let rec complete_tsort map = function
  ...
  let rec rm_quant ?(forall=true) = function
    ...
    | Bind (Forall, senv, phi, _) ->
      ...
      else failwith "not supported @ 2"
    | Bind (Exists, senv, phi, _) ->
      if not forall then
        let senv', phi' = rm_quant ~forall phi in
        Set.union (Set.Poly.of_list senv) senv', phi'
      else failwith "not supported @ 3"
  ...
  let elim_pvars unknowns =
    map_atom ~f:(function
        ...
        | Atom.App (pred, args, _) ->
          mk_atom @@ Atom.mk_app pred @@ List.map args ~f:(Term.elim_pvars unknowns)
  ...
  let rec elim_unused_bounds = function
    ...
    | UnaryOp (op, phi1, info) ->
      UnaryOp (op, elim_unused_bounds phi1, info)
    | BinaryOp (op, phi1, phi2, info) ->
  ...
  let rec elim_let_equisat = function
    ...
    | BinaryOp ((Iff | Xor), _phi1, _phi2, _info) as phi ->
      Map.Poly.empty, phi(* assume that phi does not contain a let-expression*)
    | BinaryOp (((And | Or) as op), phi1, phi2, info) ->
  ...
  let elim_let_equi is_forall phi =
    if is_forall then
      let senv, phi = elim_let_equivalid phi in forall (Map.Poly.to_alist senv) phi
    else
      let senv, phi = elim_let_equisat phi in exists (Map.Poly.to_alist senv) phi
  ...
  let rm_div_mod f =
    ...
    if Set.is_empty divmod then f else
      let map =
        Set.to_map divmod ~f:Ident.(fun _ -> mk_fresh_tvar (), mk_fresh_tvar ())
      in
      ...
      let check_zero outerlist innerlist =
        List.fold innerlist ~init:(Formula.mk_true (), outerlist) ~f:(fun (acc, outerlist) a ->
      ...
      let make_restriction mapouter mapinner =
        ...
        let f1 =
          Formula.and_of @@ List.map innerlist ~f:(fun ((e, n), (d, m)) ->
      ...
      let rec divide_map map1 map2 vars =
        ...
        if Map.Poly.is_empty map21 then (map1, map2, vars)
          ...
          let newvars =
            Map.Poly.fold map21 ~init:Set.Poly.empty ~f:(fun ~key:_ ~data:(d, m) newvars -> Set.add (Set.add newvars d) m)
          in
      ...
      let rec add_restriction mapouter mapinner vars = function
        ...
        | BinaryOp (op, phi1, phi2, info) ->
          BinaryOp (op, add_restriction mapouter mapinner vars phi1, add_restriction mapouter mapinner vars phi2, info)
        | Bind (binder, bounds, phi, info) ->
          ...
          let f =
            ...
            Formula.mk_and restriction (add_restriction newmap map2 newvars phi)
          in
  ...
  let rec rm_atom ?(polarity=true) ?(is_and=true) ~f = function
    ...
    | BinaryOp (And, phi1, phi2, info) ->
      BinaryOp (And, rm_atom ~polarity ~is_and:true ~f phi1, rm_atom ~polarity ~is_and:true ~f phi2, info)
    | BinaryOp (Or, phi1, phi2, info) ->
      BinaryOp (Or, rm_atom ~polarity ~is_and:false ~f phi1, rm_atom ~polarity ~is_and:false ~f phi2, info)
  ...
  let rec dnf_of_aux ?(process_pure=false) exi_senv senv = function
    ...
    | Atom (Atom.False _, _) | UnaryOp (Not, Atom (Atom.True _, _), _) ->
      Set.Poly.empty
    | phi when not process_pure && Set.is_empty @@ Set.inter (fvs_of phi) (Map.key_set exi_senv) ->
    ...
    | BinaryOp (Or, phi1, phi2, _) ->
      ...
      let phis, cls =
        ...
        |> Set.partition_tf ~f:(fun (ps, ns, phis) ->
            ...
            Set.is_empty @@ Set.inter (Map.key_set exi_senv) (fvs_of @@ and_of @@ Set.to_list phis))
        |> Pair.map (Set.Poly.map ~f:(fun (_, _, phis) -> and_of @@ Set.to_list phis)) Fn.id
      in
      ...
      else Set.add cls (Set.Poly.empty, Set.Poly.empty, Set.Poly.singleton @@ or_of @@ Set.to_list phis)
    | BinaryOp (Imply, phi1, phi2, _) ->
    ...
    | LetFormula (var, sort, def, body, info) ->
      ...
      Set.Poly.map (dnf_of_aux ~process_pure exi_senv senv' body) ~f:(fun (ps, ns, phis) ->
          ...
          Set.Poly.singleton @@ insert_let_formula var sort def info @@ and_of @@ Set.to_list phis)
  ...
  let dnf_of ?(process_pure=false) exi_senv phi =
    ...
    |> Set.Poly.map ~f:(fun (ps, ns, phis) -> ps, ns, and_of @@ Set.to_list phis)
  let rec cnf_of_aux ?(process_pure=false) exi_senv senv = function
    ...
    | BinaryOp (And, phi1, phi2, _) ->
      let phis, cls =
        ...
        |> Set.partition_map ~f:(fun (ps, ns, phis) ->
            if Set.is_empty ps && Set.is_empty ns &&
               Set.is_empty @@ Set.inter (Map.key_set exi_senv) (fvs_of @@ or_of @@ Set.to_list phis)
    ...
    | BinaryOp (Or, phi1, phi2, _) ->
      ...
      Set.cartesian_map cls1 cls2 ~f:(fun (ps1, ns1, phis1) (ps2, ns2, phis2) ->
          Set.union ps1 ps2, Set.union ns1 ns2, Set.union phis1 phis2)
    | BinaryOp (Imply, phi1, phi2, _) ->
      cnf_of_aux ~process_pure exi_senv senv @@ mk_or (nnf_of @@ mk_neg phi1) (nnf_of phi2)
    | BinaryOp (Iff, phi1, phi2, _) ->
    ...
    | LetFormula (var, sort, def, body, info) ->
      ...
      Set.Poly.map (cnf_of_aux ~process_pure exi_senv senv' body) ~f:(fun (ps, ns, phis) ->
          Set.Poly.map ~f:(Atom.insert_let_pvar_app var sort def info) ps,
          Set.Poly.map ~f:(Atom.insert_let_pvar_app var sort def info) ns,
          Set.Poly.singleton @@ insert_let_formula var sort def info @@ or_of @@ Set.to_list phis)
  ...
  let cnf_of ?(process_pure=false) exi_senv =
    ...
    >> Set.Poly.map ~f:(fun (ps, ns, phis) -> ps, ns, or_of @@ Set.to_list phis)
  let pnf_of f = uncurry mk_binds @@ split_quantifiers f
  let skolemize skolem_pred =
    let rec aux senv fsenv = function
      | Atom (_, _) as phi -> (* assume that phi does not contain quantifiers *)
        senv, fsenv, phi
      | UnaryOp (unop, phi, _) ->
        ...
        if skolem_pred then
          let sub, senv, fsenv, fnpv_apps =
            List.fold ~init:(Map.Poly.empty, senv, fsenv, []) exi_senv
              ~f:(fun (sub, senv, fsenv, fnpv_apps) (Ident.Tvar name, sort) ->
                  let sk_tvar =
                  ...
                  let fnpv_app =
                    Atom.mk_pvar_app pvar sorts @@ Term.of_sort_env args @ [ret]
                  in
          ...
          in
          aux senv fsenv Formula.(mk_imply (and_of fnpv_apps) (subst sub phi))
        else
          let sub, fsenv =
            List.fold ~init:(Map.Poly.empty, fsenv) exi_senv
              ~f:(fun (sub, fsenv) (Ident.Tvar name, sort) ->
                  ...
                  in
                  let sorts = List.map ~f:snd args @ [sort] in
                  let fapp = Term.mk_fvar_app sk_tvar sorts @@ Term.of_sort_env args in
                  Map.Poly.add_exn sub ~key:(Ident.Tvar name) ~data:fapp,
                  Map.Poly.add_exn fsenv ~key:sk_tvar ~data:(Sort.mk_fun sorts))
          in
...
= struct
  ...
  let ifte phi = mk_if_then_else (of_formula phi)
...
= struct
  ...
  let mk_not_pdiv ?(info=Dummy) t1 t2 = Atom.mk_app (Predicate.Psym NotPDiv) [t1; t2] ~info
...
= struct
  ...
  let base_terms_of dt =
    List.map (Datatype.base_conses_of dt)
      ~f:(fun cons -> mk_cons dt cons.name [])
  let size_of_cons t =
    let size = ref 0 in
    ignore @@ Term.map_term true t ~f:(function
        | Term.FunApp (DTCons _, _, _) as t -> incr size; t
        | t -> t);
    !size
  let inst_unknown_sel_term simplify_term =
    Formula.map_term ~f:(simplify_term >> function
       ...
       | Some cons ->
         List.fold2_exn (Datatype.sels_of_cons cons) ts ~init:None
           ~f:(fun ret sel t ->
               ...
               | Some _ -> ret
               | None ->
                 if String.(Datatype.name_of_sel sel = sel_name) then Some t else None)
...
= struct
  ...
  let full_name_of_dt dt =
    String.concat_map_list_append ~sep:", " (params_of_dt dt) ~f:Term.str_of_sort (name_of_dt dt)

  let name_of t = t.name
  let flag_of t = t.flag
  let dts_of t = t.dts
  let look_up_dt t name = List.find (dts_of t) ~f:(name_of_dt >> String.(=) name)
  let dt_of t =
    match look_up_dt t @@ name_of t with
    | Some dt -> dt
    | _ -> failwith @@ sprintf "unknown dt: %s" (name_of t)
  let conses_of t = conses_of_dt @@ dt_of t
  let base_conses_of t = List.filter (conses_of t) ~f:(sels_of_cons >> List.is_empty)
  ...
  let look_up_cons_of_sel t name =
    List.find (conses_of t) ~f:(sels_of_cons >> List.exists ~f:(name_of_sel >> String.(=) name))
  ...
  let look_up_base_cons t =
    let has_direct_base t =
      List.exists (conses_of t) ~f:(sels_of_cons >>
    ...
    List.find (conses_of t) ~f:(fun cons ->
        List.for_all (sels_of_cons cons) ~f:(function
            | Sel _ -> true
            | InSel (_, ret_name, _) ->
              Option.is_some @@ look_up_other_base { t with name = ret_name } [t]))
  let full_name_of t =
    String.concat_map_list_append ~sep:", " (params_of t) ~f:Term.str_of_sort (name_of t)
  let short_name_of t =
    ...
    if List.is_empty params then name
      ...
      sprintf "%s%s"
        (String.paren @@ String.concat_map_list ~sep:"," params ~f:Term.short_name_of_sort)
        name
  let sort_of t =
    ...
    | Alias s -> s
    | _ -> T_dt.SDT t
  let svs_of t = Set.Poly.union_list @@ List.map ((*ToDo*)params_of t) ~f:Term.svs_of_sort
  let evs_of t = Set.Poly.union_list @@ List.map ((*ToDo*)params_of t) ~f:Term.evs_of_sort
  let rvs_of t = Set.Poly.union_list @@ List.map ((*ToDo*)params_of t) ~f:Term.rvs_of_sort
  let pat_match_sort dt1 dt2 =
    if String.(dt1.name = dt2.name) then
      let omaps, smaps, emaps =
        List.unzip3 @@ List.map2_exn (params_of dt1) (params_of dt2) ~f:Term.pat_match_sort
      in
      Map.force_merge_list omaps,
      Map.force_merge_list smaps,
      Map.force_merge_list emaps
  ...
  let rec update_params t params =
    ...
    let omap, smap, emap =
      let omaps, smaps, emaps =
        try
          List.unzip3 @@ List.map2_exn (params_of_dt dt) params ~f:Term.pat_match_sort
        with _ ->
          failwith @@ sprintf "[update_params] %s with {%s}" (full_name_of_dt dt) (String.concat_map_list ~sep:" " ~f:Term.str_of_sort params)
      in
      Map.force_merge_list omaps,
      Map.force_merge_list smaps,
      Map.force_merge_list emaps
            ...
            { cons with
              sels =
                List.map (sels_of_cons cons) ~f:(function
                    | InSel (name, ret_name, params) ->
                      InSel (name, ret_name, List.map params ~f:(Term.subst_sorts_sort smap))
                    | Sel (name, ret_sort) ->
        ...
        List.map (conses_of_dt dt) ~f:(fun cons ->
            { cons with
              sels =
                List.map (sels_of_cons cons) ~f:(function
                    | InSel (name, ret_name, params) ->
                      InSel (name, ret_name, List.map params ~f:(Term.subst_conts_sort emap))
                    | Sel (name, ret_sort) ->
        ...
        List.map (conses_of_dt dt) ~f:(fun cons ->
            { cons with
              sels =
                List.map (sels_of_cons cons) ~f:(function
                    | InSel (name, ret_name, params) ->
                      InSel (name, ret_name, List.map params ~f:(Term.subst_opsigs_sort omap))
                    | Sel (name, ret_sort) ->
        ...
        List.map (conses_of_dt dt) ~f:(fun cons ->
            { cons with
              sels =
                List.map (sels_of_cons cons) ~f:(function
                    | InSel (name, ret_name, params) ->
                      InSel (name, ret_name, List.map params ~f:(Term.subst_sort maps))
                    | Sel (name, ret_sort) ->
  ...
  let subst_opsigs omap dt =
    update_params dt @@ List.map ~f:(Term.subst_opsigs_sort omap) @@ params_of dt
  let subst maps dt =
        ...
        List.map (conses_of_dt dt) ~f:(fun cons ->
            { cons with
              sels =
                List.map (sels_of_cons cons) ~f:(function
                    | InSel (name, ret_name, params) ->
                      InSel (name, ret_name, List.map params ~f:Term.fresh_conts_sort)
                    | Sel (name, ret_sort) ->
        ...
        List.map (conses_of_dt dt) ~f:(fun cons ->
            { cons with
              sels =
                List.map (sels_of_cons cons) ~f:(function
                    | InSel (name, ret_name, params) ->
                      InSel (name, ret_name, List.map params ~f:Term.fresh_rvs_sort)
                    | Sel (name, ret_sort) ->
  ...
  let fresh_of t =
                ...
                let sels' =
                  List.map (sels_of_cons cons) ~f:(function
                      ...
                      | InSel (name, ret_name, params1) ->
                        InSel (name, ret_name, if String.(ret_name = name_of_dt dt) then params else params1)
                      | sel -> sel)
                in
                { cons with sels = sels' }))
      in
    ...
    let dts' =
      List.map (dts_of t) ~f:(fun dt ->
          let params' =
            List.map (params_of_dt dt) ~f:(function Sort.SVar _ -> Sort.mk_fresh_svar () | arg -> arg)
          in
  ...
  let full_dts_of t =
    dts_of @@ List.fold_left (conses_of t) ~init:t ~f:(fun ret cons ->
  ...
  let is_finite t =
    not @@ List.exists (conses_of t) ~f:(fun cons ->
        List.for_all (sorts_of_cons t cons) ~f:(fun arg -> Stdlib.(arg = T_dt.SDT t) && T_dt.is_finite_dt arg))
  let rec is_singleton = function
    | T_dt.SDT t as sort ->
      (match conses_of t with
       | [cons] ->
         List.for_all (sorts_of_cons t cons) ~f:(fun arg -> Stdlib.(arg = sort) || is_singleton arg)
       | _ -> false)
    | _ -> false
  ...
  let fsym_of_sel t sel =
    ...
    | Some (InSel (name, ret_name, params)) ->
      match look_up_dt t ret_name with
      | Some _ ->
        T_dt.DTSel (name, t, sort_of @@ update_params (update_name t ret_name) params)
  ...
  let size_of_cons dt cons =
    1 + (List.length @@ List.filter ~f:(sort_of_sel dt >> T_dt.is_sdt) @@ sels_of_cons cons)
  let used_dt_and_sorts_of dt =
    let rec inner visited dt ret =
      if Set.mem visited dt then (visited, ret)
        ...
        let sorts = Set.Poly.of_list @@ List.concat_map (conses_of dt) ~f:(sorts_of_cons dt) in
        let ret = Set.union ret sorts in
        let dts = Set.Poly.filter_map sorts ~f:(function T_dt.SDT dt -> Some dt | _ -> None) in
        Set.fold dts ~init:(visited, ret) ~f:(fun (visited, ret) dt -> inner visited dt ret)
    in
  ...
  let size_fun_of dt =
    let min_size_of = function
      | T_dt.SDT dt -> Integer.min_list @@ List.map (conses_of dt) ~f:(size_of_cons dt)
      | _ -> 0
    in
  ...
  let mk_option_dt () =
    ...
    add_cons dt (mk_cons "Some" ~sels:[mk_sel (mk_name_of_sel "Some" 0) param])
  let mk_list_dt () =
    let param = Sort.mk_fresh_svar () in
    let dt = make "list" [mk_dt "list" [param]] FDt in
    let dt = add_cons dt @@ mk_cons "[]" in
    add_cons dt @@
    mk_cons "::" ~sels:[mk_sel (mk_name_of_sel "::" 0) param;
...
= struct
  ...
  let mk_is_in_regexp fin regexp t = Atom.mk_psym_app (SeqInRegExp (fin, regexp)) [t]
  let mk_is_not_in_regexp fin regexp t = Atom.mk_psym_app (NotSeqInRegExp (fin, regexp)) [t]
end

and T_regex : Type.T_regexType
  with type term := Term.t
   and type atom := Atom.t
= struct
  type Sort.t += SRegEx
  ...
  let mk_complement ?(info=Dummy) t1 = Term.mk_fsym_app ~info RegComplement [t1]
  ...
  let mk_concat ?(info=Dummy) t1 t2 = Term.mk_fsym_app ~info RegConcat [t1; t2]
  let mk_union ?(info=Dummy) t1 t2 = Term.mk_fsym_app ~info RegUnion [t1; t2]
  let mk_inter ?(info=Dummy) t1 t2 = Term.mk_fsym_app ~info RegInter [t1; t2]

  let mk_str_in_regexp ?(info=Dummy) t1 t2 = Atom.mk_psym_app ~info StrInRegExp [t1; t2]
  let mk_not_str_in_regexp ?(info=Dummy) t1 t2 = Atom.mk_psym_app ~info NotStrInRegExp [t1; t2]
end
        ...
        sprintf
          ...
          (String.concat_map_list ~sep:" " params ~f:(fun (x, s) ->
               String.paren @@ sprintf "%s:%s"
                 (Ident.name_of_tvar x) (Term.str_of_sort s)))
          (Term.str_of def)
          (Formula.str_of prop))
    |> sprintf "fun env:\n%s"
end
...
= struct
  ...
  let str_of t =
    "datatype env:\n  " ^
    String.concat ~sep:"\n  " @@
    Map.Poly.data @@ Map.Poly.mapi t ~f:(fun ~key ~data -> ignore key;(*key ^ ": " ^*) Datatype.str_of data)
