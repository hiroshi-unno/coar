...
let rec eval_term ?(env = Map.Poly.empty) = let open Term in function
    | Var (tvar, _, _) ->
    ...
    | FunApp (FVar (fvar, _), ts, _) as _term ->
      ...
      let params, _, def, _, _ =
        Map.Poly.find_exn (get_fenv ()) fvar
      in
      let env =
        Map.update_with env @@ Map.Poly.of_alist_exn @@
        List.zip_exn (List.map ~f:fst params) @@ List.map ts ~f:(eval_term ~env)
      in
      eval_term ~env def
    ...
    | FunApp (T_bool.IfThenElse, [t1; t2; t3], _) ->
      ...
      if Stdlib.(t2' = t3') then t2'
        ...
        | Value.Bool true -> t2'
        | Value.Bool false -> t3'
        | _ -> failwith "a conditional expression must be evaluated to a boolean value"
      end
    | FunApp (T_int.Int i, [], _) -> Value.Int i
    | FunApp (T_real.Real r, [], _) -> Value.Real r
    | FunApp (T_real_int.ToInt, [FunApp (T_real.Real r, [], _)], _) -> Value.Int (Q.to_bigint r(* ToDo: conforms to the semantics of smtlib? *))
    | FunApp (T_real_int.ToReal, [FunApp (T_int.Int i, [], _)], _) -> Value.Real (Q.of_bigint i)
    | FunApp ((T_int.Neg | T_real.RNeg), [t1], _) -> Value.neg (eval_term ~env t1)
    | FunApp ((T_int.Abs | T_real.RAbs), [t1], _) -> Value.abs (eval_term ~env t1)
    ...
    | LetTerm (var, _, def, body, _) ->
      let subst = Map.Poly.singleton var @@ Term.of_value @@ eval_term ~env def in
...
and eval_pred ?(env = Map.Poly.empty) psym terms =
  ...
  | T_bool.Neq, [t1; t2] ->
    Value.compare Z.Compare.(<>) Q.(<>) ~opb:Stdlib.(<>) t1 t2
  | T_int.PDiv, [t1; t2] ->
    Value.compare Z.Compare.(=) Q.(=) (Value.bmod t2 t1) (Value.Int Z.zero)
  | T_int.NotPDiv, [t1; t2] ->
    Value.compare Z.Compare.(<>) Q.(<>) (Value.bmod t2 t1) (Value.Int Z.zero)
  | _ -> failwith "[eval_pred] not supported"
(*val eval_atom: Atom.t -> bool*)
and eval_atom ?(env = Map.Poly.empty) = let open Atom in
  function
  ...
  | App (Predicate.Psym (T_dt.NotIsCons (name, _)),
         [Term.FunApp (T_dt.DTCons (name1, _, _), _, _)], _) ->
    Stdlib.(name1 <> name)
  | App (Predicate.Psym (T_tuple.IsTuple _),
         [Term.FunApp (T_tuple.TupleCons _, _, _)], _) -> true
  | App (Predicate.Psym (T_tuple.NotIsTuple _),
         [Term.FunApp (T_tuple.TupleCons _, _, _)], _) -> false
  | App (Predicate.Psym psym, terms, _) ->
    eval_pred ~env psym terms
...
and eval ?(env = Map.Poly.empty) = let open Formula in
  function
...
and simplify_term = let open Term in
  function
  | Var (_, sort, _) when Datatype.is_unit_sort sort ->
    Datatype.mk_unit ()
  | Var (tvar, sort, info) ->
    Var (tvar, sort, info)
  ...
  | LetTerm (tvar, sort, def, body, info) ->
    ...
    if size <= size_threshold || count <= count_threshold ||
       size * count <= prod_threshold then
      simplify_term @@ Term.subst (Map.Poly.singleton tvar def') body'
    else LetTerm (tvar, sort, def', body', info)
  | FunApp (fsym, ts, info) ->
    let ts' = List.map ~f:simplify_term ts in
    try of_value @@ eval_term @@ FunApp (fsym, ts', info) with _ ->
    match fsym, ts' with
    | T_bool.Formula phi, [] ->
      ...
      if T_int.is_zero t1 then T_int.zero ()
        ...
        (match t1, t2 with
         | FunApp (T_int.Int n1, [], _), FunApp (T_int.Int n2, [], _)
           when Z.Compare.(n2 <> Z.zero) -> FunApp (T_int.Int Z.(ediv n1 n2), [], info)
         | _, _ -> FunApp (fsym, [t1; t2], info))
    | T_real.RDiv, [t1; t2] ->
      if T_real.is_rzero t1 then T_real.rzero ()
        ...
        (match t1, t2 with
         | FunApp (T_real.Real r1, [], _), FunApp (T_real.Real r2, [], _)
           when Q.(r2 <> zero) -> FunApp (T_real.Real Q.(div r1 r2), [], info)
         | _, _ -> FunApp (fsym, [t1; t2], info))
    | T_int.Mod, [t1; t2] ->
      if T_int.is_zero t1 then T_int.zero ()
        ...
        (match t1, t2 with
         | FunApp (T_int.Int n1, [], _), FunApp (T_int.Int n2, [], _)
           when Z.Compare.(n2 <> Z.zero) -> FunApp (T_int.Int Z.(erem n1 n2), [], info)
         | _, _ -> FunApp (fsym, [t1; t2], info))
    | T_ref.Deref _, [t] ->
    ...
    | T_regex.RegComplement, [FunApp (T_regex.RegFull, [], _); _] ->
      T_regex.mk_empty ()
    | T_regex.RegComplement, [FunApp (T_regex.RegComplement, [t1], _); _] ->
      t1
    ...
    | T_regex.RegConcat, [t1; FunApp (T_regex.RegEpsilon, [], _)] ->
      t1
    | fsym, ts -> FunApp (fsym, ts, info) (* including T_int.int, T_real.Real *)
    ...
    | App (pred, terms, info) ->
      ...
      | Predicate.Psym T_bool.Eq, [t1; t2] when Term.is_bool_sort @@ Term.sort_of t1 -> (*ToDo*)
        if can_simplify t1 && can_simplify t2 then
          ...
          simplify @@ Formula.mk_or
            (Formula.mk_and p1 p2)
            (Formula.mk_and (Formula.mk_neg p1) (Formula.mk_neg p2)) ~info
      ...
      | Predicate.Psym T_bool.Neq, [t1; t2] when Term.is_bool_sort @@ Term.sort_of t1 -> (*ToDo*)
        if can_simplify t1 && can_simplify t2 then
          ...
          simplify @@ Formula.mk_or
            ...
            (Formula.mk_and p1 (Formula.mk_neg p2)) ~info
      ...
      | Predicate.Psym (T_dt.IsCons (name, dt)), [t] when Datatype.is_base dt name ->
        Formula.eq t (T_dt.mk_cons dt name [])
      | Predicate.Psym (T_dt.NotIsCons (name, _)),
        [Term.FunApp (T_dt.DTCons (name1, _, _), _, _)] ->
        Formula.mk_bool @@ String.(name1 <> name)
      | Predicate.Psym (T_dt.NotIsCons (name, dt)), [t] when Datatype.is_base dt name ->
      ...
      | Predicate.Psym (T_tuple.IsTuple _), [Term.FunApp (T_tuple.TupleCons _, _, _)] ->
        Formula.mk_true ()
      | Predicate.Psym (T_tuple.NotIsTuple _), [Term.FunApp (T_tuple.TupleCons _, _, _)] ->
        Formula.mk_false ()
      | Predicate.Psym T_bool.Eq,
      ...
      | Predicate.Psym T_bool.Eq, [Term.FunApp (T_tuple.TupleCons sorts, ts, _); t]
      | Predicate.Psym T_bool.Eq, [t; Term.FunApp (T_tuple.TupleCons sorts, ts, _)] ->
        simplify @@ Formula.and_of @@
        List.mapi ts ~f:(fun i ti -> Formula.eq ti @@ T_tuple.mk_tuple_sel sorts t i)
      | Predicate.Psym T_bool.Neq, [Term.FunApp (T_tuple.TupleCons sorts, ts, _); t]
      | Predicate.Psym T_bool.Neq, [t; Term.FunApp (T_tuple.TupleCons sorts, ts, _)] ->
        simplify @@ Formula.or_of @@
        List.mapi ts ~f:(fun i ti -> Formula.neq ti @@ T_tuple.mk_tuple_sel sorts t i)
...
and simplify_atom_neg atom = let open Atom in
  try begin match atom with
    ...
    | App (Predicate.Var (pvar, sorts), args, _info) ->
      ...
      Atom.mk_pvar_app pvar sorts @@ List.map ~f:simplify_term args
    | App (pred, args, info) ->
      simplify_atom @@
...
and simplify_andor_norec op phi1 phi2 info = let open Formula in
  ...
  | And,
    ...
    when Stdlib.(x11 = x12 && x21 = x22) &&
         T_bool.is_true t11 && T_bool.is_true t21 &&
         T_bool.is_true t12 && T_bool.is_true t22 -> (* ToDo: generalize *)
...
and simplify ?(next=Fn.id) = let open Formula in function
    | Atom (atom, _) ->
      next @@ simplify_atom atom
    | UnaryOp (Not, phi, _) ->
      simplify_neg phi ~next
    ...
    | BinaryOp (Iff, phi1, phi2, info) ->
      ...
      simplify phi1 ~next:(fun phi1' -> simplify phi2 ~next:(fun phi2' ->
    ...
    | LetFormula (var, sort, def, body, info) ->
      ...
      simplify body ~next:(fun body' ->
          ...
          if size <= size_threshold || count <= count_threshold ||
             size * count <= prod_threshold then
            simplify (Formula.apply_pred (var, body') def') ~next
...
and simplify_neg ?(next=Fn.id) = let open Formula in function
    ...
    | BinaryOp (And, phi1, phi2, info) ->
      simplify_neg phi1 ~next:(fun phi1' -> simplify_neg phi2 ~next:(fun phi2' ->
...
let rec local_is_valid is_valid = function
  ...
  | Formula.Bind (Formula.Forall , _, phi, _) ->
    local_is_valid is_valid phi
  | phi -> is_valid phi
(* cons x y = tl z*)
