...
let count_pvar_apps cls =
  ...
  List.map (Set.to_list pvs) ~f:(fun pvar ->
      ...
      List.map (Set.to_list cls) ~f:(fun ((_, ps, ns, _) : Clause.t) ->
          let pc = Set.count ps ~f:(fun t -> Set.mem (Term.fvs_of t) pvar) in
          let nc = Set.count ns ~f:(fun t -> Set.mem (Term.fvs_of t) pvar) in
...
let resolve_one_step_all ~print positive negative exi_senv =
  Set.concat_map ~f:(Clause.resolve_one_step_all ~print positive negative exi_senv)
...
let pred_of_pos exi_senv cs tvar =
  ...
  | [] ->
    (try
     ...
     with _ ->
       failwith @@ sprintf "[pred_of_pos] %s not found" (Ident.name_of_tvar tvar))
  | (env, phi) :: ps ->
    env,
    BoolTerm.or_of @@
    phi :: List.map ps ~f:(fun (env', phi') ->
        let sub =
          ...
          List.map2_exn env' env ~f:(fun (x, _) (y, _) -> x, Term.mk_var y)
        in
        Term.subst sub phi')
...
let pred_of_neg exi_senv cs tvar =
  ...
  | [] ->
    (try
     ...
     with _ ->
       failwith @@ sprintf "[pred_of_neg] %s not found" (Ident.name_of_tvar tvar))
  | (env, phi) :: ps ->
    env,
    BoolTerm.and_of @@
    phi :: List.map ps ~f:(fun (env', phi') ->
  ...
  | [] -> begin
      ...
      | [] ->
        (try
           ...
           BoolTerm.mk_bool true
         with _ ->
           failwith @@ sprintf "[pred_of] %s not found" (Ident.name_of_tvar tvar))
      | (env, phi) :: ps ->
        env,
        BoolTerm.and_of @@
        phi :: List.map ps ~f:(fun (env', phi') ->
...
let partial_sols_of ~print is_valid exi_senv sol lbs clauses0 target_pvars ignored =
  let clauses =
    simplify exi_senv @@ subst exi_senv
  ...
  let pvars =
    Set.diff
      (Set.Poly.map pvars0 ~f:(fun pvar ->
           Set.find_exn cl_pvars ~f:(fun cl_pvar -> Ident.is_related_tvar cl_pvar pvar)))
      ignored
  in
  print @@ lazy ("pvars: " ^ String.concat_map_set ~sep:"," pvars ~f:Ident.name_of_tvar);
  let lbs =
    Map.rename_keys (Map.Poly.of_alist_exn @@ Hashtbl.Poly.to_alist lbs)
      ~f:(fun pvar -> Set.find cl_pvars ~f:(fun cl_pvar -> Ident.is_related_tvar cl_pvar pvar))
  in
  ...
  let table =
    Map.of_set_exn @@ Set.Poly.map pvars ~f:(fun pvar ->
        ...
        print @@ lazy
          (sprintf "pred: %s.%s"
             (Logic.str_of_sort_env_list Logic.ExtTerm.str_of_sort uni_senv)
        ...
        let left =
          ...
          Logic.ExtTerm.to_old_formula Map.Poly.empty (Map.Poly.of_alist_exn uni_senv)
            (Map.Poly.find_exn sol pvar) (List.map uni_senv ~f:(fst >> ExtTerm.mk_var))
        in
        let check_cond2a pvs =
          ...
          let right =
            Normalizer.normalize @@ Evaluator.simplify @@
            Logic.ExtTerm.to_old_formula Map.Poly.empty (Map.Poly.of_alist_exn uni_senv) pred []
          in
        ...
        print @@ lazy
          (sprintf "SAT2a(%s)={%s}" (Ident.name_of_tvar pvar) @@
           String.concat_map_set ~sep:"," pvss ~f:(fun pvs ->
               sprintf "{%s}" @@ String.concat_map_set ~sep:"," pvs ~f:Ident.name_of_tvar));
        pvar, Set.Poly.map pvss ~f:(Set.diff pvs_def))
  in
  let rec inner not_psol =
    let added =
      Set.filter (Set.diff pvars not_psol)
        ~f:(Map.Poly.find_exn table >> Set.(for_all ~f:(inter not_psol >> Fn.non is_empty)))
    in
