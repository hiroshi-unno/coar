...
type t = (Logic.SortMap.t * Atom.t Set.Poly.t * Atom.t Set.Poly.t * Formula.t)
...
let simplify npfvs is_valid (senv, pos, neg, phi) =
  ...
  if Set.Poly.is_empty (Set.Poly.inter pos neg) then
    ...
    if Set.Poly.is_empty @@ Set.Poly.inter npfvs fvs_phi then
  ...
  Set.Poly.filter_map (if Stdlib.(=) mode `Forward then c_neg else c_pos) ~f:(fun papp' ->
      ...
      let c_pos' =
        Set.Poly.map (if Stdlib.(=) mode `Backward then Set.Poly.remove c_pos papp' else c_pos) ~f:(fun atm ->
            Atom.subst theta atm |> Normalizer.normalize_atom) in
      let c_neg' =
        Set.Poly.map (if Stdlib.(=) mode `Forward then Set.Poly.remove c_neg papp' else c_neg) ~f:(fun atm ->
            Atom.subst theta atm |> Normalizer.normalize_atom) in
      let c_pure' = Formula.subst theta c_pure |> Evaluator.simplify in
...
let is_only_pure (pos, neg, _) = Set.Poly.is_empty pos && Set.Poly.is_empty neg
...
let refresh_tvar (senv, ps, ns, phi) =
  (*let map =
    ...
    |> Util.Map.of_set
    in*)
  let map = Map.Poly.of_alist_exn @@ Logic.SortMap.to_alist @@ Logic.SortMap.map senv ~f:(fun _ -> Ident.mk_fresh_tvar()) in
  Logic.SortMap.rename_keys_map map senv,
...
let reduce_sort_map ((senv, ps, ns, phi) : t) =
  let fvs = Set.Poly.inter (Logic.SortMap.key_set senv) (fvs_of (senv, ps, ns, phi)) in
  Logic.SortMap.of_set @@ Set.Poly.map ~f:(fun x -> x, Logic.SortMap.find_exn senv x) fvs, ps, ns, phi
