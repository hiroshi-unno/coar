...
let simplify_with positive negative (c_pos, c_neg, c_pure) =
  if Set.is_empty (Set.inter positive c_pos) &&
...
let rec separate vs phis =
  let phis1, phis2 = Set.partition_tf phis ~f:(fun (fvs, _) ->
      Set.is_empty @@ Set.inter fvs vs) in
...
let simplify unknowns is_valid (senv, pos, neg, phi) =
  ...
  let phis =
    Set.Poly.map (Formula.disjuncts_of phi) ~f:(fun phi -> Formula.fvs_of phi, phi)
  in
  if Set.is_empty (Set.inter pos neg) then
    ...
    if Set.is_empty @@ Set.inter unknowns fvs_phi then
      ...
      if is_valid (Formula.or_of @@ Set.to_list (Set.Poly.map ~f:snd phis1)) then
        None
      else
        Some (senv, pos, neg,
...
let resolve_one_step_all positive negative c =
  let cs_b = Set.Poly.map negative ~f:(fun neg -> resolve_one_step `Backward neg c) in
  let cs_f = Set.Poly.map positive ~f:(fun pos -> resolve_one_step `Forward pos c) in
...
let is_query wfpvs fnpvs (pos, _, _) =
  Set.for_all pos ~f:(fun atm ->
      Atom.is_pvar_app atm && let p = Atom.pvar_of atm in Set.mem wfpvs p ||
                                                          Set.mem fnpvs p)
...
let refresh_tvar (senv, ps, ns, phi) =
  ...
  Set.Poly.map ~f:(Atom.rename map) ps, Set.Poly.map ~f:(Atom.rename map) ns,
...
let reduce_sort_map ((senv, ps, ns, phi) : t) =
  ...
  Map.of_set_exn @@ Set.Poly.map fvs ~f:(fun x -> x, Map.Poly.find_exn senv x),
  ps, ns, phi
        ...
        | Atom.App (Predicate.Var (pvar, _), _, _) as atom ->
          (match Map.Poly.find sub pvar with
           ...
           | Some (params, phi_pos, phi_neg, keep) ->
             let psub =
               Map.Poly.singleton pvar (params, if is_pos then phi_pos else phi_neg)
             in
