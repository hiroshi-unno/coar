...
let to_old_formula exi_senv (uni_senv, ps, ns, phi) =
  ExtTerm.to_old_formula exi_senv uni_senv (to_formula (uni_senv, ps, ns, phi)) []
...
let pvs_of ((_, ps, ns, _) : t) =
  Set.union (Set.Poly.map ps ~f:Term.pvar_of_atom) (Set.Poly.map ns ~f:Term.pvar_of_atom)
let fvs_of ((_, ps, ns, phi) : t) =
  Set.Poly.union_list
    [Term.fvs_of phi; Set.concat_map ~f:Term.fvs_of ps; Set.concat_map ~f:Term.fvs_of ns]
  ...
  | Some atm ->
    ...
    let _, _, phi' =
      ...
      (Map.force_merge map uni_senv, [],
       BoolTerm.or_of @@ phi :: neqs @ List.map ~f:BoolTerm.neg_of @@ Set.to_list neg)
    in
    tvar, (env, BoolTerm.(exists (Map.Poly.to_alist uni_senv) @@ nnf_of @@ neg_of phi'))
...
let pred_of_co_definite exi_senv (uni_senv, pos, neg, phi) =
  ...
  | Some atm ->
    ...
    let _, _, phi' =
      Qelim.qelim map exi_senv @@
      (Map.force_merge map uni_senv, [], BoolTerm.or_of @@ phi :: neqs @ Set.to_list pos)
    in
...
let simplify_with positive negative ((senv, c_pos, c_neg, c_phi) : t) =
  ...
  if Set.is_empty (Set.inter positive c_pos) && Set.is_empty (Set.inter negative c_neg)
  then Some (senv, Set.diff c_pos negative, Set.diff c_neg positive, c_phi) else None
...
let resolve_one_step_all ~print positive negative exi_senv (c : t) =
  let cs_b =
    Set.Poly.map negative ~f:(fun neg -> resolve_one_step ~print `Backward neg exi_senv c)
  in
  let cs_f =
    Set.Poly.map positive ~f:(fun pos -> resolve_one_step ~print `Forward pos exi_senv c)
  in
...
let is_query wfpvs fnpvs ((_, pos, _, _) : t) =
  Set.for_all pos ~f:(fun atm ->
      let p = BoolTerm.pvar_of_atom atm in Set.mem wfpvs p || Set.mem fnpvs p)

let refresh_pvar_args exi_senv ((senv, ps, ns, phi) : t) =
  let ps', pres =
    Set.unzip @@ Set.Poly.map ps ~f:(fun atm ->
        let pvar, args = Term.let_var_app atm in
        let env =
          mk_fresh_sort_env_list @@ Sort.args_of @@ Map.Poly.find_exn exi_senv pvar
        in
        Term.mk_var_app pvar @@ List.map env ~f:(fst >> Term.mk_var),
        (env,
         List.map2_exn args env ~f:(fun arg (x, s) -> Term.mk_apps (BoolTerm.mk_neq s) [Term.mk_var x; arg])))
  in
  let xss1, pneqss = Set.unzip pres in
  let ns', nres =
    Set.unzip @@ Set.Poly.map ns ~f:(fun atm ->
        let pvar, args = Term.let_var_app atm in
        let env =
          mk_fresh_sort_env_list @@ Sort.args_of @@ Map.Poly.find_exn exi_senv pvar
        in
  ...
  let senv' =
    Map.force_merge senv
      (Map.of_set_exn @@ Set.concat_map ~f:Set.Poly.of_list (Set.union xss1 xss2))
  in
  senv', ps', ns',
  BoolTerm.or_of @@
  phi :: List.concat (Set.to_list pneqss) @ List.concat (Set.to_list nneqss)
...
let refresh_tvar ((senv, ps, ns, phi) : t) =
  ...
  Set.Poly.map ~f:(Term.rename map) ps, Set.Poly.map ~f:(Term.rename map) ns,
  Term.rename map phi

let fvs_new (_, ps, ns, phi) =
  Set.Poly.union_list @@
  ExtTerm.fvs_of phi ::
  List.map (Set.to_list ps) ~f:ExtTerm.fvs_of @
  List.map (Set.to_list ns) ~f:ExtTerm.fvs_of
let reduce_sort_map (senv, ps, ns, phi) =
  let ftvs = fvs_new (senv, ps, ns, phi) in
  Map.Poly.filter_keys senv ~f:(Set.mem ftvs),
  ps, ns, phi
...
let equal (c1 : sort_env_map * term Set.Poly.t * term Set.Poly.t * term) c2 =
  ...
  Set.equal ps1 ps2 && Set.equal ns1 ns2 && Stdlib.(phi1 = phi2)

let normalize_uni_senv ((senv, ps, ns, phi) : t) =
  (*Debug.print @@ lazy "a";*)
  let vars =
    List.map ~f:fst @@ List.sort ~compare:(curry @@ function
      | (_, Some n1), (_, Some n2) -> Int.compare n1 n2
      | (v1, _), (v2, _) -> String.compare (Ident.name_of_tvar v1) (Ident.name_of_tvar v2)) @@
    let reg_number = Str.regexp "[^0-9]*" in
    List.map (Map.Poly.keys senv) ~f:(fun x ->
        x,
        try Some (Int.of_string @@ Str.global_replace reg_number "" @@ Ident.name_of_tvar x)
        with _ -> None)
  in
  (*Debug.print @@ lazy "b";*)
  let sub =
...
let normalize_uni_senv_formula (senv, phi) =
  let senv, _, _,phi = normalize_uni_senv (senv, Set.Poly.empty, Set.Poly.empty, phi) in
  senv, phi
let ast_size_of ((_, ps, ns, phi) : t) =
  Set.fold ps ~init:0 ~f:(fun acc t -> 1 + acc + ExtTerm.ast_size t) + 1 +
  Set.fold ns ~init:0 ~f:(fun acc t -> 1 + acc + ExtTerm.ast_size t) + 1 +
  ExtTerm.ast_size phi

let must_be_satisfied ?(print=fun _ -> ())
    target_pvars pvars_the_target_pvar_depends_on clause =
  ...
  print @@ lazy
    (sprintf "check: [%s] <= [%s]"
       (Ident.str_of_tvars ~sep:"," pvars_of_the_clause_related_to_target_pvars)
  ...
  Set.is_subset pvars_of_the_clause_related_to_target_pvars ~of_:pvars_the_target_pvar_depends_on
