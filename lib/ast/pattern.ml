...
let rec str_of ?(with_sort = false) = function
  | PAny sort ->
    "_" ^ if with_sort then " : " ^ Term.str_of_sort sort else ""
  | PVar (tvar, sort) ->
...
let rec tvars_of = function
  ...
  | PTuple pats | PCons (_, _, pats) -> Set.Poly.union_list @@ List.map pats ~f:tvars_of
...
let rec term_of = function
  ...
  | PTuple ps -> T_tuple.mk_tuple_cons (List.map ps ~f:sort_of) @@ List.map ps ~f:term_of
...
let rec of_term = function
  ...
  | Term.FunApp (T_tuple.TupleCons _, ts, _) -> mk_ptuple @@ List.map ts ~f:of_term
  | Term.FunApp (T_dt.DTCons (cons, _, dt), ts, _) -> mk_pcons dt cons @@ List.map ts ~f:of_term
...
let rec match_with pat term =
  ...
  | PCons (dt, cons_name, pats) -> (*ToDo*)
    Map.force_merge_list @@
    List.mapi pats ~f:(fun i pi ->
        match_with pi @@
        T_dt.mk_sel_by_cons dt cons_name i term)
