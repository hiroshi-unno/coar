...
module NonLinear = struct
  ...
  let div_by_gcd m =
    ...
    | c :: cs ->
      let lcm_ = List.fold ~init:(Z.abs @@ Value.int_of c) cs ~f:(fun c1 c2 -> Z.gcd c1 (Z.abs @@ Value.int_of c2)) in
...
module Linear = struct
  ...
  let mk_int_term monomials =
    if Map.is_empty monomials then T_int.zero ()
      ...
      let l = Map.Poly.fold monomials ~init:[] ~f:(fun ~key:term ~data:c l ->
          match term with
    ...
    Map.Poly.fold monomials ~init:Map.Poly.empty
      ~f:(fun ~key:op ~data:value map ->
          ...
          | Some (Term.Var (tvar, T_int.SInt, _)) ->
            Map.Poly.add_exn map ~key:(Some (Term.mk_var tvar T_real.SReal)) ~data:value
          | None -> Map.Poly.add_exn map ~key:None ~data:value
          | _ -> assert false)
  let real_to_int monomials =
    let lcm = Q.of_bigint @@ Map.Poly.fold monomials ~init:Z.one
        ~f:(fun ~key:_ ~data:value n -> Z.lcm n @@ Q.den @@ Value.real_of value) in
    Map.Poly.fold (real_mult monomials (Value.Real lcm)) ~init:Map.Poly.empty
      ~f:(fun ~key:op ~data:value map ->
          ...
          | Some (Term.Var (tvar, T_real.SReal, _)) ->
            Map.Poly.add_exn map ~key:(Some (Term.mk_var tvar T_int.SInt)) ~data:value
          | None -> Map.Poly.add_exn map ~key:None ~data:value
          | _ -> assert false)
  ...
  let subst_eqterm cterm eqterm tvar atom monomials_of =
    ...
    | Atom.App (Predicate.Psym ((T_bool.Eq | T_bool.Neq) as sym), [t; t0], _) ->
      (match find_var_monomial (Term.mk_var tvar SInt) @@
         monomials_of (Value.Int Z.one) t with
      | Some (a, rest) ->
        let term =
          mk_add (mk_mult (Term.of_value a) eqterm) (mk_mult cterm (mk_int_term rest))
        in
