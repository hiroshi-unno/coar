...
type t = Ord | FN | NE | WF | NWF of nwf * (Ident.tvar * Ident.tvar) | IntFun | RegEx
...
let kind_map_of_nwf nwf =
  ...
  Map.Poly.of_alist_exn @@ List.concat_map tags ~f:(fun (tag_l, _) ->
...
let app_nwf_predicate nwf params (tag_l, params_l) (tag_r, params_r) =
  ...
  assert (List.length sorts_l = List.length params_l &&
...
let app_nwf_predicate_old nwf params (tag_l, params_l) (tag_r, params_r) =
  ...
  let uni_senv =
    ...
    |> List.map ~f:LogicOld.Term.sort_env_of |> Set.Poly.union_list |> Set.to_list
    |> List.map ~f:(fun (v, s) -> v, ExtTerm.of_old_sort s) |> Map.Poly.of_alist_exn
  in
  wfp, ExtTerm.to_old_sort psort,
  ExtTerm.to_old_formula (Map.Poly.singleton wfp psort) uni_senv term []
let app_nwf_predicate_and_add_tag
  ...
  Set.fold tvs ~init:kind_map ~f:(fun acc key  ->
      if Map.Poly.mem acc key then
        ...
        if Stdlib.(kind' = kind) then acc
          ...
          failwith @@ sprintf "%s is defined as %s and %s"
            (Ident.name_of_tvar key) (str_of kind) (str_of kind')
