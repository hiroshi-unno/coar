...
let sign model atom =
  ...
  if Evaluator.eval_atom @@ Atom.subst model atom then atom else Atom.negate atom
...
let rec atoms_of model = function
  ...
  | BinaryOp (And, phi1, phi2, _) -> Set.union (atoms_of model phi1) (atoms_of model phi2)
  | BinaryOp (Or, phi1, phi2, _) ->
    let s1 = if Evaluator.eval @@ Formula.subst model phi1 then atoms_of model phi1 else Set.Poly.empty in
    let s2 = if Evaluator.eval @@ Formula.subst model phi2 then atoms_of model phi2 else Set.Poly.empty in
    Set.union s1 s2
  | _phi -> raise NotNormalized(*failwith (Formula.str_of phi ^ " is not atomic formula")*)
let normalize_mbp model atm =
  Set.concat @@
  Set.Poly.map ~f:(fun atm ->
      Set.Poly.map ~f:Normalizer.normalize_atom @@
      atoms_of model @@ Evaluator.simplify_atom atm) @@
  Set.Poly.map ~f:Normalizer.normalize_atom @@
  atoms_of model @@ Evaluator.simplify_atom atm
...
module LRA = struct
  let let_sort names =
    Map.Poly.fold ~init:Map.Poly.empty ~f:(fun ~key:tvar ~data:term model ->
        if Set.mem names (Ident.name_of_tvar tvar) then model(*ToDo*)
          ...
          | Value.Bool b -> Map.Poly.add_exn model ~key:tvar ~data:(T_bool.make b)
          | Value.Int n -> Map.Poly.add_exn model ~key:tvar ~data:(T_real.mk_real (Q.of_bigint n))(*ToDo*)
          | Value.Real r -> Map.Poly.add_exn model ~key:tvar ~data:(T_real.mk_real r))
  ...
  let ub_and_lb tvar =
    Set.fold ~init:(Set.Poly.empty, Set.Poly.empty, Set.Poly.empty)
      ~f:(fun (ub, lb, others) -> function
          | Atom.App (Predicate.Psym (T_real.(RGeq | RGt(*ToDo*))), [t; _], _) as atom ->
            let varterm = Term.mk_var tvar T_real.SReal in
            (match Linear.find_var_monomial varterm @@
               linear_real_monomials_of (Value.Real Q.one) t with
            | Some (Real r, rest) ->
              ...
              if Q.(r < zero) then
                ...
                Map.Poly.map rest ~f:(Fn.flip Value.div (Value.neg (Real r))),
                lb,
                others
  ...
  let lub model ub =
    fst @@ Set.fold ub ~init:(None, Q.inf) ~f:(fun (lub, lub_val) t ->
        match Evaluator.eval_term @@ Term.subst model t with
        | Real r -> if Q.(r < lub_val) then (Some t, r) else (lub, lub_val)
        | _ -> failwith "lub")
  let glb model lb =
    fst @@ Set.fold lb ~init:(None, Q.minus_inf) ~f:(fun (glb, glb_val) t ->
        match Evaluator.eval_term @@ Term.subst model t with
  ...
  let model_based_projection model tvar atoms =
    ...
    | Some t ->
      ...
      Set.concat @@ Set.Poly.map atoms ~f:(Atom.subst sub >> normalize_mbp model)
    | None ->
      ...
      | Some lub, Some glb ->
        let sub =
          ...
          T_real.mk_rmult (T_real.mk_radd lub glb) (T_real.mk_real (Q.of_float 0.5))
        in
        Set.concat @@ Set.Poly.map rest ~f:(Atom.subst sub >> normalize_mbp model)
      | Some lub, None ->
        let sub = Map.Poly.singleton tvar @@ T_real.mk_rsub lub (T_real.rone ()) in
        Set.concat @@ Set.Poly.map rest ~f:(Atom.subst sub >> normalize_mbp model)
      | None, Some glb ->
        let sub = Map.Poly.singleton tvar @@ T_real.mk_radd glb (T_real.rone ()) in
        Set.concat @@ Set.Poly.map rest ~f:(Atom.subst sub >> normalize_mbp model)
      | None, None ->
        if Set.exists rest ~f:(fun atom -> Set.mem (Atom.fvs_of atom) tvar)
        then raise NotNormalized (*failwith @@ "no constraint on " ^ Ident.name_of_tvar tvar*)
...
module LIA = struct
  let let_sort names =
    Map.Poly.fold ~init:Map.Poly.empty ~f:(fun ~key:tvar ~data:term model ->
        if Set.mem names (Ident.name_of_tvar tvar) then model(*ToDo*)
          ...
          match Evaluator.eval_term term with
          | Value.Bool b -> Map.Poly.add_exn model ~key:tvar ~data:(T_bool.make b)
          | Value.Int n -> Map.Poly.add_exn model ~key:tvar ~data:(T_int.mk_int n)
          | Value.Real _ -> failwith "LIA")

  let is_pdiv atom tvar =
    match atom with
    | Atom.App (Predicate.Psym ((T_int.PDiv | T_int.NotPDiv) as psym), [d; t], _) ->
      (match Linear.find_var_monomial (Term.mk_var tvar T_int.SInt) @@
  ...
  let rm_pdiv pdivs model tvar =
    ...
    let u_val = Evaluator.eval_term @@ T_int.mk_mod varmodel (T_int.mk_int lcm) in
    let u = Term.of_value u_val in
    let newtvar = Ident.mk_fresh_tvar () in
    let newterm =
      T_int.mk_add u (T_int.mk_mult (T_int.mk_int lcm) (Term.mk_var newtvar T_int.SInt))
    in
    let newmodel =
      let newvalue =
    ...
    let pdivs =
      Set.Poly.map pdivs ~f:(fun (psym, d, c, t) ->
          match psym with
          | T_int.PDiv ->
            Normalizer.normalize_atom @@ Atom.mk_app (Predicate.Psym psym) @@
            [Term.of_value d;
             T_int.mk_add (T_int.mk_int Z.(mul (Value.int_of c) (Value.int_of u_val)))
  ...
  let ub_and_lb tvar =
    Set.fold ~init:(Set.Poly.empty, Set.Poly.empty, Set.Poly.empty)
      ~f:(fun (ub, lb, others) -> function
          | Atom.App (Predicate.Psym T_int.Geq, [t; _], _) as atom ->
            ...
            | Some (Int n, rest) ->
              ...
              if Z.Compare.(n < Z.zero) then
                Set.add ub (Z.(-n), Linear.mk_int_term rest),
                lb,
                others
              else if Z.Compare.(n > Z.zero) then
                ub,
                Set.add lb (n, Linear.mk_int_term @@
                            Map.Poly.map rest ~f:(Fn.flip Value.mult (Int Z.minus_one))),
                others
              else failwith "ub_and_lb"
            | _ -> ub, lb, Set.add others atom)
          | atom -> ub, lb, Set.add others atom)
  let lub model ub =
    fst @@ Set.fold ub ~init:(None, Q.inf) ~f:(fun (lub, lub_val) (c, t) ->
  ...
  let glb model lb =
    fst @@ Set.fold lb ~init:(None, Q.minus_inf) ~f:(fun (glb, glb_val) (c, t) ->
        let tv = Evaluator.eval_term @@ Term.subst model t in
        let v = Q.make (Value.int_of tv) c in
  ...
  let model_based_projection_sub model tvar atoms =
    ...
    (*if Set.mem (Set.concat_map ~f:Formula.fvs_of rest) tvar then
      ...
      Set.concat @@ Set.Poly.map atoms ~f:(Atom.subst sub >> Formula.mk_atom)
      else*)
    match lub model ub, glb model lb with
    | Some (cub, tub), Some (clb, tlb) ->
      let lbformulas =
        Set.Poly.map lb ~f:(fun (c, t) ->
            T_int.mk_leq (T_int.mk_mult t @@ T_int.mk_int clb) (T_int.mk_mult tlb @@ T_int.mk_int c))
      in
      let ubformulas =
        Set.concat_map ub ~f:(fun (c, t) ->
            resolve model clb tlb c t)
      in
      let sub =
        let glb = T_int.mk_div tlb @@ T_int.mk_int clb in
        let lub = T_int.mk_add (T_int.mk_div tub @@ T_int.mk_int cub) (T_int.one ()) in
        Map.Poly.singleton tvar
          (T_int.mk_div (T_int.mk_add glb lub) (T_int.mk_int (Z.of_int 2)))
      in
      (*print_endline @@ "lbformulas: " ^ List.to_string ~f:str_of lbformulas;
        print_endline @@ "ubformulas: " ^ List.to_string ~f:str_of ubformulas;*)
      Set.concat @@ Set.Poly.map ~f:(normalize_mbp model) @@
      Set.Poly.union_list [lbformulas; ubformulas; Set.Poly.map rest ~f:(Atom.subst sub)]
    | Some (cub, tub), None ->
      let sub = Map.Poly.singleton tvar @@ T_int.mk_div tub @@ T_int.mk_int cub in
      Set.concat @@ Set.Poly.map rest ~f:(Atom.subst sub >> normalize_mbp model)
    | None, Some (clb, tlb) ->
      let sub = Map.Poly.singleton tvar @@ T_int.mk_add (T_int.mk_div tlb @@ T_int.mk_int clb) (T_int.one ()) in
      Set.concat @@ Set.Poly.map rest ~f:(Atom.subst sub >> normalize_mbp model)
    | None, None ->
      if Set.exists rest ~f:(fun atom -> Set.mem (Atom.fvs_of atom) tvar)
      then raise NotNormalized (*failwith @@ "no constraint on " ^ Ident.name_of_tvar tvar*)
  ...
  let rec model_based_projection model tvar atoms =
    ...
    | None ->
      ...
      let atoms = elim_neq atoms in*)
      let pdivs, rest =
        Set.fold atoms ~init:(Set.Poly.empty, Set.Poly.empty) ~f:(fun (pdivs, rest) atom ->
      ...
      if Set.is_empty pdivs then model_based_projection_sub model tvar rest
        ...
        let pdivs = Set.concat @@ Set.Poly.map pdivs ~f:(normalize_mbp model) in
        Set.union pdivs @@
