...
let rank_of phi =
  let max_const = Set.Poly.max_elt @@ Set.Poly.filter_map ~f:(function T_int.Int i -> Some (Z.abs i) | _ -> None) @@ Formula.funsyms_of phi in
  let num_vars = Set.Poly.length @@ Formula.tvs_of phi in
...
let compare_aux x y =
  ...
  | None, Some _ -> -1
  | Some _, None -> 1
  | Some x, Some y -> if Z.Compare.(x < y) then -1 else if Z.Compare.(x > y) then 1 else 0
...
let str_of (x1, x2, x3) =
  match x1 with
  | Some x1 -> sprintf "(%s, %d, %d)" (Z.to_string x1) x2 x3
  | None -> sprintf "(_, %d, %d)" x2 x3
