  ...
  let eqcs, phis =
    List.partition_map phis ~f:(fun phi ->
        if BoolTerm.is_not phi then
          ...
          | TyApp (Con (BoolTerm.Eq, _), ty, _), [Var (x1, _); Var (x2, _)] -> First (Set.Poly.of_list [x1; x2], ty)
          | _ -> Second phi
        else
          match Term.let_apps phi with
          | TyApp (Con (BoolTerm.Neq, _), ty, _), [Var (x1, _); Var (x2, _)] -> First (Set.Poly.of_list [x1; x2], ty)
          | _ -> Second phi)
  in
  let eqcs' =
        ...
        | (bv :: bvs') as bvs, fvs -> begin
            ...
            | None -> begin
                match fvs with
                  ...
                  [],
                  List.map (fvs @ bvs') ~f:(fun x -> BoolTerm.neq_of ty tt (Term.mk_var x))
                | fv :: fvs' ->
                  let tt = Term.mk_var fv in
                  List.map fvs' ~f:(fun x -> x, (tt, ty)),
...
let elim_consts bounds (uni_senv, defs, phi) =
  ...
  if List.contains_dup ~compare:Stdlib.compare @@ List.map ~f:fst ttsub then
    ...
    uni_senv, List.map defs ~f:(Term.subst sub), Term.subst sub @@ BoolTerm.or_of phis1
...
let rec qelim_aux1 let_bounds bounds exi_senv (uni_senv, defs, phi) =
  let simplify (uni_senv, defs, body) =
    let body' =
      ExtTerm.simplify_formula exi_senv (Map.force_merge let_bounds uni_senv) body
    in
  ...
  let res =
    ...
    | None ->
      List.filter_map (BoolTerm.disjuncts_of phi) ~f:(fun phi ->
          match List.map ~f:(consts_of Map.Poly.empty) @@ BoolTerm.conjuncts_of @@ BoolTerm.nnf_of phi with
          | [First (x1, sym1, BoolTerm.SBool); First (x2, sym2, BoolTerm.SBool)] ->
            Some (x1, sym1, x2, sym2)
          | _ -> None)
      |> List.find_distinct_pair ~f:(fun (x11, sym11, x12, sym12) (x21, sym21, x22, sym22) ->
          if (Stdlib.(x11 = x21) && Stdlib.(x12 = x22) && Stdlib.(sym11 <> sym21) ||
              Stdlib.(x11 = x22) && Stdlib.(x12 = x21) && Stdlib.(sym11 <> sym22)) &&
             Stdlib.(x11 <> x12) && Stdlib.(sym11 = sym12) && Stdlib.(sym21 = sym22) then
            if not (Map.Poly.mem bounds' x11) && not (Map.Poly.mem let_bounds x12) then
              Some (x11, BoolTerm.neg_of @@ Term.mk_var x12)
            else if not (Map.Poly.mem bounds' x12) && not (Map.Poly.mem let_bounds x11) then
              Some (x12, BoolTerm.neg_of @@ Term.mk_var x11)
            else None
          else if (Stdlib.(x11 = x21) && Stdlib.(x12 = x22) && Stdlib.(sym11 <> sym21) ||
                   Stdlib.(x11 = x22) && Stdlib.(x12 = x21) && Stdlib.(sym11 <> sym22)) &&
                  Stdlib.(x11 <> x12) && Stdlib.(sym11 <> sym12) && Stdlib.(sym21 <> sym22) then
            if not (Map.Poly.mem bounds' x11) && not (Map.Poly.mem let_bounds x12) then
              Some (x11, Term.mk_var x12)
            else if not (Map.Poly.mem bounds' x12) && not (Map.Poly.mem let_bounds x11) then
              Some (x12, Term.mk_var x11)
            else None
          else None)
  in
  match res with
...
let rec qelim ?(let_bounds=Map.Poly.empty) bounds exi_senv (uni_senv, defs, phi) =
  match phi with
  | Let (var, sort, def, body, info) -> begin
      let let_bounds' = Map.Poly.set ~key:var ~data:sort let_bounds in
      match qelim ~let_bounds:let_bounds' bounds exi_senv (Map.Poly.set uni_senv ~key:var ~data:sort, def :: defs, body) with
      | _, [], _ -> assert false
      | uni_senv_body, def' :: defs', body' ->
        ...
        if Set.mem fvs_body var then
          ...
          in
          Map.force_merge uni_senv_def uni_senv_body', defs', Let (var, sort, def'', body', info)
...
let qelim_old bounds exi_senv (uni_senv, phi) =
  ...
  in
  uni_senv', ExtTerm.to_old_formula exi_senv uni_senv' phi' []
