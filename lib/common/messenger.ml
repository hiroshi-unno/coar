...
let update_max_client t max_client = { t with max_client = max_client }
...
let iter messenger ~f =
  let rec inner i infos =
    if i <= 0 then ()
      ...
      | Some msg -> f msg; send messenger msg; inner (i - 1) infos
      | None -> ()
  in
...
let map messenger ~f =
  let rec inner i infos =
    if i <= 0 then ()
      ...
      match receive messenger with
      | Some msg -> send messenger (f msg); inner (i - 1) infos
      | None -> ()
  in
  inner (Atomic.get messenger.length)
  ...
  let rec inner i rets =
    if i <= 0 then rets
      ...
      | Some msg ->
        ...
        else begin send messenger msg; inner (i - 1) rets end
      | None -> rets
  in
  inner (Atomic.get messenger.length) []
  ...
  let rec inner i =
    if i <= 0 then None
      ...
      | Some msg ->
        ...
        else begin send messenger msg; inner (i - 1) end
      | None -> None
  in
  inner (Atomic.get messenger.length)
...
let receive_boardcast messenger my_id =
  match receive messenger with
  | Some (Broadcast (src_id, received, info)) when
      Stdlib.(src_id <> my_id) && Fn.non (Hash_set.mem received) my_id ->
    ...
    Some (src_id, info)
  | Some msg -> send messenger msg; None
  | None -> None
let receive_all_infos_with messenger my_id ~filter =
  let rec inner i infos =
    if i <= 0 then infos
      ...
      match receive messenger with
      | Some (Broadcast (src_id, received, info)) when
          Stdlib.(src_id <> my_id) && not @@ Hash_set.mem received my_id && filter info ->
        Hash_set.add received my_id;
        (if Hash_set.length received < messenger.max_client - 1 then
           send messenger @@ Broadcast (src_id, received, info));
        inner (i - 1) @@ (src_id, info) :: infos
      | Some (Request (src_id, dst_id, info)) when Stdlib.(my_id = dst_id) && filter info ->
        inner (i - 1) @@ (src_id, info) :: infos
      | Some (Response (src_id, dst_id, info)) when Stdlib.(my_id = dst_id) && filter info ->
        inner (i - 1) @@ (src_id, info) :: infos
      | Some msg -> send messenger msg; inner (i - 1) infos
      | None -> infos
  in
  inner (Atomic.get messenger.length) []
...
let wait_request ?(src=None) messenger dst ~f =
  ...
  while Option.is_none !request do
    request := find messenger
        ~f:(function
            | Request (src0, dst0, info) when
                Stdlib.(dst0 = dst) && f info -> begin
                match src with
                | Some src -> Stdlib.(src0 = src)
                | _ -> true
              end
...
let exec_request messenger (src, dst, info) =
  match info with
  | Terminate -> send messenger (mk_response dst src Terminate); raise Terminated
  | Sleep s -> send messenger (mk_response dst src (Sleep s)); Core_unix.sleep s
  | Pause ->
    send messenger (mk_response dst src Pause);
    ignore @@ wait_request messenger ~src:(Some src) dst ~f:(function Resume -> true | _ -> false);
...
let wait_response messenger src dst ~f =
  while Option.is_none @@ find messenger ~f:(function
      | Response (src0, dst0, info) as message when
          Stdlib.(src0 = dst) && Stdlib.(dst0 = src) && f info ->
        MDebug.print @@ lazy ("** receive " ^ str_of_message message); true
      | _ -> false) do ignore @@ Core_unix.nanosleep 0.1 done
