...
module type SolverType = sig
  val solve : ?timeout:int option -> ?bpvs:(Ast.Ident.tvar Set.Poly.t) -> ?print_sol:bool ->
    ?preds:((Ident.pvar, sort_env_list * Formula.t) Map.Poly.t) ->
    ?copreds:((Ident.pvar, sort_env_list * Formula.t) Map.Poly.t) ->
    PCSP.Problem.t -> (PCSP.Problem.solution * int) Or_error.t
  val preprocess : ?bpvs:Ast.Ident.tvar Set.Poly.t -> PCSP.Problem.t -> PCSP.Problem.t Or_error.t
  val reset : unit -> unit
  val push : unit -> unit
  val pop : unit -> unit
    ...
    if Map.Poly.is_empty preds && Map.Poly.is_empty copreds then
      ...
      Map.Poly.iteri preds ~f:(fun ~key:pvar ~data:(senv, phi) ->
          let sub =
            ...
            List.mapi senv ~f:(fun i (x, _) -> x, Ident.Tvar (sprintf "x%d" i))
          in
          let senv = List.map senv ~f:(fun (x, s) -> Map.Poly.find_exn sub x, s) in
  ...
  let solve ?(timeout=None) ?(bpvs=Set.Poly.empty) ?(print_sol=false) ?(preds=Map.Poly.empty) ?(copreds=Map.Poly.empty) pcsp =
    ...
    | Some tm when tm > 0 ->
      Timer.enable_timeout tm Fn.id ignore
        ...
        (fun _ res -> res)
        (fun _ -> function Timer.Timeout -> Ok (PCSP.Problem.Unknown, -1) | e -> raise e)
    | _ -> solve ~bpvs ~print_sol ~preds ~copreds pcsp
  let preprocess =
    match solver with
    | SPCSat (module PCSat) -> fun ?(bpvs=Set.Poly.empty) pcsp -> PCSat.preprocess ~bpvs pcsp
    | _ -> fun ?bpvs _ -> ignore bpvs; failwith "preprocess unsupported (need apply pcsat)"


  let print =
    match solver with
    | SPrinter (module Printer) ->
      fun pcsp -> Printer.string_of_pcsp pcsp
    | _ -> fun _ -> failwith "print function expects printer config"

  let reset () =
    match solver with
    | SPCSat (module PCSat) -> PCSat.reset ()
    | _ -> ()
  let push () =
    match solver with
    | SPCSat (module PCSat) -> PCSat.push ()
    | _ -> ()
  let pop () =
    match solver with
    | SPCSat (module PCSat) -> PCSat.pop ()
    | _ -> ()
end

let make (config : Config.t) = (module Make(struct let config = config end) : SolverType)
