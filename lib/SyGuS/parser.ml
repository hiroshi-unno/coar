...
let count = ref 0
let get_var () = incr count; Ident.Tvar (sprintf "INV_%d" !count)
...
module IntTerm : TermType = struct
  ...
  let term_of_op op args = match op with
    ...
    | text ->
      ...
      with Invalid_argument err -> failwith ("[term_of_op] " ^ op ^ " not supported\n" ^ err)
end
...
module Make (Term : TermType) (ProblemTerm : Problem.TermType) : sig
  val parse_fundef : term_subst_map -> sort_env_map -> Sexp.t -> Ident.tvar * term
  val parse_term : Problem.Make(ProblemTerm).t -> term_subst_map -> sort_env_map -> Sexp.t -> term
  ...
  let rec parse_bfTerm = function
    ...
    | Sexp.List (Sexp.Atom id :: bfTerms) ->
      begin
        ...
        | Some term ->
          List.map ~f:parse_bfTerm bfTerms |> CFG.mk_bfTerm_fun (CFG.mk_identifier term)
        | None -> failwith "unknown style of bfTerm"
      end
  ...
  let parse_cfg (symbol_def : Sexp.t list) grammer_def =
    ...
    let grammer =
      List.fold ~init:Map.Poly.empty
        ~f:(fun acc (sym, rule) -> Map.Poly.add_exn acc ~key:sym ~data:rule) @@
  ...
  let parse_synth_fun problem name (args : Sexp.t list) (sort : Sort.t) (grammer : Sexp.t list) : Problem.t =
    let cfg =
      match grammer with
      | [] -> None
    ...
    in
    let sort = List.fold_right ~init:sort args ~f:(parse_arg >> Sort.mk_arrow) in
      ...
      begin
        ...
        | Some (_sort, _cfg) ->
          List.fold ~init:(mk_var (Ident.Tvar op)) args
            ~f:(fun acc -> parse_term problem fenv venv >> mk_app acc)
        | None ->
          match Map.Poly.find fenv (Ident.Tvar op) with
          | Some term -> (* if the term is an interpreted variable, substitute it  *)
            beta_reduction term (List.map args ~f:(parse_term problem fenv venv))
          | None ->
            match op with
            | "+" -> sum (List.map ~f:(parse_term problem fenv venv) args)
            | "*" -> prod (List.map ~f:(parse_term problem fenv venv) args)
            | "and" -> and_of (List.map ~f:(parse_term problem fenv venv) args)
            | "or" -> or_of (List.map ~f:(parse_term problem fenv venv) args)
            | "distinct" ->
              and_of @@ Set.to_list @@ Set.fold_distinct_pairs ~init:Set.Poly.empty ~f:(fun acc x y -> Set.add acc @@ neq_of SInt x y) @@
              Set.Poly.of_list @@ List.map args ~f:(parse_term problem fenv venv)
            | "let" -> begin
                match args with
            ...
            | _ ->
              ...
              | Some x ->
                List.fold ~init:x args ~f:(fun acc -> parse_term problem fenv venv >> mk_app acc) (* if the operator is defined in the theory *)
  ...
  let parse_inv_constraint problem fenv inv_f pre_f trans_f post_f =
    ...
    let mk_atom pvar var_list =
      ...
      | None ->
        List.fold var_list ~init:(mk_var pvar) ~f:(fun acc -> fst >> mk_var >> mk_app acc)
    in
    ...
    List.fold ~init:problem ~f:(Problem.add_declared_var >> uncurry) @@ (var_list @ var_list'),
  ...
  let add_args_to_venv venv =
    List.fold ~init:venv ~f:(fun acc (var, sort) -> Map.Poly.update acc var ~f:(fun _ -> sort))
  ...
  let parse_fundef fenv venv = function
    | Sexp.List [Sexp.Atom "define-fun"; Sexp.Atom name; Sexp.List args; Sexp.Atom _sort; def] ->
      ...
      Ident.Tvar name, mk_lambda args @@
      parse_term (Problem.mk_problem Map.Poly.empty Map.Poly.empty []) fenv venv' def
    | _ -> failwith @@ sprintf "sygus function definition should start from 'define-fun'"
  ...
  let parse_smt_cmd problem fenv venv = function
    ...
    | Sexp.List [Sexp.Atom "define-sort"] -> failwith "define-sort is not supported yet"
    | Sexp.List [Sexp.Atom "set-info"] -> failwith "not implemented yet"
    | Sexp.List [Sexp.Atom "set-option"] -> failwith "not implemented yet"
    | Sexp.List (Sexp.Atom cmd :: _) -> failwith @@ sprintf "unknown smt cmd : %s" cmd
  ...
  let rec parse_cmds problem fenv (venv : sort_env_map) = function
    ...
    | Sexp.List (Sexp.Atom "constraint" :: constr :: []) :: es ->
      let problem' = Problem.add_constraint problem @@ parse_term problem fenv venv constr in
    ...
    | Sexp.List [Sexp.Atom "inv-constraint"; Sexp.Atom inv_f; Sexp.Atom pre_f; Sexp.Atom trans_f; Sexp.Atom post_f] :: es ->
      let inv_f, pre_f, trans_f, post_f =
        Ident.Tvar inv_f, Ident.Tvar pre_f, Ident.Tvar trans_f, Ident.Tvar post_f
      in
      let problem, constrs = parse_inv_constraint problem fenv inv_f pre_f trans_f post_f in
      (*print_endline @@ sprintf "[parse_inv_constraint]\n%s"
         (String.concat_map_list ~sep:"\n" constrs ~f:(fun t -> "*) " ^ str_of t));*)
      let problem' = List.fold ~init:problem constrs ~f:Problem.add_constraint in
      parse_cmds problem' fenv venv es
    ...
    | Sexp.List (Sexp.Atom "synth-fun" :: Sexp.Atom name :: Sexp.List args :: Sexp.Atom sort :: grammer_def) :: es ->
      let problem' =
        parse_synth_fun problem (Ident.Tvar name) args (Term.sort_of_text sort) grammer_def
      in
      parse_cmds problem' fenv venv es
    | Sexp.List (Sexp.Atom "synth-inv" :: Sexp.Atom name :: Sexp.List args :: grammer_def) :: es ->
      let problem' = parse_synth_fun problem (Ident.Tvar name) args SBool grammer_def in
      parse_cmds problem' fenv venv es
    | cmd :: es ->
  ...
  let of_problem =
    parse_cmds (Problem.mk_problem Map.Poly.empty Map.Poly.empty [])
      Map.Poly.empty Map.Poly.empty
end

let parse = function
  | Sexp.List [Sexp.Atom "set-logic"; Sexp.Atom logic] :: es -> begin
  ...
  | _ ->
    Result.fail @@
    Error.of_thunk (fun () -> sprintf "sygus problem file should start from 'set-logic'")
...
let solution_from_file filename =
  try
    ...
    | Ok sexps ->
      let module Parser = Make (ExtTerm) (Problem.ExtTerm) in
      Result.return @@ List.map sexps ~f:(Parser.parse_fundef Map.Poly.empty Map.Poly.empty)
    | Error err ->
      Result.fail @@ Error.of_thunk (fun () ->
          let pos = Parse_error.position err in
          let msg = Parse_error.message err in
