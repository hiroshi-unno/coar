...
module CFG (Term : TermType) : sig
  ...
  type bfTerm = Id of identifier | Lit of Logic.term | Fun of identifier * bfTerm list
  type gTerm = Constant of Logic.Sort.t | Variable of Logic.Sort.t | BfTerm of bfTerm
...
module Make (Term : TermType) : sig
  ...
  val find_synth_fun_of: t -> Ident.tvar -> (Logic.Sort.t * CFG(Term).t option) option
  val str_of: t -> string
end = struct
  module CFG = CFG(Term)
