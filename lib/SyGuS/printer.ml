...
let rec str_of_formula = function
  ...
  | Formula.Bind (Forall, params, phi, _) ->
    sprintf "(forall (%s) %s)"
      (str_of_sort_env_list str_of_sort params) (str_of_formula phi)
...
and str_of_atom = function
  ...
  | Atom.App (pred, args, _) ->
    if List.length args = 0 then str_of_pred pred
      ...
      sprintf "(%s %s)"
        (str_of_pred pred) (String.concat_map_list ~sep:" " ~f:str_of_term args)
...
and str_of_term = function
  ...
  | Term.FunApp (T_bool.IfThenElse, [cond; then_; else_], _) ->
    sprintf "(ite %s %s %s)" (str_of_term cond) (str_of_term then_) (str_of_term else_)
  | Term.FunApp (T_int.Int n, [], _) ->
    (match mode with
     | SyGuS_IF1 -> Z.to_string n
     | SyGuS_IF2 ->
       Z.(if Compare.(<) n zero then "(- " ^ to_string (-n) ^ ")" else to_string n))
  ...
  if Fn.non Map.Poly.is_empty params && Set.is_empty sol then assert false
    ...
    Set.Poly.map sol ~f:(fun (Ident.Pvar ident, (params, formula)) ->
        sprintf "(define-fun %s (%s) Bool %s)"
          ident (str_of_sort_env_list str_of_sort params) @@
        str_of_formula @@
        Formula.elim_neq(* T_bool.Neq is not supported by SyGuS-IF *) @@
        Z3Smt.Z3interface.simplify ~id:None Map.Poly.empty(* TODO *) @@
        Evaluator.simplify formula)
