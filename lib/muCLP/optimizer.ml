...
module Config = struct
  ...
  let load_ext_file = function
    ...
    | Filename filename ->
      ...
      | Error msg ->
        error_string @@ sprintf "Invalid Optimizer Configuration (%s): %s" filename msg
end

module type OptimizerType = sig
  val f: ?id:int option -> ?elim_forall:bool -> ?elim_exists:bool -> ?elim_pvar_args:bool -> Problem.t -> Problem.t
end
...
module Make (Config: Config.ConfigType) = struct
  ...
  module EraseUnreachedPredicates : sig
    ...
    let pvargraph_of_muclp muclp =
      ...
      |> List.map ~f:(Formula.pvs_of >> Set.Poly.map ~f:(fun pvar ->
          try Hashtbl.Poly.find_exn pvar_to_id pvar
          with _ -> failwith @@ "not found: " ^ Ident.name_of_pvar pvar) >> Set.to_list)
      |> Array.of_list
    ...
    let optimize muclp =
      let reachable = Array.to_list @@ reachable_from 0 @@ pvargraph_of_muclp muclp in
      let preds, query = Problem.let_muclp muclp in
      let preds =
...
*)
  ...
  module InlineExtension : sig
    ...
    let get_pvar_called_counts pvar_to_id preds query =
      ...
      List.iter (query :: List.map preds ~f:Pred.body_of)
        ~f:(Formula.iter_atom ~f:(function
            ...
            Formula.map_atom body ~f:(function
                | Atom.App (Predicate.Var (pvar, _), args, _) as atm ->
                  ...
                  if called_counts.(pred_id') = 1 && pred_id' > pred_id then begin
                    ...
                    let subst =
                      Map.Poly.of_alist_exn @@ List.zip_exn (List.map ~f:fst args') args
                    in
                    Formula.subst subst body
                  end else Formula.mk_atom atm
                | atm -> Formula.mk_atom atm)
          in
          preds.(pred_id) <- Pred.make fix pvar args body);
      let preds =
        Array.to_list preds
        |> List.filter ~f:(fun pred -> not expanded.(pvar_to_id @@ Pred.pvar_of pred))
      in
      Problem.make preds query
  end

  module EraseQuantifiers : sig
    val optimize: ?elim_forall:bool -> ?elim_exists:bool -> Problem.t -> Problem.t
  end = struct
    let seed = 1007
  ...
  *)
    let rec adj_vars_of_term = function
      ...
      *)
      ...
      | FunApp ((T_int.Int _
                ...
                | T_bool.Formula (Formula.Atom (Atom.False _, _))), [], _) ->
        Set.Poly.empty
    ...
    let check_separatable avfvs =
      ...
      List.iter tvars ~f:(fun tvar ->
          if Hashtbl.Poly.find_exn fv_count tvar = 1 then Queue.enqueue queue tvar);
      while Fn.non Queue.is_empty queue do
        let tvar = Queue.dequeue_exn queue in
        if Hashtbl.Poly.find_exn fv_count tvar = 1 then
          Hashtbl.find_multi indexes_of_ftv tvar
          |> List.iter ~f:(fun idx ->
              if not is_assigned.(idx) && Set.mem (fst avfvs.(idx)) tvar then begin
                is_assigned.(idx) <- true;
                Set.iter (snd avfvs.(idx)) ~f:(fun tvar' ->
    ...
    let bound_flags_of_app_opt bounds args =
      ...
      let avs = List.map ~f:adj_vars_of_term args |> List.map ~f:(Set.inter bounds) in
      let fvs = List.map ~f:Term.tvs_of args |> List.map ~f:(Set.inter bounds) in
      ...
      if List.exists ~f:(fun (av, fv) -> Fn.non Set.is_empty fv && Set.is_empty av) avfvs then
        (* if FV(t) and bounds != {} /\ AV(t) and bounds = {} for some arg t *)
        None
      else if check_separatable (List.filter ~f:(snd >> Fn.non Set.is_empty) avfvs) then
      (*
          ...
          let search_for_dual_formulas fmls ht =
            List.fold_left ~init:None (List.zip_index fmls) ~f:(fun res (fml, i) ->
                if Option.is_none res then
                  if Set.is_empty @@ SimpleFormula.get_fpv fml then (* TODO: to be more efficient *)
                    let fmls = get_formulas inner_op @@ SimpleFormula.neg fml in
    ...
    let optimize ?(elim_forall=true) ?(elim_exists=true) muclp =
      ...
      let startpvar = if Array.is_empty preds then endpvar else Pred.pvar_of preds.(0) in
      let ht_of_binder = function
        | Formula.Forall -> pvar_to_fpvar | Exists -> pvar_to_epvar | _ -> assert false
      in
      ...
      Array.iteri preds ~f:(fun i pred ->
          ...
          let nxtpvar =
            if i = Array.length preds - 1 then endpvar else Pred.pvar_of preds.(i+1)
          in
      ...
      let add_pred binder bound_flags pvar =
        ...
        let pvar, pvar', pred =
          let fix, pvar, bounds, body = Pred.let_ @@ Hashtbl.find_exn pvar_to_pred pvar in
          let pvar' = Ident.mk_fresh_pvar () in (* TODO *)
          Debug.print @@ lazy
            (sprintf "add_pred (pvar): %s. %s"
               (Formula.str_of_binder binder) (Ident.name_of_pvar pvar'));
          let qbounds, bounds' =
            List.zip_exn bounds bound_flags
            |> List.partition_map ~f:(fun (b, f) -> if f then First b else Second b)
          in
          pvar, pvar',
          Pred.make fix pvar' bounds' @@ Formula.mk_bind_if_bounded binder qbounds body
        in
      ...
      let get_or_add_pred binder bound_flags pvar =
        ...
        let ht' =
          ...
          Hashtbl.Poly.find_or_add ~default:(fun _ -> Hashtbl.Poly.create ()) ht pvar
        in
      ...
      let binder_of_op = function
        | Formula.Forall -> Formula.And | Formula.Exists -> Formula.Or | _ -> assert false
      in
        ...
        *)
          ...
          *)
            ...
            let fml2 =
              ...
              |> erase_quantifiers_rep_bind binder bounds
            in
            SimpleFormula.mk_branch_with_simplification_one (Formula.flip_binop op) [fml1; fml2]
          else
          (*
            exists a. P(a) /\ Q
              -> (exists a. P(a)) /\ Q
          ...
          *)
            ...
            Hashtbl.Poly.iter ht ~f:(fun ids ->
                ...
                Array.iteri ids ~f:(fun i _ ->
                    if i+1 < Array.length ids then UnionFind.unite ids.(i) ids.(i+1) uf));
          ...
          *)
          ...
          Debug.print @@ lazy
            (sprintf "erase_quantifiers_rep_bind(is_app): " ^ SimpleFormula.string_of fml);
          match bound_flags_of_app_opt bounds args with
          | Some bound_flags ->
            let pvar' = get_or_add_pred binder bound_flags pvar in
            let args =
              List.zip_exn args bound_flags |> List.filter ~f:(snd >> not) |> List.map ~f:fst
            in
      ...
      and erase_quantifiers_rep fml =
        if SimpleFormula.is_and fml then
          ...
          let binder, bounds, fml' = SimpleFormula.let_bind fml in
          if Stdlib.(elim_forall && binder = Formula.Forall ||
                     elim_exists && binder = Formula.Exists) then
      ...
      while Stdlib.(!current_pvar <> endpvar) do
        Queue.enqueue preds_queue @@ Hashtbl.Poly.find_exn pvar_to_pred !current_pvar;
  ...
  module CheckAndReplaceToTopOrBot : sig
    ...
    let gen_smt_solver timeout_milliseconds =
      let ctx = Z3.mk_context ["timeout", string_of_int timeout_milliseconds] in
      Z3.Solver.mk_simple_solver ctx, ctx

    let check_always_true ?(timeout_milliseconds=1000) fml =
    ...
    let rec replace_to_topbot = function
      ...
      | Formula.Bind (binder, bounds, fml, info) as phi ->
        if check_always_true phi then Formula.mk_true ()
        else if check_always_true (Evaluator.simplify_neg phi) then Formula.mk_false ()
        else Formula.mk_bind binder bounds ~info @@ replace_to_topbot fml
      | _ -> failwith "replace_to_topbot"

    let optimize = Problem.map_formula replace_to_topbot
  end
  ...
  module EraseConstVariables : sig
    ...
    let merge_vt vt1 vt2 =
      ...
      | ConstReal x, ConstReal y -> if Q.(equal x y) then ConstReal x else NonConst
      | ConstBool x, ConstBool y -> if Stdlib.(x = y) then ConstBool x else NonConst
      | _ -> NonConst
    ...
    let vt_of_pt ht_vt (pvar, tvar) = Hashtbl.Poly.(find_exn (find_exn ht_vt pvar) tvar)
    ...
    let init_ht_vt muclp =
      ...
      List.iter (Problem.preds_of muclp) ~f:(fun (_, pvar, bounds, _) ->
          let ht = Hashtbl.Poly.create ~size:seed () in
          List.iter bounds ~f:(fun (tvar, _) -> Hashtbl.Poly.set ht ~key:tvar ~data:UnReached);
      ...
      | Term.FunApp (funsym, args, _) ->
        ...
        if List.exists ~f:(Stdlib.(=) NonConst) arg_vts then NonConst
          ...
          (match funsym, arg_vts with
           | T_bool.Formula (Formula.Atom (Atom.True _, _)), [] -> ConstBool true
           | T_bool.Formula (Formula.Atom (Atom.False _, _)), [] -> ConstBool false
           ...
           | T_int.Power, [ConstInt x; ConstInt y] -> ConstInt (Z.pow x (Z.to_int(*ToDo*) y))
           | T_real.Real x, [] -> ConstReal x
           | T_real.RNeg, [ConstReal x] -> ConstReal (Q.neg x)
           | T_real.RAbs, [ConstReal x] -> ConstReal (Q.abs x)
           | T_real.RAdd, [ConstReal x; ConstReal y] -> ConstReal (Q.(+) x y)
           | T_real.RSub, [ConstReal x; ConstReal y] -> ConstReal (Q.(-) x y)
           | T_real.RMult, [ConstReal x; ConstReal y] -> ConstReal (Q.( * ) x y)
           | T_real.RDiv, [ConstReal x; ConstReal y] -> ConstReal (Q.(/) x y)
           | _ ->
             failwith @@
             "EraseConstVariables.vt_of_term: illegal funapp " ^ Term.str_of_funsym funsym)
    ...
    let rec get_vargraph_rep (tvars_of_pvar: Ident.pvar -> Ident.tvar list)
      ...
      | Formula.BinaryOp (_, fml1, fml2, _) ->
        let consts, nonconsts =
          get_vargraph_rep tvars_of_pvar pvar arg_tvars consts nonconsts graph fml1
        in
      ...
      | Formula.Atom (Atom.App (Predicate.Var (pvar', _), args, _), _) ->
        ...
        |> List.fold_left ~init:(consts, nonconsts) ~f:(fun (consts, nonconsts) (arg, tvar') ->
            ...
            if Set.is_empty ftv then
            ...
            else if Set.is_empty @@ Set.diff ftv arg_tvars then begin
              ...
              consts, nonconsts
            end else consts, (pvar', tvar') :: nonconsts)
      | Formula.Atom (_, _) -> consts, nonconsts
      | Formula.Bind (_, bounds, fml, _) ->
        let arg_tvars = Set.diff arg_tvars @@ Set.Poly.of_list @@ List.map ~f:fst bounds in
        get_vargraph_rep tvars_of_pvar pvar arg_tvars consts nonconsts graph fml
      | _ -> failwith "get_vargraph_rep"

    let tvars_of_pvar_of_muclp muclp =
      let tvars_of_pvar_ht = Hashtbl.Poly.create ~size:seed () in
      List.iter (fst @@ Problem.let_muclp muclp) ~f:(fun (_, pvar, bounds, _) ->
          Hashtbl.Poly.set tvars_of_pvar_ht ~key:pvar ~data:(List.map ~f:fst bounds));
    ...
    let gen_ht_vt muclp =
      ...
      while Fn.non Queue.is_empty queue do
        ...
        if Stdlib.(vt_of_pt ht_vt pt <> NonConst) then
          List.iter (Hashtbl.Poly.find_exn graph pt) ~f:(fun (nxt_pvar, nxt_tvar, term) ->
              let nxt_pt = nxt_pvar, nxt_tvar in
              ...
              if Stdlib.(current_vt <> NonConst) then
                ...
                | ConstBool _ | ConstInt _ | ConstReal _ as con -> update_ht_vt nxt_pt con
                | UnReached -> ());
      done;
      ht_vt
      ...
      Formula.map_atom ~f:(function
          | Atom.App (Predicate.Var (pvar, sorts), args, info) ->
            ...
            let args, sorts =
              ...
              |> List.filter ~f:(snd >> Hashtbl.Poly.find_exn ht_vt_of_tvar >> is_const >> not)
        ...
        List.map preds ~f:(fun (fix, pvar, bounds, fml) ->
            ...
            let subst =
              List.fold_left bounds ~init:Map.Poly.empty ~f:(fun subst (tvar, _) ->
                  ...
                  | ConstBool x -> Map.Poly.set subst ~key:tvar ~data:(T_bool.make x)
                  | ConstInt x -> Map.Poly.set subst ~key:tvar ~data:(T_int.mk_int x)
                  | ConstReal x -> Map.Poly.set subst ~key:tvar ~data:(T_real.mk_real x)
                  | _ -> subst)
            in
            let fml = filter_nonconst_args tvars_of_pvar ht_vt @@ Formula.subst subst fml in
            let bounds =
              List.filter bounds ~f:(fst >> Hashtbl.Poly.find_exn ht_vt_of_tvar >> is_const >> not)
            in
  ...
  let f ?(id=None) ?(elim_forall=true) ?(elim_exists=true) ?(elim_pvar_args=true) muclp =
    let elim = Problem.map_formula Formula.(elim_unused_bounds >> elim_let(*ToDo*)) in
    Debug.set_id id;
    if config.enable then
      ...
      |> CheckAndReplaceToTopOrBot.optimize
      |> Problem.simplify
      |> InlineExtension.optimize
    else muclp |> elim
end

let make (config : Config.t) = (module Make (struct let config = config end) : OptimizerType)
