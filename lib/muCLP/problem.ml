...
let avoid_dup pvar pvars =
  if not @@ List.exists pvars ~f:(Ident.pvar_equal pvar) then pvar
    ...
    let suffix = ref 2 in
    while List.exists pvars ~f:(fun pvar' -> Ident.pvar_equal pvar' (Ident.Pvar (Ident.name_of_pvar pvar ^ string_of_int !suffix))) do suffix := !suffix + 1 done;
  ...
  let open Formula in function
    | Atom (atom, info) as fml -> begin
        ...
        | App (Fixpoint (fp, pvar, bounds, body), outer_args, info') ->
          ...
          let additional_params = Set.to_list @@
            Set.diff fvs_of_body
              (Set.inter bound_tvars (Set.Poly.of_list bounds)) in
        ...
        | App (Var (pvar, sorts), args, info') ->
          ...
          in
          Formula.mk_atom (Atom.mk_app new_pred args ~info:info') ~info, preds, used_pvars
...
let str_of muclp =
  sprintf "%s\ns.t.\n%s" (Formula.str_of muclp.query) (Pred.str_of_list muclp.preds)

let has_only_mu muclp = List.for_all ~f:(Pred.fixpoint_of >> Stdlib.(=) Predicate.Mu) muclp.preds
let has_only_nu muclp = List.for_all ~f:(Pred.fixpoint_of >> Stdlib.(=) Predicate.Nu) muclp.preds
let has_only_exists muclp =
  let rec check fml =
    ...
    else if Formula.is_exists fml then check @@ Triple.snd @@ Formula.let_exists fml
    else failwith "not implemented"(*Debug.print @@ lazy (sprintf "not implemented for: %s" @@ Formula.str_of fml)*)
  in
...
let has_only_forall muclp =
  let rec check fml =
    if Formula.is_atom fml then true
      ...
      let _, fml_left, fml_right, _ = Formula.let_binop fml in
      check fml_left && check fml_right
    else if Formula.is_forall fml then check @@ Triple.snd @@ Formula.let_forall fml
...
let aconv_tvar muclp =
  make
    (List.map muclp.preds ~f:(fun (fp, pvar, params, phi) ->
         let map, params' =
           List.unzip @@ List.map params ~f:(fun (x, sort) ->
               let x' = Ident.mk_fresh_tvar () in (x, Term.mk_var x' sort), (x', sort))
         in
         fp, pvar, params', Formula.(subst (Map.Poly.of_alist_exn map) @@ aconv_tvar phi)))
...
let get_dual muclp =
  ...
  let subst formula = List.fold ~init:formula pvars ~f:(Fn.flip Formula.subst_neg) in
  make
    (List.map muclp.preds ~f:(fun (fixpoint, pvar, args, formula) ->
         Predicate.flip_fop fixpoint, pvar, args, Evaluator.simplify_neg (subst formula)))
...
let bind_fvs_with_forall muclp = make muclp.preds @@ Formula.bind_fvs_with_forall muclp.query
...
let detect_undefined_preds muclp =
  ...
  let rec mk_env map = function
    | [] -> map
    | (_, pvar, _, _) :: xs -> mk_env (Map.Poly.add_exn map ~key:pvar ~data:pvar) xs
  in
...
let of_chc ?(only_pos = true) chc =
  ...
  let groups =
    ...
    |> Set.to_list |> List.classify (fun (_, ps1, _, _)  (_, ps2, _, _) ->
  ...
  let goals, defs =
    List.partition_tf groups ~f:(function
        | [] -> assert false | (_senv, p, _ns, _phi) :: _group -> Option.is_none p)
  in
  let preds =
          ...
          Pred.make Predicate.Mu p args
            (Normalizer.normalize @@ Evaluator.simplify @@ Formula.or_of @@
             List.map group ~f:(fun (senv, _, ns, phi) ->
                 let phi =
                   Formula.and_of @@
                   Evaluator.simplify phi :: List.map (Set.to_list ns) ~f:Formula.mk_atom
                 in
                 let senv = Map.Poly.filter_keys senv ~f:(Set.mem @@ Formula.fvs_of phi) in
                 let senv, phi =
                   Pair.map_snd Evaluator.simplify_neg @@
                   Ast.Qelim.qelim_old
                     (Map.of_list_exn @@ Logic.ExtTerm.of_old_sort_env args)
                     (PCSP.Problem.senv_of chc) (senv, Evaluator.simplify_neg phi)
                 in
                 let unbound =
                   Map.to_alist @@ Logic.to_old_sort_env_map Logic.ExtTerm.to_old_sort @@
                   Map.Poly.filter_keys senv ~f:(Fn.non @@ List.Assoc.mem ~equal:Stdlib.(=) args)
                 in
    ...
    | [goals] ->
      ...
      List.map goals ~f:(fun (senv, _, ns, phi) ->
          let senv, phi =
            ...
            in
            let senv =
              Map.Poly.filter_keys senv ~f:(Set.mem @@ Formula.tvs_of phi(*ToDo:also use pvs?*))
            in
            Pair.map_snd Evaluator.simplify_neg @@
  ...
  let undef_preds =
    ...
    |> Map.Poly.filter_keys ~f:(Ident.tvar_to_pvar >> List.mem def_preds ~equal:Stdlib.(=) >> not)
    |> Map.Poly.mapi ~f:(fun ~key:(Ident.Tvar n) ~data ->
        ...
        in
        Pred.make Predicate.Mu(*ToDo*) (Ident.Pvar n) (args sorts) (Formula.mk_false ()))
    |> Map.Poly.data
  in
...
let rec of_lts ?(live_vars=None) ?(cut_points=None) = function
  | lts, LTS.Problem.Term ->
    ...
    let tenv_of =
      ...
      | None ->
        let tenv =
          Set.to_list @@ Set.filter ~f:(fst >> Ident.name_of_tvar >> String.is_prefix ~prefix:LTS.Problem.nondet_prefix >> not) @@ LTS.Problem.term_sort_env_of lts
        in
        fun _ -> tenv
      | Some live_vars ->
        fun s ->
          try Set.to_list (live_vars s) with Stdlib.Not_found -> failwith ("not found: " ^ s)
    in
          ...
          | (from, c, to_) :: trs ->
            let next =
              ...
              |> List.map ~f:(fun (c, to_) ->
                  ...
                  let phi =
                    LTS.Problem.wp c (Formula.mk_atom @@ Atom.pvar_app_of_senv pvar senv)
                  in
                  let nondet_tenv =
                    Set.to_list @@ Set.filter (Formula.term_sort_env_of phi)
            ...
            | Some cut_points when not @@ Set.mem cut_points from ->
              First (Pred.make Predicate.Fix (pvar_of from) (tenv_of from) next)
            | _ -> Second (Pred.make Predicate.Mu (pvar_of from) (tenv_of from) next))
    in
    (match start with
     | None ->
     ...
     | Some start ->
       let undef_preds =
         Set.diff (Set.add (Set.Poly.of_list @@ List.map transitions ~f:Triple.trd) start)
           (Set.Poly.of_list @@ List.map transitions ~f:Triple.fst)
         |> Set.Poly.map ~f:(fun from ->
             Pred.make Predicate.Fix (pvar_of from) (tenv_of from) (Formula.mk_true ()))
         |> Set.to_list
       in
       let query =
         let tenv = tenv_of start in
         Formula.mk_forall_if_bounded tenv @@ Formula.mk_atom @@
         Atom.mk_pvar_app (pvar_of start) (List.map ~f:snd tenv) (Term.of_sort_env tenv)
       in
       make (preds_mu @ preds_nu @ undef_preds) query)
  | lts, LTS.Problem.NonTerm ->
    get_dual @@ of_lts ~live_vars ~cut_points (lts, LTS.Problem.Term)
  | _, (LTS.Problem.Safe | LTS.Problem.NonSafe | LTS.Problem.MuCal | LTS.Problem.Rel) ->
