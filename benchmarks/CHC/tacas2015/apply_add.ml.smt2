(set-logic HORN)
(set-info :source |
  Benchmark: ./benchmarks/ocaml/safety/tacas2015//apply_add.ml
  Generated by Refinement Caml
|)
(set-info :status unknown)
(declare-fun |$P5| (Int Int) Bool)
(declare-fun |$P4| (Int Int) Bool)
(declare-fun |$P0| (Int) Bool)
(declare-fun |$P1| (Int Int) Bool)
(declare-fun |$P2| (Int) Bool)
(declare-fun |$P3| (Int Int Int) Bool)
(declare-fun |$P6| (Int Int Int) Bool)
(assert (forall ((x1 Int)(x0 Int)) (=> (|$P5| x0 x1) (|$P2| x1))))
(assert (forall ((x0 Int)(x1 Int)(x2 Int)) (=> (and (|$P5| x0 x1) (|$P4| x1 x2)) (|$P6| x0 x1 x2))))
(assert (forall ((x1 Int)(x2 Int)(x0 Int)) (=> (and (|$P0| x1) (and (|$P1| x1 x2) (= x0 (+ x1 x2)))) (|$P3| x1 x2 x0))))
(assert (forall ((x0 Int)) (|$P0| x0)))
(assert (forall ((x0 Int)(x1 Int)) (=> (|$P2| x1) (|$P1| x0 x1))))
(assert (forall ((x1 Int)(x2 Int)(x0 Int)) (=> (and (|$P2| x1) (|$P3| x0 x1 x2)) (|$P4| x1 x2))))
(assert (forall ((x0 Int)(x1 Int)) (=> (= x1 0) (|$P5| x0 x1))))
(assert (not (exists ((x2 Int)(x0 Int)(x1 Int)) (and (|$P6| x1 0 x2) (<= (+ 1 x2) x0)))))
(check-sat)
