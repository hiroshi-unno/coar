...
let[@annot_MB "(bool -> ({Set: s1} |> bool / (s2 / ?) => (s2 / ?))) -> ?"] rec f g =
  let b1 = perform Get in
  if g b1 then ()
  else
    let b2 = perform Get in
    if b1 = b2 then f g else perform Raise
      ...
      false)
    {
      ...
      effc =
        (fun (type b) (e : b eff) ->
          match e with
          | Set _x ->
              Some (fun (k : (b, _) continuation) (s : bool) -> continue k () s)
          | Get -> Some (fun k (s : bool) -> continue k s s)
          | Raise -> Some (fun _k (_s : bool) -> perform Raise));
    }
    true
