...
if [ -f $1 ]; then 
    fname=`basename $1` 
    _build/default/main.exe -c ./config/solver/printer.json -p cltl $1 > $2/$fname.hes
...
do  
    ...
    if [ -f $1/$fname ]; then 
        _build/default/main.exe -c ./config/solver/printer.json -p cltl $1/$fname > $2/$fname.hes
        echo "testing $2/$fname.hes"
        _build/default/main.exe -c ./config/solver/printer.json -p muclp $2/$fname.hes 1> /dev/null
    fi
    if [ -d $1/$fname ]; then 
